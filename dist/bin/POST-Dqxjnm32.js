import{createRequire as F}from"module";import{d,g as M,p as k,a as W,w as P,x as H,c as B,r as Y,l as U,k as J}from"../server.bundle.mjs";import{s as l,j as q}from"./drizzle-CZ2vWAZ3.js";import"./express-C3pMhyZZ.js";import"tty";import"os";import"util";import"path";import"./iconv-DatUeE_T.js";import"buffer";import"string_decoder";import"node:zlib";import"./es-abstract-B57Whf45.js";import"url";import"node:fs";import"node:path";import"fs";import"crypto";import"node:http";import"node:buffer";import"node:querystring";import"node:net";import"stream";import"node:events";import"./mysql2-HVwznT_d.js";import"net";import"events";import"process";import"timers";import"tls";import"zlib";import"node:process";import"./better-auth-B2r-Xofv.js";import"./kysely-9kO2vj_T.js";import"node:crypto";import"./noble-DY0vnRA-.js";import"node:fs/promises";import"node:os";import"./multer-RX-gfMuA.js";import"fs/promises";import"./jszip-BoIs7TcQ.js";import"node:util";import"http";import"https";import"assert";import"./pdf-lib-MF9mTnF3.js";const Z=F(import.meta.url);async function c(t,e,g,s,p){try{const h=await e();return p[t]=Array.isArray(h)?h.length:1,h}catch(h){const f=String((h==null?void 0:h.message)??h);return console.warn(`[annette-context] ${t} FAILED: ${f}`),s.push(`${t}: ${f.slice(0,120)}`),p[t]=-1,g}}function G(t,e){const g=new Date(t);return Math.round((e.getTime()-g.getTime())/864e5)}async function X(t,e,g){const s=new Date,p=s.toISOString().slice(0,10),h=new Date(s.getTime()+14*864e5).toISOString().slice(0,10),f=new Date(s.getTime()-14*864e5).toISOString().slice(0,10),i=[],r={},n={companyName:g,companyId:t,runAt:s.toISOString(),seeDollars:e.seeDollars,warnings:i,moduleCounts:r,jobs:{total:0,byStatus:{},noForms:[],approvedNoProgress:[],noPhotos:[],noFiles:[],stalled:[]},todos:{overdueCount:0,dueTodayCount:0,overdue:[],dueToday:[]},fleet:{total:0,serviceOverdue:[],regoOverdue:[],serviceDue14:[],regoDue14:[],openFlags:[],noPrestartDays:null},estimates:{draftTooLong:[],pendingApproval:[]},forms:{incompleteSubmissions:[],jobsWithNoForms:0},shareLinks:{total:0,expired:[],maxed:[]}};return e.canJobs&&(await c("jobs_summary",async()=>{const[o]=await d.execute(l`SELECT status, COUNT(*) as cnt FROM jobs
            WHERE company_id = ${t} GROUP BY status`);let a=0;for(const u of o??[])n.jobs.byStatus[u.status]=Number(u.cnt),a+=Number(u.cnt);return n.jobs.total=a,o},[],i,r),await c("jobs_no_forms",async()=>{const[o]=await d.execute(l`SELECT j.id, j.job_number, j.name, j.status
            FROM jobs j
            ...
            ORDER BY j.created_at DESC LIMIT 20`);return n.jobs.noForms=o??[],n.forms.jobsWithNoForms=(o??[]).length,o},[],i,r),await c("jobs_approved_no_progress",async()=>{const[o]=await d.execute(l`SELECT j.id, j.job_number, j.name
            FROM jobs j
            WHERE j.company_id = ${t}
            ...
            ORDER BY j.created_at DESC LIMIT 20`);return n.jobs.approvedNoProgress=o??[],o},[],i,r),await c("jobs_no_photos",async()=>{const[o]=await d.execute(l`SELECT j.id, j.job_number, j.name, j.status
            FROM jobs j
            WHERE j.company_id = ${t}
              AND j.status NOT IN ('Completed','Cancelled','Archived')
              AND NOT EXISTS (
            ...
            ORDER BY j.created_at DESC LIMIT 20`);return n.jobs.noPhotos=o??[],o},[],i,r),await c("jobs_no_files",async()=>{const[o]=await d.execute(l`SELECT j.id, j.job_number, j.name, j.status
            FROM jobs j
            WHERE j.company_id = ${t}
              AND j.status NOT IN ('Completed','Cancelled','Archived')
              AND NOT EXISTS (
            ...
            ORDER BY j.created_at DESC LIMIT 20`);return n.jobs.noFiles=o??[],o},[],i,r),await c("jobs_stalled",async()=>{const[o]=await d.execute(l`SELECT j.id, j.job_number, j.name, j.status,
                   DATEDIFF(NOW(), j.updated_at) as days_since_update
            FROM jobs j
            WHERE j.company_id = ${t}
              AND j.status IN ('Active','In Progress','Approved')
              AND j.updated_at < ${f}
            ORDER BY j.updated_at ASC LIMIT 15`);return n.jobs.stalled=(o??[]).map(a=>({...a,days_since_update:Number(a.days_since_update)})),o},[],i,r)),e.canJobs&&(await c("todos_overdue",async()=>{const[o]=await d.execute(l`SELECT t.id, t.title, j.name as job_name, t.due_date,
            ...
            WHERE j.company_id = ${t}
              AND t.status = 'Open'
              AND t.due_date < ${p}
            ORDER BY t.due_date ASC LIMIT 30`);return n.todos.overdue=(o??[]).map(a=>({...a,days_overdue:Number(a.days_overdue)})),n.todos.overdueCount=n.todos.overdue.length,o},[],i,r),await c("todos_today",async()=>{const[o]=await d.execute(l`SELECT t.id, t.title, j.name as job_name, t.due_date
            FROM job_todos t
            JOIN jobs j ON j.id = t.job_id
            WHERE j.company_id = ${t}
              AND t.status = 'Open'
              AND t.due_date = ${p}
            ORDER BY t.title ASC LIMIT 20`);return n.todos.dueToday=o??[],n.todos.dueTodayCount=n.todos.dueToday.length,o},[],i,r)),e.canFleet&&(await c("fleet_total",async()=>{var a;const[o]=await d.execute(l`SELECT COUNT(*) as cnt FROM fleet_assets
            WHERE company_id = ${t} AND archived = 0`);return n.fleet.total=Number(((a=o==null?void 0:o[0])==null?void 0:a.cnt)??0),o},[],i,r),await c("fleet_service_overdue",async()=>{const[o]=await d.execute(l`SELECT id, name, rego, DATE(service_date) as service_date,
                   DATEDIFF(NOW(), service_date) as days_overdue
            FROM fleet_assets
            WHERE company_id = ${t} AND archived = 0
              AND service_date IS NOT NULL AND DATE(service_date) < ${p}
            ORDER BY service_date ASC LIMIT 20`);return n.fleet.serviceOverdue=(o??[]).map(a=>({...a,days_overdue:Number(a.days_overdue)})),o},[],i,r),await c("fleet_rego_overdue",async()=>{const[o]=await d.execute(l`SELECT id, name, rego, DATE(rego_expiry) as rego_expiry,
                   DATEDIFF(NOW(), rego_expiry) as days_overdue
            FROM fleet_assets
            WHERE company_id = ${t} AND archived = 0
              AND rego_not_applicable = 0
              AND rego_expiry IS NOT NULL AND DATE(rego_expiry) < ${p}
            ORDER BY rego_expiry ASC LIMIT 20`);return n.fleet.regoOverdue=(o??[]).map(a=>({...a,days_overdue:Number(a.days_overdue)})),o},[],i,r),await c("fleet_service_due14",async()=>{const[o]=await d.execute(l`SELECT id, name, rego, DATE(service_date) as service_date,
                   DATEDIFF(service_date, NOW()) as days_until
            FROM fleet_assets
            WHERE company_id = ${t} AND archived = 0
              AND service_date IS NOT NULL
              AND DATE(service_date) >= ${p} AND DATE(service_date) <= ${h}
            ORDER BY service_date ASC LIMIT 20`);return n.fleet.serviceDue14=(o??[]).map(a=>({...a,days_until:Number(a.days_until)})),o},[],i,r),await c("fleet_rego_due14",async()=>{const[o]=await d.execute(l`SELECT id, name, rego, DATE(rego_expiry) as rego_expiry,
                   DATEDIFF(rego_expiry, NOW()) as days_until
            FROM fleet_assets
            WHERE company_id = ${t} AND archived = 0
              AND rego_not_applicable = 0
              AND rego_expiry IS NOT NULL
              AND DATE(rego_expiry) >= ${p} AND DATE(rego_expiry) <= ${h}
            ORDER BY rego_expiry ASC LIMIT 20`);return n.fleet.regoDue14=(o??[]).map(a=>({...a,days_until:Number(a.days_until)})),o},[],i,r),await c("fleet_flags",async()=>{const[o]=await d.execute(l`SELECT fa.name as asset_name, fp.issue_comment, fp.created_at as flagged_at
            FROM fleet_prestarts fp
            JOIN fleet_assets fa ON fa.id = fp.asset_id
            WHERE fp.company_id = ${t}
              AND fp.issue_needs_attention = 1
            ORDER BY fp.created_at DESC LIMIT 20`);return n.fleet.openFlags=o??[],o},[],i,r),await c("fleet_last_prestart",async()=>{var u;const[o]=await d.execute(l`SELECT MAX(created_at) as last_at FROM fleet_prestarts
            WHERE company_id = ${t}`),a=(u=o==null?void 0:o[0])==null?void 0:u.last_at;return n.fleet.noPrestartDays=a?G(a,s):999,o},[],i,r)),e.canEstimating&&(await c("estimates_draft_long",async()=>{const[o]=await d.execute(l`SELECT e.id, j.name as job_name, e.title,
            ...
            ORDER BY e.created_at ASC LIMIT 15`);return n.estimates.draftTooLong=(o??[]).map(a=>({...a,days_in_draft:Number(a.days_in_draft),amount:e.seeDollars?Number(a.amount):void 0})),o},[],i,r),await c("estimates_pending",async()=>{const[o]=await d.execute(l`SELECT e.id, j.name as job_name, e.title,
            ...
            ORDER BY e.updated_at ASC LIMIT 15`);return n.estimates.pendingApproval=(o??[]).map(a=>({...a,days_pending:Number(a.days_pending),amount:e.seeDollars?Number(a.amount):void 0})),o},[],i,r)),e.canForms&&await c("forms_incomplete",async()=>{const[o]=await d.execute(l`SELECT ft.name as form_name, j.name as job_name, jfs.created_at as submitted_at
...
`);A=$.pop()??"";for(const v of $){if(!v.startsWith("data: "))continue;const T=v.slice(6).trim();if(T!=="[DONE]")try{const R=(h=(p=(s=JSON.parse(T).choices)==null?void 0:s[0])==null?void 0:p.delta)==null?void 0:h.content;R&&N(R)}catch{}}}O({warnings:b.warnings,moduleCounts:b.moduleCounts})}catch(f){const i=String((f==null?void 0:f.message)??f);console.error("[annette] error:",i),e.headersSent?(e.write(`data: ${JSON.stringify({text:`

⚠️ Error: ${i}`})}

`),e.write(`data: ${JSON.stringify({done:!0,error:!0})}

`),e.end()):e.status(500).json({error:i})}}export{Je as default};
