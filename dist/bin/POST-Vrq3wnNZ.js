import{createRequire as C}from"module";import{g as j,d as U,p as _,a as k,w as E,b as D,c as N,r as T,e as A,f as W,h as B,i as R,j as O,k as H}from"../server.bundle.mjs";import{j as J}from"./drizzle-Bu3hp1FF.js";import"./express-WU0_rYSr.js";import"tty";import"os";import"util";import"path";import"./iconv-DatUeE_T.js";import"buffer";import"string_decoder";import"node:zlib";import"./es-abstract-B57Whf45.js";import"url";import"node:fs";import"node:path";import"fs";import"crypto";import"node:http";import"node:buffer";import"node:querystring";import"node:net";import"stream";import"node:events";import"./radix-ui-BjnQoRNV.js";import"./react-router-8V4KISLV.js";import"./react-dom-D6zqUVF0.js";import"async_hooks";import"node:url";import"./mysql2-BJ7xsy_S.js";import"net";import"events";import"process";import"timers";import"tls";import"zlib";import"node:process";import"./better-auth-CQVzbiZe.js";import"./kysely-9kO2vj_T.js";import"node:crypto";import"./noble-DY0vnRA-.js";import"./opentelemetry-MOfsRXlr.js";import"node:fs/promises";import"node:os";import"./multer-DNTYjcWN.js";import"fs/promises";import"http";import"https";import"assert";const Q=C(import.meta.url);function M(s){if(!/^[0-9\s+\-*/.()%]+$/.test(s)||s.length>200)return null;let a=0;const c=()=>s[a],d=()=>s[a++],u=()=>{for(;a<s.length&&s[a]===" ";)a++};function r(){let p=f();for(u();a<s.length&&(c()==="+"||c()==="-");){const $=d();u();const S=f();p=$==="+"?p+S:p-S,u()}return p}function f(){let p=h();for(u();a<s.length&&(c()==="*"||c()==="/"||c()==="%");){const $=d();u();const S=h();$==="*"?p*=S:$==="/"?p=S!==0?p/S:NaN:p=p%S,u()}return p}function h(){if(u(),c()==="("){d();const $=r();return u(),c()===")"&&d(),$}if(c()==="-")return d(),-h();if(c()==="+")return d(),h();let p="";for(;a<s.length&&/[0-9.]/.test(s[a]);)p+=d();return p?parseFloat(p):NaN}try{const p=r();return isFinite(p)?p:null}catch{return null}}function Y(s){const a=s.trim().slice(0,500),c=a.match(/^(?:what\s+is\s+|calculate\s+|calc\s+|work\s+out\s+)?([0-9\s+\-*/.()%]+)=?$/i);if(c){const t=c[1].trim(),o=M(t);if(o!==null)return`${o}`}const d=a.match(/(?:add\s+gst\s+to|gst\s+on|plus\s+gst|add\s+10%\s+to)\s*\$?([\d,]+(?:\.\d+)?)/i)??a.match(/\$?([\d,]+(?:\.\d+)?)\s*\+\s*gst/i);if(d){const t=parseFloat(d[1].replace(/,/g,""));if(!isNaN(t)){const o=+(t*.1).toFixed(2),n=+(t+o).toFixed(2);return`GST calculation:
• Base: $${t.toLocaleString("en-AU",{minimumFractionDigits:2})}
• GST (10%): $${o.toLocaleString("en-AU",{minimumFractionDigits:2})}
• Total inc. GST: $${n.toLocaleString("en-AU",{minimumFractionDigits:2})}`}}const u=a.match(/(?:remove\s+gst\s+from|ex\s+gst\s+|excluding\s+gst\s+|gst\s+exclusive\s+of)\s*\$?([\d,]+(?:\.\d+)?)/i)??a.match(/\$?([\d,]+(?:\.\d+)?)\s+ex\.?\s+gst/i);if(u){const t=parseFloat(u[1].replace(/,/g,""));if(!isNaN(t)){const o=+(t/1.1).toFixed(2),n=+(t-o).toFixed(2);return`GST removal:
• Total inc. GST: $${t.toLocaleString("en-AU",{minimumFractionDigits:2})}
• GST (10%): $${n.toLocaleString("en-AU",{minimumFractionDigits:2})}
• Base ex. GST: $${o.toLocaleString("en-AU",{minimumFractionDigits:2})}`}}const r=a.match(/add\s+([\d.]+)%\s+markup\s+(?:to\s+)?\$?([\d,]+(?:\.\d+)?)/i);if(r){const t=parseFloat(r[1]),o=parseFloat(r[2].replace(/,/g,""));if(!isNaN(t)&&!isNaN(o)){const n=+(o*t/100).toFixed(2),i=+(o+n).toFixed(2);return`Markup calculation (${t}%):
• Cost: $${o.toLocaleString("en-AU",{minimumFractionDigits:2})}
• Markup (${t}%): $${n.toLocaleString("en-AU",{minimumFractionDigits:2})}
• Sell price: $${i.toLocaleString("en-AU",{minimumFractionDigits:2})}`}}const f=a.match(/(?:what\s+is\s+)?(\d+(?:\.\d+)?)%\s+margin\s+on\s+\$?([\d,]+(?:\.\d+)?)/i);if(f){const t=parseFloat(f[1]),o=parseFloat(f[2].replace(/,/g,""));if(!isNaN(t)&&!isNaN(o)&&t<100){const n=+(o/(1-t/100)).toFixed(2),i=+(n-o).toFixed(2);return`Margin calculation (${t}%):
• Cost: $${o.toLocaleString("en-AU",{minimumFractionDigits:2})}
• Margin (${t}%): $${i.toLocaleString("en-AU",{minimumFractionDigits:2})}
• Sell price: $${n.toLocaleString("en-AU",{minimumFractionDigits:2})}`}}const h=a.match(/concrete.*?(\d+(?:\.\d+)?)\s*(?:m|metres?|meters?)?\s*[x×*by]\s*(\d+(?:\.\d+)?)\s*(?:m|metres?|meters?)?\s*[x×*by]\s*(\d+(?:\.\d+)?)\s*(m|mm|metres?|meters?|millimetres?)?/i);if(h){const t=parseFloat(h[1]),o=parseFloat(h[2]);let n=parseFloat(h[3]);if((h[4]??"m").toLowerCase().startsWith("mm")&&(n=n/1e3),!isNaN(t)&&!isNaN(o)&&!isNaN(n)&&n>0){const l=+(t*o*n).toFixed(3),m=+(l*1.1).toFixed(3);return`Concrete volume:
• Slab: ${t}m × ${o}m × ${n<1?(n*1e3).toFixed(0)+"mm":n+"m"}
• Volume: **${l} m³**
• With 10% waste: **${m} m³**

_Order at least ${m} m³. Verify with your concrete supplier._`}}const p=a.match(/(?:area\s+of|what\s+is\s+the\s+area)\s+(\d+(?:\.\d+)?)\s*(?:m|metres?|meters?)?\s*(?:x|×|\*|by)\s*(\d+(?:\.\d+)?)/i);if(p){const t=parseFloat(p[1]),o=parseFloat(p[2]);if(!isNaN(t)&&!isNaN(o)){const n=+(t*o).toFixed(2);return`Area calculation:
• ${t}m × ${o}m = **${n} m²**`}}const $=a.match(/fall\s+(?:for\s+)?(\d+(?:\.\d+)?)\s*m.*?(?:at\s+)?1\s*(?::|in)\s*(\d+(?:\.\d+)?)/i);if($){const t=parseFloat($[1]),o=parseFloat($[2]);if(!isNaN(t)&&!isNaN(o)&&o>0){const n=+(t/o*1e3).toFixed(0),i=+(t/o).toFixed(3);return`Pipe fall calculation:
• Length: ${t}m at 1:${o}
• Fall: **${n}mm** (${i}m)
• Invert drop: ${n}mm over ${t}m`}}const S=a.match(/(?:what\s+is\s+)?(\d+(?:\.\d+)?)%\s+of\s+\$?([\d,]+(?:\.\d+)?)/i);if(S){const t=parseFloat(S[1]),o=parseFloat(S[2].replace(/,/g,""));if(!isNaN(t)&&!isNaN(o)){const n=+(o*t/100).toFixed(2);return`${t}% of $${o.toLocaleString("en-AU",{minimumFractionDigits:2})} = **$${n.toLocaleString("en-AU",{minimumFractionDigits:2})}**`}}const e=a.match(/perimeter\s+(?:of\s+)?(\d+(?:\.\d+)?)\s*(?:m|metres?|meters?)?\s*(?:x|×|\*|by)\s*(\d+(?:\.\d+)?)/i);if(e){const t=parseFloat(e[1]),o=parseFloat(e[2]);if(!isNaN(t)&&!isNaN(o)){const n=+(2*(t+o)).toFixed(2);return`Perimeter:
• ${t}m × ${o}m rectangle = **${n} lineal metres**`}}return null}function q(s,a){var r,f,h,p,$,S;const c=s.toLowerCase().trim(),d=a.permissions,u=a.companyName;if(/another company|other company|different company|competitor|someone else'?s?\s+(quote|job|data|estimate)/i.test(c))return`I can't access another company's private IWILLBUILD data. I only have access to ${u}'s data.`;if(/how many jobs|job count|number of jobs|total jobs/i.test(c)){if(!d.canJobs)return"You don't have Jobs access.";const e=((r=a.jobs)==null?void 0:r.length)??0;return e===0?`📋 From IWILLBUILD data:
...
📊 Confidence:
High`}if(/how many estimate|estimate count|number of estimate|how many quote/i.test(c)){if(!d.canEstimating)return"You don't have Estimating access.";const e=((h=a.estimates)==null?void 0:h.length)??0;if(e===0)return`📋 From IWILLBUILD data:
No estimates found for ${u} yet.
...
`)}function V(s){return[`📋 From IWILLBUILD data:
...
`)}function z(s,a){return[`🧠 AI reasoning:
...
`)}export{Pe as buildSystemPrompt,Ge as default};
