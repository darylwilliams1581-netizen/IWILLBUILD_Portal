...
function InlineEditor({
  member, callerIsOwner, onClose, onSuccess,
}: {
  member: TeamMember; callerIsOwner: boolean;
  onClose: () => void; onSuccess: (updated: TeamMember) => void;
}) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      onSuccess({ ...member, role, status, permissions: perms });
      onClose();
    ...
    >
      <div className="px-4 pb-4 pt-1 border-t border-slate-100 bg-slate-50/60">
        {targetIsOwner && !callerIsOwner && (
          <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2.5 mb-3 text-xs text-amber-700 font-semibold">
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div>
            ...
            {roleLocked ? (
              <div className="flex items-center gap-1.5 px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs text-slate-500">
                <Lock size={11} /> Owner
              </div>
            ) : (
              <select value={role} onChange={(e) => setRole(e.target.value as Role)}
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary">
          ...
          <div>
            ...
            <select value={status} onChange={(e) => setStatus(e.target.value as Status)}
              ...
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary disabled:opacity-50">
            ...
            {PERMISSIONS.map(({ key, label }) => {
              ...
              return (
                <button key={key} type="button"
                  onClick={() => { if (!permsLocked) setPerms(p => ({ ...p, [key]: !p[key] })); }}
                  disabled={permsLocked}
                  className={`flex items-center justify-between px-3 py-2 rounded-lg border text-xs transition-colors ${
                    on ? 'bg-primary/5 border-primary/20 text-slate-800 font-semibold' : 'bg-white border-slate-200 text-slate-400'
                  } ${permsLocked ? 'cursor-not-allowed opacity-60' : 'hover:border-primary/40 cursor-pointer'}`}
                >
                  <span>{label}</span>
                  {on
                    ? <ToggleRight size={16} className="text-primary shrink-0" />
        ...
        <div className="flex gap-2">
          <button onClick={onClose}
            className="flex-1 border border-slate-200 text-slate-600 text-xs font-semibold py-2 rounded-lg hover:bg-white transition-colors">
            Cancel
          </button>
...
export default function TeamPermissionsTab({ isAdmin }: { isAdmin: boolean }) {
  ...
  const { isOwner, isPlatformOwner } = usePermissions();
            ...
            return (
              <div key={member.id}
                className={`border rounded-xl transition-colors overflow-hidden ${
                  isExpanded ? 'border-primary/30 bg-white shadow-sm' : 'border-slate-200 bg-white hover:border-slate-300'
                }`}
              >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {activePerms.length > 0 ? (
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {activePerms.map(p => (
                          <span key={p.key}
                            className="text-[9px] font-semibold bg-primary/8 text-primary border border-primary/15 px-1.5 py-0.5 rounded-full">
                            {p.label}
                          </span>
                        ))}
                      </div>
                    ) : (
                      <p className="text-[10px] text-slate-300 mt-1 italic">No permissions assigned</p>
                    )}
                  </div>
