/**
 * Reusable email dialog for documents that are generated and attached by the
 * server. This is the standard document-email path for quotes, invoices and
 * completed forms; it deliberately does not use mailto:, which cannot attach
 * generated files reliably.
 */
import { useState } from 'react';
import { AnimatePresence, motion } from 'motion/react';
import { AlertCircle, CheckCircle2, FileText, Loader2, Mail, X } from 'lucide-react';
        ...
        >
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-bold text-gray-900">Email {documentLabel}</p>
                <p className="text-xs text-muted-foreground truncate max-w-[260px]">{documentName}</p>
              </div>
            </div>
          ...
          {sent ? (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="font-semibold text-gray-900">{documentLabel} sent!</p>
              <p className="text-sm text-muted-foreground">
                The email was sent to <strong>{email}</strong> with the PDF attached.
              </p>
              <button
                onClick={onClose}
            ...
            <>
              <div className="flex items-start gap-2.5 bg-violet-50 border border-violet-100 rounded-xl px-3 py-3 mb-4">
                <FileText size={15} className="text-violet-600 mt-0.5 shrink-0" />
                <p className="text-xs text-violet-900">
                  The latest PDF will be generated and attached automatically. The recipient does not need an IWILLBUILD login.
                </p>
              </div>
