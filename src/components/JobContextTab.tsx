...
import { HardHat, X, ExternalLink, MapPin, User, ChevronUp, ChevronDown, Briefcase } from 'lucide-react';
...
export default function JobContextTab() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/jobs/${jobId}`, {
      credentials: 'include'
    }).then(r => r.ok ? r.json() as Promise<JobApiResponse> : Promise.resolve({
      error: 'not found'
    })).then(d => {
  ...
  return <>
      ...
      <div className="hidden md:block">
        {/* Vertical pill tab */}
        <button onClick={() => setOpen(v => !v)} className="fixed right-0 top-1/2 -translate-y-1/2 z-40 flex flex-col items-center gap-1.5 bg-primary text-white px-2 py-4 rounded-l-xl shadow-lg hover:bg-violet-700 transition-colors" title={open ? 'Close job panel' : `Job: ${displayName}`} style={{
        writingMode: 'vertical-rl',
        textOrientation: 'mixed'
      }}>
          <HardHat size={14} className="shrink-0 rotate-90" style={{
        ...
        <AnimatePresence>
          {open && <>
            ...
            duration: 0.15
          }} className="fixed inset-0 z-30 bg-black/20" onClick={() => setOpen(false)} />
              {/* Panel */}
              <motion.div initial={{
            x: '100%'
          }} animate={{
            ...
            damping: 38
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50">
                  ...
                  <button onClick={() => setOpen(false)} className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-600 hover:text-slate-800 hover:bg-slate-200 transition-colors">
                    <X size={14} />
                  </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
                  <div>
                    ...
                    {job.jobNumber && <p className="text-xs text-primary font-semibold mt-0.5">#{job.jobNumber}</p>}
                  </div>
                ...
                <div className="p-4 border-t border-slate-100">
                  <Link to={`/jobs/${job.id}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-xl bg-primary text-white text-sm font-bold hover:bg-violet-700 transition-colors">
