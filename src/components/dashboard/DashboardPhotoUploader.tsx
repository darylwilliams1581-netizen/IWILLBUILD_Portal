...
import {
  Camera, Search, ChevronDown, X, Upload, CheckCircle2,
  AlertCircle, Loader2, ImagePlus, ExternalLink, RotateCcw,
} from 'lucide-react';
...
function JobSelector({
  ...
  async function fetchJobs(q: string) {
    setLoading(true);
    try {
      const params = new URLSearchParams({ status: 'active', limit: '30' });
      if (q) params.set('q', q);
    ...
    <div ref={containerRef} className="relative">
      ...
      >
        ...
        {selected ? (
          <button
            type="button"
            onClick={e => { e.stopPropagation(); onSelect(null as unknown as JobOption); }}
              ...
              {jobs.length === 0 && !loading ? (
                ...
                jobs.map(job => (
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 px-3.5 py-2.5 text-left hover:bg-muted/60 transition-colors group"
                  >
    ...
    <div className="flex items-center gap-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          {entry.status === 'error' && (
            <span className="text-[10px] font-semibold text-destructive flex items-center gap-1 truncate">
              <AlertCircle size={10} /> {entry.error ?? 'Failed'}
            </span>
          )}
          {entry.status === 'pending' && (
            <span className="text-[10px] text-muted-foreground">{formatBytes(entry.file.size)}</span>
          )}
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          {done ? (
            ...
            <motion.div key="form" className="flex flex-col gap-3">
              ...
              {/* Upload button */}
              <AnimatePresence>
                {pendingFiles.length > 0 && selectedJob && (
                  <motion.button
                    ...
                    onClick={uploadAll}
                    disabled={uploading}
                    className="w-full flex items-center justify-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-semibold py-2.5 rounded-xl transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    {uploading ? (
