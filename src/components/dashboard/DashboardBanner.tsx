...
const STYLE_MAP: Record<string, { bg: string; border: string; text: string; subtext: string; iconColor: string }> = {
  standard:    { bg: 'bg-slate-50',   border: 'border-slate-200', text: 'text-slate-800',  subtext: 'text-slate-500',  iconColor: 'text-slate-500' },
  safety:      { bg: 'bg-amber-50',   border: 'border-amber-200', text: 'text-amber-900',  subtext: 'text-amber-700',  iconColor: 'text-amber-600' },
  seasonal:    { bg: 'bg-emerald-50', border: 'border-emerald-200', text: 'text-emerald-900', subtext: 'text-emerald-700', iconColor: 'text-emerald-600' },
  celebration: { bg: 'bg-purple-50',  border: 'border-purple-200', text: 'text-purple-900', subtext: 'text-purple-700', iconColor: 'text-purple-600' },
  warning:     { bg: 'bg-red-50',     border: 'border-red-200',   text: 'text-red-900',    subtext: 'text-red-700',    iconColor: 'text-red-600' },
};

const ICON_MAP: Record<string, React.ComponentType<{ size?: number; className?: string }>> = {
  Megaphone, ShieldAlert, Gift, Sparkles, Sun, Heart, Shield, TrendingUp, Zap, Clover,
};

function BannerIcon({ name, style, size = 14 }: { name: string; style: string; size?: number }) {
...
export default function DashboardBanner({ userId = 'anon' }: DashboardBannerProps) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!res.ok) return;
      const data = await res.json() as { banner?: { dashboardBanner?: BannerConfig; seasonalSkins?: SeasonalSkinsConfig } };
  ...
  if (showCompany && bannerConfig) {
    ...
    const iconName = bannerConfig.style === 'safety' ? 'ShieldAlert'
      : bannerConfig.style === 'warning' ? 'ShieldAlert'
      : bannerConfig.style === 'celebration' ? 'Sparkles'
      : bannerConfig.style === 'seasonal' ? 'Gift'
      : 'Megaphone';
    ...
    return (
      <div
        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border ${s.bg} ${s.border} min-w-0 flex-1 mx-2 md:mx-4 print:hidden`}
        role="status"
        aria-live="polite"
      >
        ...
        {bannerConfig.linkLabel && bannerConfig.linkUrl && (
          isExternal ? (
            <a
              href={bannerConfig.linkUrl}
              target="_blank"
              rel="noopener noreferrer"
              className={`flex items-center gap-1 text-xs font-semibold shrink-0 underline underline-offset-2 ${s.text} hover:opacity-70 transition-opacity`}
            >
            ...
            <Link
              ...
              className={`text-xs font-semibold shrink-0 underline underline-offset-2 ${s.text} hover:opacity-70 transition-opacity`}
            >
              {bannerConfig.linkLabel}
            </Link>
          )
        ...
        {bannerConfig.dismissible && (
          <button
            ...
            className={`p-0.5 rounded hover:bg-black/10 transition-colors shrink-0 ${s.iconColor}`}
  ...
  if (showSeasonal && seasonalEvent) {
    ...
    return (
      <div
        className="flex items-center gap-2 px-3 py-1.5 rounded-lg border min-w-0 flex-1 mx-2 md:mx-4 print:hidden"
        style={{
        ...
        aria-live="polite"
      >
        <span style={{ color: seasonalEvent.accentColor }}>
          <BannerIcon name={iconName} style="standard" size={14} />
        </span>
        <div className="flex-1 min-w-0 flex items-baseline gap-2 flex-wrap">
          <span className="text-xs font-bold shrink-0" style={{ color: seasonalEvent.accentColor }}>
            {seasonalEvent.bannerTitle}
          </span>
          {!subtle && (
            <span className="text-xs truncate text-slate-500">{seasonalEvent.bannerMessage}</span>
          )}
        </div>
      </div>
    );
