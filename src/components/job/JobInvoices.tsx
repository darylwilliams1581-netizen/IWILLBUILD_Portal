...
const STATUS_ICONS: Record<string, React.ElementType> = {
  draft: FileText, sent: Send, partially_paid: Clock,
  paid: CheckCircle2, overdue: AlertTriangle, void: XCircle,
};
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-white border border-border rounded-xl p-5">
        ...
        {/* Stats */}
        {seeDollars && invoices.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: 'Total Invoiced', value: fmtMoney(totalInvoiced), color: 'text-slate-700' },
              { label: 'Paid', value: fmtMoney(totalPaid), color: 'text-emerald-600' },
              { label: 'Balance Due', value: fmtMoney(balanceDue), color: balanceDue > 0 ? 'text-red-600' : 'text-emerald-600' },
              { label: 'Invoices', value: `${invoices.length} total`, color: 'text-slate-700' },
            ].map((s) => (
        ...
        {!seeDollars && invoices.length > 0 && (
          <div className="flex gap-3 flex-wrap">
            {[
              ...
              { label: 'Sent / Unpaid', count: sentCount, color: 'text-blue-600' },
              { label: 'Paid', count: paidCount, color: 'text-emerald-600' },
            ].map((s) => (
              <div key={s.label} className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-center">
          ...
          {invoices.map((inv) => {
            ...
            return (
              <Link
                key={inv.id}
                to={`/invoices/${inv.id}`}
                className="flex items-center gap-3 bg-white border border-border rounded-xl px-4 py-3 hover:border-primary/40 hover:shadow-sm transition-all group"
              >
                ...
                {seeDollars && (
                  <div className="text-right shrink-0">
                    ...
                    {parseFloat(inv.balance_due) > 0 && inv.status !== 'paid' && (
                      <p className="text-[10px] text-amber-600 font-semibold">Bal {fmtMoney(inv.balance_due)}</p>
                    )}
                  </div>
                )}
                <button
