...
import { FileText, Plus, Loader2, AlertCircle, Clock, CheckCircle2, Trash2, X, ChevronDown, ChevronRight, Mail, Eye } from 'lucide-react';
...
function StatusBadge({
  ...
  if (status === 'completed' || status === 'submitted') {
    return <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 shrink-0">
        <CheckCircle2 size={10} /> {status === 'submitted' ? 'Submitted' : 'Completed'}
      </span>;
  }
  if (status === 'sent') {
    return <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200 shrink-0">
        <Eye size={10} /> Sent
      </span>;
  }
  return <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200 shrink-0">
...
function DeleteConfirm({
  ...
  return <motion.div initial={{
    ...
    opacity: 0
  }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4" onClick={onCancel}>
      <motion.div initial={{
      scale: 0.92,
      opacity: 0
    }} animate={{
      ...
      duration: 0.15
    }} className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2">
          <button onClick={onCancel} disabled={deleting} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors disabled:opacity-50">
            Cancel
          </button>
          <button onClick={onConfirm} disabled={deleting} className="flex-1 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white text-sm font-bold transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
...
function SubmissionRow({
  ...
  return <motion.div layout initial={{
    ...
    duration: 0.15
      ...
      <div onClick={onOpen} className="flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-slate-50 active:bg-slate-100 transition-colors">
        {/* Status icon */}
      ...
      {/* Email modal */}
      {emailOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setEmailOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col gap-4" onClick={e => e.stopPropagation()}>
            ...
            {emailSent ? <div className="flex items-center gap-2 text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-2.5">
                ...
                <div>
                  ...
                  <input type="email" value={emailTo} onChange={e => setEmailTo(e.target.value)} placeholder="recipient@example.com" className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400" onKeyDown={e => {
              if (e.key === 'Enter') void sendEmail();
            }} autoFocus />
                </div>
                {emailError && <div className="flex items-center gap-2 text-xs text-red-600 bg-red-50 border border-red-200 rounded-xl px-3 py-2">
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <button onClick={() => void sendEmail()} disabled={emailSending || !emailTo.trim()} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold disabled:opacity-50 transition-colors">
...
function NewFormPicker({
  ...
  return <div className="relative" ref={ref}>
      <button onClick={() => setOpen(v => !v)} disabled={starting !== null} className="flex items-center gap-1.5 px-4 py-2 bg-primary hover:bg-violet-700 disabled:opacity-60 text-white rounded-lg text-sm font-bold transition-colors">
      ...
      <AnimatePresence>
        ...
        duration: 0.12
          ...
          setOpen(false);
          onStart(t.id);
        }} disabled={starting === t.id} className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-violet-50 transition-colors disabled:opacity-50">
...
export default function JobForms({
  ...
  return <>
      <div className="flex flex-col gap-4">
        {error && <div className="flex items-center gap-2 text-xs text-red-700 bg-red-50 border border-red-200 rounded-xl px-3 py-2">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {submissions.length > 0 ? `${submissions.length} form${submissions.length !== 1 ? 's' : ''} on this job` : 'No forms started yet'}
            </p>
          </div>
          <NewFormPicker templates={templates} starting={starting} onStart={startForm} />
        </div>

        {/* No templates state */}
        {templates.length === 0 && <div className="flex flex-col items-center justify-center py-12 text-center bg-white rounded-xl border border-slate-200">
        ...
        {/* Submissions list */}
        {submissions.length > 0 && <div className="flex flex-col gap-2">
            <AnimatePresence mode="popLayout">
              {submissions.map(s => {
            const template = templates.find(t => t.id === s.templateId);
            const templateName = template?.name ?? `Form #${s.templateId}`;
            return <SubmissionRow key={s.id} submission={s} templateName={templateName} onOpen={() => openSubmission(s)} onDelete={() => setDeleteTarget(s)} canDelete={canDelete} canReset={['owner', 'admin'].includes(userRole ?? '')} onStatusChange={load} />;
          })}
            </AnimatePresence>
          </div>}

        {/* Empty state when templates exist but no submissions */}
        {templates.length > 0 && submissions.length === 0 && <div className="flex flex-col items-center justify-center py-12 text-center bg-white rounded-xl border border-dashed border-slate-200">
      ...
      {/* Delete confirmation */}
      <AnimatePresence>
        {deleteTarget && <DeleteConfirm templateName={templates.find(t => t.id === deleteTarget.templateId)?.name ?? 'this form'} onConfirm={confirmDelete} onCancel={() => setDeleteTarget(null)} deleting={deleting} />}
      </AnimatePresence>
