 ...
 * JobDetailsDashboard
 *
 * Option C hybrid: the Details tab becomes a mini job dashboard.
 * Top row — stat cards (status, tasks, costs, attendance, schedule).
 * Below  — job info (read) or edit form, + linked customer card.
 * All other tabs remain unchanged full-width content areas.
 */
import { Link } from 'react-router-dom';
import {
  HardHat, FileText, User, MapPin, Calendar, CalendarClock,
  CalendarCheck, UserCheck, Users, Phone, Mail, Building2,
  ExternalLink, CheckSquare, DollarSign, Clock, Camera,
  AlertCircle, Edit2, X, Save, Loader2, UserCircle,
} from 'lucide-react';
...
interface Props {
  ...
  costSummary: { actual: number; approved: number } | null;
  ...
  CustomerSelector: React.ComponentType<{ value: Customer | null; onChange: (c: Customer | null) => void }>;
  AssetSelector: React.ComponentType<{ value: number | null; onChange: (id: number | null, name?: string) => void }>;
...
export default function JobDetailsDashboard({
  ...
  // ── Task progress ──────────────────────────────────────────────────────────
  const taskPct = summary && summary.tasksTotal > 0
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <StatCard
          icon={CheckSquare}
          label="Tasks"
          value={summary ? `${summary.tasksDone}/${summary.tasksTotal}` : '—'}
          sub={taskPct !== null ? `${taskPct}% complete` : 'No tasks yet'}
        ...
        <StatCard
          icon={DollarSign}
          label="Costs"
          value={costSummary ? `$${fmt(costSummary.actual)}` : '—'}
          sub={costSummary?.approved ? `$${fmt(costSummary.approved)} approved` : undefined}
          color="bg-emerald-50 text-emerald-600"
          onClick={() => onTabSwitch('costs')}
        />
        ...
        <StatCard
          icon={UserCircle}
          label="On Site"
          value={summary ? summary.onSiteCount : '—'}
          sub={summary?.onSiteCount === 1 ? 'person signed in' : 'people signed in'}
          color="bg-green-50 text-green-600"
          onClick={() => onTabSwitch('attendance')}
        />
          ...
          <div className="bg-white rounded-xl border border-border p-5 flex flex-col gap-4">
            ...
            {!editing && (
              <div className="flex flex-col gap-3">
                ...
                />
                {(job.assignedSupervisorUserId || job.assignedTeamLabel) && (
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => onFormChange({ jobNumber: e.target.value })}
                      className="w-full px-3 py-2.5 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors"
                    />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <CustomerSelector
                    ...
                    onChange={(c) => {
                      ...
                      if (c && !form.client) onFormChange({ client: c.name });
                    }}
                  />
                </div>
                <div>
                  ...
                  <AssetSelector
                    ...
                    onChange={(id, name) => onEditingAssetIdChange(id, name)}
                  />
                </div>
                <div>
                ...
                <div className="pt-2 border-t border-border">
                  ...
                  <div className="mb-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <input type="date" value={form.scheduledStartDate}
                        onChange={(e) => onFormChange({ scheduledStartDate: e.target.value })}
                        className="w-full px-3 py-2.5 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors"
                      />
                      <input type="time" value={form.scheduledStartTime} placeholder="Start time"
                  ...
                  <div className="mb-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <input type="date" value={form.expectedCompletionDate}
                        onChange={(e) => onFormChange({ expectedCompletionDate: e.target.value })}
                        className="w-full px-3 py-2.5 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors"
                      />
                      <input type="time" value={form.scheduledEndTime} placeholder="End time"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input type="date" value={form.actualCompletionDate}
                        onChange={(e) => onFormChange({ actualCompletionDate: e.target.value })}
                        className="w-full px-3 py-2.5 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors"
                      />
                    </div>
                  </div>
                  <div className="mt-3">
                    ...
                    <select
                      ...
                      onChange={(e) => onFormChange({ assignedSupervisorUserId: e.target.value })}
                      className="w-full px-3 py-2.5 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors bg-white"
                    >
                  ...
                  <div className="mt-3">
                    ...
                    <input
                      ...
                      onChange={(e) => onFormChange({ assignedTeamLabel: e.target.value })}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-col gap-0.5 mt-1.5">
                    {linkedCustomer.phone && (
                      <a href={`tel:${linkedCustomer.phone}`} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary transition-colors">
                        <Phone size={10} />{linkedCustomer.phone}
                      </a>
                    )}
                    {linkedCustomer.mobile && (
                      <a href={`tel:${linkedCustomer.mobile}`} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary transition-colors">
                        <Phone size={10} />{linkedCustomer.mobile}
                      </a>
                    )}
                    {linkedCustomer.email && (
                      <a href={`mailto:${linkedCustomer.email}`} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary transition-colors">
          ...
          <div className="bg-white rounded-xl border border-border p-4">
            ...
            <div className="flex flex-col gap-1">
              {([
                ...
                { label: 'Notes',      tab: 'notes',      icon: FileText },
                { label: 'Costs',      tab: 'costs',      icon: DollarSign },
                { label: 'Progress',   tab: 'progress',   icon: Clock },
                { label: 'Safety',     tab: 'safety',     icon: AlertCircle },
                { label: 'Attendance', tab: 'attendance', icon: UserCircle },
              ] as const).map(({ label, tab, icon: Icon }) => (
                <button
                  key={tab}
                  onClick={() => onTabSwitch(tab)}
                  className="flex items-center gap-2 px-2.5 py-2 rounded-lg text-xs font-medium text-muted-foreground hover:bg-muted hover:text-foreground transition-colors text-left w-full"
                >
