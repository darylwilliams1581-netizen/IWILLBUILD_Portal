...
import { ShieldAlert, Plus, Loader2, Users, Printer, Wand2, Trash2, ClipboardList, Link2, UserCheck, ChevronDown, Library, FileText, AlertTriangle } from 'lucide-react';
...
import { type SwmsTemplate, type SafetyPlanTemplate, type Signoff, type JobSwmsRecord, type JobSafetyPlan, type JobInfo, fmtDate, statusBadge, SignonModal, AddSwmsModal, SwmsEditModal, SwmsPrintModal, AddSafetyPlanModal } from './JobSafetyModals';

// ── Add SWMS dropdown button ──────────────────────────────────────────────────

function AddSwmsDropdown({
  onFromLibrary,
  onCreateNew
}: {
  onFromLibrary: () => void;
  ...
  return <div className="relative" ref={ref}>
      <button onClick={() => setOpen(v => !v)} className="flex items-center gap-1.5 bg-primary hover:bg-violet-700 text-white text-sm font-bold px-3 py-2 rounded-lg transition-colors">
        <Plus size={14} />
...
function SwmsSubTab({
  ...
  const load = useCallback(() => {
    setLoading(true);
    fetch(`/api/safety/job-swms?jobId=${jobId}`, {
      credentials: 'include'
    }).then(r => r.json()).then(d => setList((d.jobSwms ?? []).map((js: JobSwmsRecord) => ({
      ...js,
      signoffs: js.signoffs ?? []
  ...
  async function handleDelete(id: number, title: string) {
    ...
    setDeleting(id);
    try {
      const r = await fetch(`/api/safety/job-swms/${id}`, {
        method: 'DELETE',
        credentials: 'include'
      });
  ...
  async function handleStatusChange(item: JobSwmsRecord, newStatus: string) {
    const r = await fetch(`/api/safety/job-swms/${item.id}`, {
      ...
      body: JSON.stringify({
        title: item.title,
        workActivity: item.work_activity,
        hazards: item.hazards,
        risks: item.risks,
        controls: item.controls,
        ppe: item.ppe,
        plantEquipment: item.plant_equipment,
        trainingCompetency: item.training_competency,
        emergencyControls: item.emergency_controls,
        environmentalControls: item.environmental_controls,
        signOffRequirements: item.sign_off_requirements,
        permitsApprovals: item.permits_approvals,
        monitoringReview: item.monitoring_review,
        notes: item.notes,
    ...
    if (r.ok) {
      ...
      setList(prev => prev.map(j => j.id === item.id ? {
        ...d.jobSwms,
  ...
  async function openPrint(item: JobSwmsRecord) {
    ...
    try {
      const r = await fetch(`/api/safety/job-swms/${item.id}/signoffs`, {
        credentials: 'include'
      });
      const d = await r.json();
      setPrinting({
  ...
  return <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <AddSwmsDropdown onFromLibrary={() => setShowAdd(true)} onCreateNew={() => setShowBuilder(true)} />
      </div>

      {list.length === 0 && <div className="flex flex-col items-center justify-center py-14 text-center bg-white rounded-xl border border-slate-200">
      ...
      {list.length > 0 && <div className="flex flex-col gap-2">
          {list.map(j => {
        const signedCount = (j.signoffs ?? []).length;
        return <div key={j.id} className="bg-white border border-slate-200 rounded-xl p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                      ...
                      {signedCount === 0 && <span className="text-xs font-bold text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-full">Unsigned</span>}
                    </div>
                  </div>

                  <div className="flex items-center gap-1 shrink-0 flex-wrap justify-end">
                    {/* Status advance */}
                    {j.status === 'draft' && <button onClick={() => handleStatusChange(j, 'reviewed')} className="px-2 py-1 rounded-md text-xs font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 transition-colors">Review</button>}
                    {j.status === 'reviewed' && <button onClick={() => handleStatusChange(j, 'approved')} className="px-2 py-1 rounded-md text-xs font-semibold text-emerald-600 bg-emerald-50 hover:bg-emerald-100 transition-colors">Approve</button>}
                    {/* Sign on */}
                    <button onClick={() => setSignonTarget(j)} className="flex items-center gap-1.5 text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white px-2.5 py-1.5 rounded-lg transition-colors" title="Sign onto SWMS">
                      <UserCheck size={12} />Sign On
                    </button>
                    {/* Print */}
                    <button onClick={() => openPrint(j)} disabled={loadingSignoffs === j.id} className="p-1.5 rounded-lg text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition-colors" title="Print / PDF">
                      {loadingSignoffs === j.id ? <Loader2 size={14} className="animate-spin" /> : <Printer size={14} />}
                    </button>
                    {/* Share / QR */}
                    <button onClick={() => setSwmsShareTarget({
                type: 'job_swms',
                id: String(j.id),
                title: j.title,
                linkType: 'swms_signon',
                defaultPermissions: ['view', 'sign']
              })} className="p-1.5 rounded-lg text-slate-400 hover:text-primary hover:bg-violet-50 transition-colors" title="Share / QR">
                      <Link2 size={14} />
                    </button>
                    {/* Edit */}
                ...
                {/* Sign-on list (inline) */}
                {signedCount > 0 && <div className="mt-3 pt-3 border-t border-slate-100">
                    <p className="text-xs font-semibold text-slate-500 mb-2">Signed On ({signedCount})</p>
                    <div className="flex flex-col gap-1.5">
                      {(j.signoffs ?? []).map(s => <div key={s.id} className="flex items-center gap-3 bg-emerald-50 border border-emerald-100 rounded-lg px-3 py-2">
                          ...
                          <p className="text-xs text-slate-400 shrink-0">{new Date(s.signed_at).toLocaleDateString('en-AU', {
                    day: 'numeric',
                    month: 'short'
                  })}</p>
                        </div>)}
      ...
      <AnimatePresence>
        {showAdd && <AddSwmsModal jobId={jobId} onClose={() => setShowAdd(false)} onAdded={items => {
        setList(prev => [...items.map(i => ({
          ...i,
          signoffs: []
        })), ...prev]);
        setShowAdd(false);
      }} />}
        {showBuilder && <SwmsBodyBuilder onClose={() => setShowBuilder(false)} onSaved={async saved => {
        ...
        try {
          ...
          const d = (await r.json()) as {
            jobSwms?: JobSwmsRecord[];
          };
          if (r.ok && d.jobSwms?.length) {
            setList(prev => [...d.jobSwms!.map(i => ({
              ...i,
              signoffs: []
            })), ...prev]);
          } else {
        ...
        {editing && <SwmsEditModal initial={editing} onClose={() => setEditing(null)} onSaved={updated => {
        setList(prev => prev.map(j => j.id === updated.id ? {
          ...updated,
          signoffs: j.signoffs ?? []
        } : j));
        setEditing(null);
        ...
        {signonTarget && <SignonModal jobSwmsId={signonTarget.id} swmsTitle={signonTarget.title} onClose={() => setSignonTarget(null)} onSigned={signoff => {
        setList(prev => prev.map(j => j.id === signonTarget.id ? {
          ...j,
          signoffs: [signoff, ...(j.signoffs ?? [])]
        } : j));
        setSignonTarget(null);
      }} />}
      </AnimatePresence>

      {swmsShareTarget && <ShareLinkModal open={true} onClose={() => setSwmsShareTarget(null)} target={swmsShareTarget} />}
    </div>;
}

// ── Safety Plans Sub-tab ──────────────────────────────────────────────────────

function SafetyPlansSubTab({
  jobId
}: {
  jobId: number;
}) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    fetch(`/api/safety/job-safety-plans?jobId=${jobId}`, {
      credentials: 'include'
  ...
  async function handleDelete(id: number, title: string) {
    ...
    setDeleting(id);
    try {
      const r = await fetch(`/api/safety/job-safety-plans/${id}`, {
        method: 'DELETE',
        credentials: 'include'
      });
  ...
  return <div className="flex flex-col gap-4">
      ...
      {plans.length === 0 && <div className="flex flex-col items-center justify-center py-14 text-center bg-white rounded-xl border border-slate-200">
      ...
      {plans.length > 0 && <div className="flex flex-col gap-2">
          {plans.map(p => <div key={p.id} className="bg-white border border-slate-200 rounded-xl p-4 flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => handleDelete(p.id, p.title)} disabled={deleting === p.id} className="p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Remove from job">
      ...
      <AnimatePresence>
        {showAdd && <AddSafetyPlanModal jobId={jobId} onClose={() => setShowAdd(false)} onAdded={plan => {
...
function SignonsSubTab({
  ...
  useEffect(() => {
    ...
    fetch(`/api/safety/job-swms?jobId=${jobId}`, {
      ...
      const withSignoffs = await Promise.all(items.map(async js => {
        try {
          const r = await fetch(`/api/safety/job-swms/${js.id}/signoffs`, {
            credentials: 'include'
          });
          const sd = await r.json();
          return {
            ...js,
            signoffs: sd.signoffs ?? []
          };
        } catch {
          return {
            ...js,
  ...
  if (loading) return <div className="flex items-center justify-center py-16"><Loader2 size={22} className="animate-spin text-primary" /></div>;
  const allSignoffs = list.flatMap(js => (js.signoffs ?? []).map(s => ({
    ...s,
    swmsTitle: js.title
  })));
  return <div className="flex flex-col gap-4">
      ...
      {allSignoffs.length === 0 && <div className="flex flex-col items-center justify-center py-14 text-center bg-white rounded-xl border border-slate-200">
      ...
      return <div key={js.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            ...
            <div className="divide-y divide-slate-100">
              {sigs.map(s => <div key={s.id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  {s.signature_data && <img src={safeUrl(s.signature_data)} alt="sig" className="h-8 max-w-[80px] object-contain border border-slate-200 rounded bg-white" />}
...
export default function JobSafety({
  ...
  return <div className="flex flex-col gap-4">
      ...
      <button onClick={() => navigate(`/jobs/${jobId}/risky`)} className="flex items-center gap-3 w-full bg-amber-50 border border-amber-200 hover:bg-amber-100 hover:border-amber-300 rounded-xl px-4 py-3 transition-colors group text-left">
      ...
      <div className="flex gap-1 border-b border-slate-200 pb-0">
        ...
        label,
        icon: Icon
      }) => <button key={id} onClick={() => setSubTab(id)} className={`flex items-center gap-1.5 px-3 py-2 rounded-t-lg text-xs font-semibold transition-colors whitespace-nowrap border-b-2 -mb-px ${subTab === id ? 'border-primary text-primary bg-violet-50/50' : 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-50'}`}>
            <Icon size={13} />{label}
          </button>)}
      </div>

      <AnimatePresence mode="wait">
        <motion.div key={subTab} initial={{
        opacity: 0,
        y: 6
      }} animate={{
        opacity: 1,
