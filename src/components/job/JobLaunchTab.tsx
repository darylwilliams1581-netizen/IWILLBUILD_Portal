 ...
 * JobLaunchTab
 *
 * Shown on the Job Detail "Launch" tab.
 * Renders a grid of module cards — each with:
 *   • "Open here"       — navigates in the current tab
 *   • "Open in new tab" — opens a standalone page with ?jobId= baked in
 *
 * The ?jobId= param is read by JobContextTab on the destination page so the
 * user always has job context even after closing the job detail page.
 */
import { Link, useNavigate } from 'react-router-dom';
import {
  FileText, ClipboardList, Calculator, Layers,
  FolderOpen, DollarSign, ShieldAlert, Truck,
...
export default function JobLaunchTab({ job }: Props) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        {modules.map((mod) => (
          <ModuleCard
            key={mod.label}
            mod={mod}
            onOpenHere={() => navigate(mod.sameTabUrl ?? mod.standaloneUrl)}
          />
        ))}
      </div>
...
function ModuleCard({ mod, onOpenHere }: { mod: ModuleCard; onOpenHere: () => void }) {
  const Icon = mod.icon;
  return (
