...
import { Loader2, Plus, X, Truck, AlertCircle, ChevronLeft, ChevronRight, Pencil, Trash2, CalendarDays, Briefcase, CheckCircle2 } from 'lucide-react';
...
function bookingPosition(booking: AssetBooking, days: Date[], dayWidth: number): {
  left: number;
  width: number;
} | null {
...
function BookingFormModal({
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/fleet/asset-bookings/${initial.id}`, {
        method: 'DELETE',
        credentials: 'include'
      });
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={e => {
    if (e.target === e.currentTarget) onClose();
      ...
      duration: 0.18
        ...
        <div className="px-5 py-4 flex flex-col gap-3">
          {error && <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-xs text-red-700">
          ...
          <div>
            ...
            <select value={fleetAssetId} onChange={e => setFleetAssetId(Number(e.target.value))} disabled={isEdit} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-400 bg-white disabled:bg-slate-50">
              {assets.map(a => <option key={a.id} value={a.id}>
          ...
          <div>
            ...
            <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Site A — Excavation" className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-400" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="date" value={endDate} min={startDate} onChange={e => setEndDate(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-400" />
            </div>
          </div>
          ...
          <div>
            ...
            <select value={status} onChange={e => setStatus(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-400 bg-white">
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} placeholder="Any additional details..." className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-400 resize-none" />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-t border-slate-100 bg-slate-50">
          <div>
            {isEdit && <button onClick={handleDelete} disabled={deleting || saving} className="flex items-center gap-1.5 text-xs font-semibold text-red-600 hover:text-red-700 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleSave} disabled={saving || deleting} className="flex items-center gap-1.5 px-4 py-1.5 bg-violet-500 hover:bg-violet-700 text-white text-xs font-bold rounded-lg transition-colors disabled:opacity-50">
...
function BookingPopover({
  ...
  return <div ref={popRef} className="absolute z-30 top-full mt-1 left-0 w-64 bg-white border border-slate-200 rounded-xl shadow-xl p-3 text-xs">
      ...
      <div className="flex flex-col gap-1 text-slate-600 mb-3">
        <div className="flex items-center gap-1.5">
          ...
          <span>
            {new Date(booking.start_date + 'T00:00:00').toLocaleDateString('en-AU', {
            day: 'numeric',
            month: 'short'
          })}
            {booking.start_date !== booking.end_date && <> – {new Date(booking.end_date + 'T00:00:00').toLocaleDateString('en-AU', {
              day: 'numeric',
              month: 'short'
            })}</>}
        ...
        {booking.job_name && <div className="flex items-center gap-1.5">
            <Briefcase size={11} className="text-slate-400 shrink-0" />
            <Link to={`/job-detail?id=${booking.job_id}`} className="text-violet-700 hover:underline truncate" onClick={onClose}>
      ...
      <button onClick={onEdit} className="w-full flex items-center justify-center gap-1.5 py-1.5 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
...
export default function AssetSchedulerView({
  ...
  const load = useCallback(() => {
    ...
    fetch(`/api/fleet/asset-bookings?startDate=${start}&endDate=${end}`, {
      credentials: 'include'
      ...
      setAssets(d.assets ?? []);
    }).catch(() => setError('Failed to load asset bookings.')).finally(() => setLoading(false));
  }, [days]);
  useEffect(() => {
    load();
  }, [load]);
  ...
  return <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-100 bg-white shrink-0 flex-wrap">
        ...
        <div className="flex items-center bg-slate-100 rounded-lg p-0.5 gap-0.5">
          {(Object.keys(WINDOW_LABELS) as TimeWindow[]).map(key => <button key={key} onClick={() => onWindowChange(key)} className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${timeWindow === key ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
              {WINDOW_LABELS[key]}
            </button>)}
        </div>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="hidden lg:flex items-center gap-3">
            {Object.entries(STATUS_LABELS).slice(0, 4).map(([k, v]) => <div key={k} className="flex items-center gap-1">
      ...
      {!loading && error && <div className="flex items-center gap-2 text-sm text-red-600 bg-red-50 border border-red-200 rounded-xl m-4 p-4">
          <AlertCircle size={16} />{error}
          <button onClick={load} className="ml-auto text-xs font-semibold underline">Retry</button>
        </div>}

      {/* Empty state */}
      {!loading && !error && assets.length === 0 && <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      {/* Timeline grid */}
      {!loading && !error && assets.length > 0 && <div className="flex-1 overflow-auto">
          <div className="flex min-w-max">
            ...
            <div className="w-44 shrink-0 sticky left-0 z-20 bg-white border-r border-slate-200">
              ...
              {assets.map(a => <div key={a.id} className="h-11 border-b border-slate-100 last:border-0 flex flex-col justify-center px-3 hover:bg-slate-50/50 transition-colors">
            ...
            width: totalWidth
                ...
                return <div key={i} style={{
                  width: dayWidth
                        ...
                        {show && <span className={`text-[10px] font-semibold ${isToday(d) ? 'text-violet-700' : 'text-slate-500'}`}>
                            {dayWidth >= 36 ? d.toLocaleDateString('en-AU', {
                      day: 'numeric',
                      month: 'short'
                    }) : d.getDate()}
                          </span>}
                ...
                {/* Asset rows */}
                {assets.map(asset => <div key={asset.id} className="relative flex border-b border-slate-100 last:border-0 hover:bg-slate-50/30 transition-colors" style={{
              height: 44
            }}>
                  ...
                  width: dayWidth
                    ...
                    <div className="absolute inset-0 flex">
                      {days.map((d, i) => <div key={i} style={{
                  width: dayWidth
                }} className="h-full shrink-0 cursor-pointer hover:bg-violet-50/30 transition-colors" onClick={() => handleCellClick(asset.id, toDateStr(d))} title={`Book ${asset.name} on ${fmtFull(d)}`} />)}
                    </div>
                ...
                return <div key={b.id} className={`absolute top-2 h-7 rounded-md ${color} text-white text-[10px] font-semibold flex items-center px-2 cursor-pointer hover:brightness-110 transition-all shadow-sm overflow-hidden whitespace-nowrap z-10 ${isActive ? 'ring-2 ring-white ring-offset-1' : ''}`} style={{
                  left: pos.left + 2,
                  width: Math.max(pos.width - 4, 20)
                }} onClick={e => {
                  e.stopPropagation();
                  setPopoverBooking(b);
                }} title={b.title}>
                ...
                return <div className="absolute top-0 bottom-0 w-0.5 bg-violet-500 z-20 pointer-events-none" style={{
                  left: todayIdx * dayWidth + dayWidth / 2
                }} />;
              })()}

                    {/* Popover */}
                    {popoverBooking && popoverBooking.fleet_asset_id === asset.id && <div className="absolute top-0 z-30" style={{
                left: (() => {
                  const pos = bookingPosition(popoverBooking, days, dayWidth);
                  return pos ? pos.left + 2 : 0;
                })()
              }}>
                        <BookingPopover booking={popoverBooking} onEdit={handleEditFromPopover} onClose={() => setPopoverBooking(null)} anchorRef={popoverAnchorRef} />
                      </div>}
                  </div>)}
              </div>
            </div>
      ...
      {/* Booking form modal */}
      <AnimatePresence>
        {formOpen && <BookingFormModal assets={assets} initial={editBooking ?? (newDefaults ? {
        fleet_asset_id: newDefaults.fleet_asset_id,
        start_date: newDefaults.start_date,
        end_date: newDefaults.start_date
      } : undefined)} onClose={() => {
        setFormOpen(false);
        setEditBooking(null);
        setNewDefaults(null);
