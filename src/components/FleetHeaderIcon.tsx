...
export default function FleetHeaderIcon() {
  ...
  return <button onClick={() => navigate('/fleet')} title="Fleet" aria-label="Go to Fleet" className={`
        flex items-center justify-center w-9 h-9 rounded-lg transition-colors
        ${isFleet ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-muted'}
      `}>
      <Truck size={18} />
    </button>;
}
