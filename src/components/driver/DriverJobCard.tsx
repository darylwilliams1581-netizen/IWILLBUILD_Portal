...
import { Briefcase, MapPin, User, Calendar, ChevronRight, RefreshCw, AlertCircle, CheckCircle2, Clock, ChevronDown, ChevronUp, ExternalLink } from 'lucide-react';
...
function StatusBadge({
  ...
  return <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${s.bg} ${s.color}`}>
...
export default function DriverJobCard({
  ...
  // ── Loading skeleton ──────────────────────────────────────────────────────
  if (loading) {
    return <div className={`bg-gray-900 rounded-2xl border border-gray-800 ${compact ? 'p-4' : 'p-5'} animate-pulse`}>
  ...
  // ── Error state ───────────────────────────────────────────────────────────
  if (error) {
    return <div className={`bg-gray-900 rounded-2xl border border-red-900/50 ${compact ? 'p-4' : 'p-5'}`}>
  ...
  // ── No active jobs ────────────────────────────────────────────────────────
  if (jobs.length === 0) {
    return <div className={`bg-gray-900 rounded-2xl border border-gray-800 ${compact ? 'p-4' : 'p-5'}`}>
  ...
  return <div className={`bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden ${compact ? '' : ''}`}>
      ...
      <div className={`flex items-center justify-between ${compact ? 'px-4 pt-4 pb-2' : 'px-5 pt-5 pb-3'}`}>
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => void load(true)} className="text-gray-500 hover:text-gray-300 transition-colors" aria-label="Refresh jobs">
            <RefreshCw size={13} className={refreshing ? 'animate-spin' : ''} />
          </button>
          {jobs.length > 1 && <button onClick={() => setShowPicker(p => !p)} className="flex items-center gap-1 text-xs text-violet-400 hover:text-violet-300 font-medium">
              Change
      ...
      <AnimatePresence>
        ...
        duration: 0.2
            ...
            <div className={`${compact ? 'px-4' : 'px-5'} pb-3 space-y-1`}>
              {selectedJob && <button onClick={clearJob} className="w-full text-left px-3 py-2 rounded-xl text-sm text-gray-400 hover:bg-gray-800 transition-colors">
                  Clear selection
                </button>}
              {jobs.map(job => <button key={job.id} onClick={() => selectJob(job)} className={`w-full text-left px-3 py-2.5 rounded-xl transition-colors ${selectedJob?.id === job.id ? 'bg-violet-500/10 border border-violet-600/30' : 'hover:bg-gray-800'}`}>
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <p className="text-white text-sm font-semibold truncate">{job.name}</p>
                      {job.jobNumber && <p className="text-gray-500 text-xs">{job.jobNumber}</p>}
                    </div>
                    <div className="shrink-0 flex items-center gap-2">
                      <StatusBadge status={job.status} />
                      {selectedJob?.id === job.id && <CheckCircle2 size={14} className="text-violet-400" />}
                    </div>
                  </div>
                </button>)}
            </div>
      ...
      {/* Selected job detail */}
      {selectedJob ? <div className={`${compact ? 'px-4 pb-4' : 'px-5 pb-5'}`}>
          {/* Job name + number */}
          <div className="mb-3">
            <h3 className="text-white font-bold text-base leading-tight">{selectedJob.name}</h3>
            {selectedJob.jobNumber && <p className="text-gray-500 text-xs mt-0.5">{selectedJob.jobNumber}</p>}
          </div>
          ...
          <div className="space-y-2">
            {selectedJob.client && <div className="flex items-start gap-2.5">
                <User size={13} className="text-gray-500 mt-0.5 shrink-0" />
                <span className="text-gray-300 text-sm">{selectedJob.client}</span>
              </div>}
            {selectedJob.address && <div className="flex items-start gap-2.5">
            ...
            {selectedJob.scheduledStartDate && <div className="flex items-start gap-2.5">
                <Calendar size={13} className="text-gray-500 mt-0.5 shrink-0" />
                <span className="text-gray-300 text-sm">
                  Started {formatDate(selectedJob.scheduledStartDate)}
                </span>
              </div>}
            {selectedJob.expectedCompletionDate && <div className="flex items-start gap-2.5">
          ...
          {/* Link to full job */}
          <Link to={`/jobs/${selectedJob.id}`} className="inline-flex items-center gap-1.5 mt-4 text-violet-400 hover:text-violet-300 text-sm font-medium transition-colors">
            Open job details
            <ChevronRight size={14} />
          </Link>
        </div> : (/* No job selected yet — show picker prompt */
    <div className={`${compact ? 'px-4 pb-4' : 'px-5 pb-5'}`}>
          <p className="text-gray-400 text-sm mb-3">
            {jobs.length} active {jobs.length === 1 ? 'job' : 'jobs'} available — select one to track.
          </p>
          <div className="space-y-2">
            {jobs.slice(0, 3).map(job => <button key={job.id} onClick={() => selectJob(job)} className="w-full flex items-center justify-between gap-3 bg-gray-800 hover:bg-gray-750 rounded-xl px-3 py-3 transition-colors text-left">
                <div className="min-w-0">
                  <p className="text-white text-sm font-semibold truncate">{job.name}</p>
                  {job.address && <p className="text-gray-500 text-xs truncate mt-0.5">{job.address}</p>}
                </div>
                <div className="shrink-0 flex items-center gap-2">
                  <StatusBadge status={job.status} />
