...
export interface LensJobOption {
  id: number;
...
interface LensJobPickerSheetProps {
  open: boolean;
  title: string;
  subtitle?: string;
  onSelect: (job: LensJobOption) => void;
  onClose: () => void;
}

// ── Helpers ───────────────────────────────────────────────────────────────────

function jobLabel(job: LensJobOption): string {
  return job.jobNumber ? `#${job.jobNumber} — ${job.name}` : job.name;
}

// ── Component ─────────────────────────────────────────────────────────────────

export default function LensJobPickerSheet({
  open,
  title,
  subtitle,
  onSelect,
  onClose,
}: LensJobPickerSheetProps) {
  const [query, setQuery]   = useState('');
  const [jobs, setJobs]     = useState<LensJobOption[]>([]);
  ...
  async function fetchJobs(q: string) {
    ...
    try {
      ...
      const data = await res.json() as { jobs?: LensJobOption[] };
      setJobs(data.jobs ?? []);
    } catch {
      setJobs([]);
    } finally {
