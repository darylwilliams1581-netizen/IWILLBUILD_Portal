...
function buildGroups(photos: LensPhoto[]): JobGroup[] {
  ...
  // Sort groups newest-first by latest photo date
...
interface JobSectionProps {
  ...
  onUpload: (jobId: number) => void;
  onCamera: (jobId: number) => void;
  selectionMode: boolean;
        ...
        {expanded && (
          <div className="hidden md:flex items-center gap-1.5 shrink-0" onClick={(e) => e.stopPropagation()}>
            <button
              ...
              title="Upload photos to this job"
            >
              <Upload size={12} /> Upload
            </button>
            <button
              type="button"
              onClick={() => onCamera(group.jobId)}
...
export default function LensGroupByJob({
  ...
  const [expandedIds, setExpandedIds] = useState<Set<number>>(() =>
    // Start with first group expanded if there's only one
