  ...
  return <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
      ...
      <div>
        {fetching ? <div className="flex items-center justify-center py-8">
          ...
          </div> : alerts.length === 0 ? <div className="flex flex-col items-center justify-center py-10 text-slate-400">
        ...
        return <div key={alert.id} onClick={() => handleClick(alert)} className={`flex items-start gap-3 px-4 py-3 border-b border-slate-50 last:border-0 cursor-pointer transition-colors duration-100 ${alert.read ? 'bg-white hover:bg-slate-50' : 'bg-blue-50/40 hover:bg-blue-50/70'}`}>
                ...
                {!alert.read && <div className="shrink-0 w-2 h-2 rounded-full bg-blue-500 mt-1.5" />}
              </div>;
      })}
      </div>
    </div>;
