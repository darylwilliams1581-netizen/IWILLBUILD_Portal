        ...
        <motion.div
          ...
          className={`rounded-full border-2 transition-all duration-150 ${
            i < filled
              ...
              : 'bg-transparent border-white/50 w-3.5 h-3.5'
          }`}
          animate={{ scale: i === filled - 1 && filled > 0 ? [1, 1.3, 1] : 1 }}
...
function AppLockGateNative({ children }: AppLockGateProps) {
      ...
      <AnimatePresence>
        {lock.isLocked && (
          <motion.div
            ...
            className="fixed inset-0 z-[9999] flex flex-col items-center justify-between bg-background"
            style={{
          ...
          >
            ...
            <div className="flex flex-col items-center gap-3">
                ...
                <div key={ri} className="flex items-center gap-5">
                  {row.map((key, ki) => (
                    <PadButton
                      ...
                      disabled={lock.verifying || isLockedOut}
                    />
                  ))}
                </div>
              ))}
              ...
              {/* Face ID row */}
              <div className="flex items-center justify-center gap-5 mt-1" style={{ height: 80 }}>
                {/* Placeholder left */}
                ...
                {lock.hasFaceId ? (
                  <motion.button
                    whileTap={{ scale: 0.88 }}
                    onClick={() => lock.tryFaceId()}
                    disabled={lock.verifying}
                    className="w-20 h-20 rounded-full bg-foreground/10 flex items-center justify-center active:bg-foreground/20 disabled:opacity-30"
            ...
            <button
              ...
              className="text-muted-foreground text-sm underline underline-offset-2 active:text-foreground transition-colors"
            >
