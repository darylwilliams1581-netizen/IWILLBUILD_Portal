...
import {
  Camera, ChevronLeft, ChevronRight, X, Download,
  Loader2, Lock, AlertCircle, Calendar, Building2,
  ImageOff, ZoomIn,
} from 'lucide-react';
...
function ErrorShell({ icon, title, body }: { icon: React.ReactNode; title: string; body: string }) {
  return (
...
export default function PhotoSharePage() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`/api/public/job-photos/${token}`)
      .then(async (r) => {
        const ct = r.headers.get('content-type') ?? '';
        if (!ct.includes('application/json')) throw new Error('Link not found or expired');
        const json = await r.json() as ShareData & { error?: string };
  ...
  if (error || !data) {
    ...
    return (
      <ErrorShell
        ...
        title={isExpired ? 'Link expired' : 'Link not found'}
        body={isExpired
          ? 'This photo gallery link has expired. Ask the site team to generate a new one.'
          ...
          <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                {job.jobNumber && (
                  <span className="text-xs text-slate-400 font-mono">{job.jobNumber}</span>
                )}
        ...
        {lightbox !== null && photos[lightbox] && (
          ...
          >
            ...
            {(photos[lightbox].label || photos[lightbox].originalName) && (
              <div className="shrink-0 px-4 py-2 text-center" onClick={(e) => e.stopPropagation()}>
                <p className="text-white/70 text-sm truncate">
                  {photos[lightbox].label ?? photos[lightbox].originalName}
                </p>
                {photos[lightbox].createdAt && (
                  <p className="text-white/40 text-xs mt-0.5">{formatDate(photos[lightbox].createdAt)}</p>
                )}
              </div>
            )}
