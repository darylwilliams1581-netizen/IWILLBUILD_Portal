...
import { Link, useSearchParams, useNavigate as useRRNavigate } from "react-router";
...
async function rescheduleJob(jobId: number, scheduledStartDate: string, expectedCompletionDate: string, scheduledStartTime?: string, scheduledEndTime?: string): Promise<boolean> {
  try {
...
function TableView({
  ...
  return <>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              {['Job', 'Client', 'Location', 'Status', 'Sched. Start', 'Exp. Completion', 'Duration', 'Supervisor / Team', 'Progress', ''].map(h => <th key={h} className="text-left px-3 py-2 text-[11px] font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">{h}</th>)}
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {jobs.map(job => {
            const style = getStatusStyle(job.status);
            const duration = job.scheduledStartDate && job.expectedCompletionDate ? `${daysBetween(job.scheduledStartDate, job.expectedCompletionDate)}d` : '—';
            const supervisor = job.supervisorName ?? job.teamLabel ?? '—';
            return <tr key={job.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-3 py-1.5 max-w-[140px]">
                    {job.address ? <a href={`https://maps.google.com/?q=${encodeURIComponent(job.address)}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-violet-700 hover:text-violet-800 text-xs truncate" title={job.address}>
      ...
      <div className="md:hidden divide-y divide-slate-100">
        {jobs.map(job => {
        const style = getStatusStyle(job.status);
        return <div key={job.id} className="p-4 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-1 text-xs text-slate-500">
                ...
                {job.address ? <a href={`https://maps.google.com/?q=${encodeURIComponent(job.address)}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-violet-700"><MapPin size={10} className="shrink-0" /><span className="truncate">{job.address}</span></a> : null}
...
function EventPopup({
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center p-4" onClick={onClose}>
      {/* Backdrop */}
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-xs overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between mb-2">
            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold border ${style.bg} ${style.color}`}>
              <span className={`w-1.5 h-1.5 rounded-full ${style.dot}`} />
              {job.status}
            </span>
            <button onClick={onClose} className="w-6 h-6 flex items-center justify-center rounded-full bg-slate-100 text-slate-400 hover:bg-slate-200 transition-colors text-xs font-bold" aria-label="Close">
              ✕
            </button>
          </div>
          ...
          {job.jobNumber && <p className="text-[11px] text-slate-400 font-mono mb-2">#{job.jobNumber}</p>}
          ...
          <div className="space-y-1.5 text-xs text-slate-600">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>
                ...
                {job.scheduledStartTime && <span className="text-violet-600 font-medium ml-1">{fmtTime(job.scheduledStartTime)}</span>}
                {' → '}
                {fmt(job.expectedCompletionDate)}
                {job.scheduledEndTime && <span className="text-slate-400 ml-1">{fmtTime(job.scheduledEndTime)}</span>}
              </span>
            </div>
            {/* Duration */}
            {job.scheduledStartDate && job.expectedCompletionDate && <div className="flex items-center gap-2">
            ...
            {/* Client */}
            {job.client && <div className="flex items-center gap-2">
                <User size={11} className="shrink-0 text-slate-400" />
                <span className="truncate">{job.client}</span>
              </div>}
            {/* Address */}
            {job.address && <div className="flex items-start gap-2">
                <MapPin size={11} className="shrink-0 text-slate-400 mt-0.5" />
                <a href={`https://maps.google.com/?q=${encodeURIComponent(job.address)}`} target="_blank" rel="noopener noreferrer" className="text-violet-600 hover:underline leading-snug" onClick={e => e.stopPropagation()}>
                  {job.address}
                </a>
              </div>}
            {/* Supervisor */}
            {(job.supervisorName ?? job.teamLabel) && <div className="flex items-center gap-2">
          ...
          {/* Open job button */}
          <button onClick={onOpen} className="mt-3 w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold text-white transition-colors" style={{
          background: color
        }}>
...
function CalendarView({
  ...
  function handleDrop(e: React.DragEvent, date: Date) {
    ...
    const duration = draggingJob.scheduledStartDate && draggingJob.expectedCompletionDate ? daysBetween(draggingJob.scheduledStartDate, draggingJob.expectedCompletionDate) - 1 : 0;
  ...
  const monthLabel = anchorDate.toLocaleDateString('en-AU', {
    month: 'long',
    year: 'numeric'
  });
  return <div className="p-2 sm:p-3">
      ...
      <div className="grid grid-cols-7 mb-0.5">
        {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => <div key={i} className="text-center text-[9px] font-bold text-slate-400 uppercase py-0.5">{d}</div>)}
      </div>
      ...
      <div className="grid grid-cols-7 gap-px bg-slate-200 border border-slate-200 rounded-xl overflow-hidden">
        ...
        return <div key={i} className={`min-h-[52px] p-1 transition-colors ${isToday ? 'bg-violet-50' : isWeekend ? 'bg-slate-50/80' : isDragTarget ? 'bg-blue-50' : 'bg-white'}`} onDragOver={e => {
          e.preventDefault();
          setDragOver(dateStr);
        }} onDragLeave={() => setDragOver(null)} onDrop={e => handleDrop(e, date)}>
              {/* Day number */}
              <div className={`text-[10px] font-bold mb-0.5 w-5 h-5 flex items-center justify-center rounded-full leading-none ${isToday ? 'bg-violet-500 text-white' : 'text-slate-400'}`}>
                {date.getDate()}
              </div>
              ...
              return <button key={job.id} draggable={isStart} onDragStart={isStart ? e => handleDragStart(e, job) : undefined} onClick={e => {
                e.stopPropagation();
                setPopupJob(job);
              }} className={`w-full text-left text-[9px] font-semibold text-white px-1 py-px rounded-sm truncate leading-tight transition-opacity hover:opacity-90 active:opacity-75 ${isStart ? 'cursor-pointer' : 'cursor-pointer opacity-80'}`} style={{
                background: barHex(job.status)
              }}>
                      {isStart && job.name}
                      {!isStart && <span className="opacity-60">▬</span>}
                    </button>;
            })}
                {cellJobs.length > 2 && <button onClick={() => setPopupJob(cellJobs[2])} className="w-full text-left text-[9px] text-slate-500 font-semibold pl-0.5 hover:text-violet-600 transition-colors">
                    +{cellJobs.length - 2}
                  </button>}
              </div>
            </div>;
      ...
      <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1">
        {Object.entries(STATUS_BAR_HEX).map(([status, color]) => <div key={status} className="flex items-center gap-1">
      ...
      {/* Event popup */}
      {popupJob && <EventPopup job={popupJob} onClose={() => setPopupJob(null)} onOpen={() => {
      navigate(`/jobs/${popupJob.id}`);
      setPopupJob(null);
    }} />}
    </div>;
}
...
function DayView({
  ...
  return <div className="flex flex-col h-full">
      {/* All-day strip */}
      {allDayJobs.length > 0 && <div className="border-b border-slate-200 bg-slate-50 px-4 py-2">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1.5">All day</p>
          <div className="flex flex-wrap gap-1.5">
            {allDayJobs.map(job => <Link key={job.id} to={`/jobs/${job.id}`} className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-[11px] font-semibold text-white ${barColor(job.status)}`}>
                {job.name}
              </Link>)}
          </div>
        </div>}
      ...
      <div className="flex flex-1 overflow-y-auto">
        {/* Time gutter */}
        <div className="w-16 shrink-0 border-r border-slate-200 bg-slate-50 relative" style={{
        height: TOTAL_HOURS * HOUR_HEIGHT
      }}>
          {hours.map(h => <div key={h} className="absolute w-full flex items-start justify-end pr-2" style={{
          top: (h - DAY_START_HOUR) * HOUR_HEIGHT - 8,
          height: HOUR_HEIGHT
        }}>
        ...
        height: TOTAL_HOURS * HOUR_HEIGHT
          ...
          {/* Hour lines */}
          {hours.map(h => <div key={h} className="absolute left-0 right-0 border-t border-slate-100" style={{
          top: (h - DAY_START_HOUR) * HOUR_HEIGHT
        }} />)}
          {/* Half-hour lines */}
          {hours.slice(0, -1).map(h => <div key={`h${h}`} className="absolute left-0 right-0 border-t border-slate-50" style={{
          top: (h - DAY_START_HOUR) * HOUR_HEIGHT + HOUR_HEIGHT / 2
        }} />)}

          {/* Now indicator */}
          {showNow && <div className="absolute left-0 right-0 z-20 flex items-center" style={{
          top: `${nowPct}%`
        }}>
          ...
          return <Link key={job.id} to={`/jobs/${job.id}`} title={`${job.name}${job.address ? `\n📍 ${job.address}` : ''}`} className={`absolute left-2 right-2 rounded-lg px-2 py-1 text-white shadow-sm hover:brightness-110 transition-all overflow-hidden ${barColor(job.status)}`} style={{
            top: `${top}%`,
            height: `${ht}%`,
            minHeight: 28
          }}>
                ...
                {job.address && <p className="text-[10px] opacity-70 leading-tight truncate flex items-center gap-0.5 mt-0.5">
                    <MapPin size={8} className="shrink-0" />{job.address}
                  </p>}
                {job.client && <p className="text-[10px] opacity-70 leading-tight truncate">{job.client}</p>}
              </Link>;
        })}

          {/* Empty state */}
          {dayJobs.length === 0 && <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-300 pointer-events-none">
...
function TimelineView({
  ...
  const headerDays = useMemo(() => Array.from({
    length: totalDays
  }, (_, i) => {
  ...
  function handleBarMouseDown(e: React.MouseEvent, job: SchedulerJob) {
    ...
    setDragging({
      jobId: job.id,
      startX: e.clientX,
      origStart: job.scheduledStartDate,
      origEnd: job.expectedCompletionDate
    });
    setDragOffset(0);
  }
  useEffect(() => {
  ...
  return <div className="overflow-x-auto select-none" ref={scrollRef}>
      <div style={{
      minWidth: totalDays * DW + 200
    }}>

        {/* Header */}
        <div className="flex border-b border-slate-200 bg-slate-50 sticky top-0 z-10">
          <div className="w-48 shrink-0 border-r border-slate-200" style={{
          minHeight: 48
        }}>
          ...
          <div className="flex flex-col flex-1">
            <div className="flex border-b border-slate-100">
              {headerDays.map((d, i) => {
              if (!shouldShowMonth(d, i)) return <div key={i} style={{
                width: DW
              }} className="shrink-0" />;
              let span = 1;
              ...
              return <div key={i} style={{
                width: span * DW
                  ...
                  month: 'short',
                  year: timeWindow === '3months' ? '2-digit' : undefined
                })}
                  </div>;
            })}
            </div>
            <div className="flex">
              ...
              return <div key={i} style={{
                width: DW
              }} className={`shrink-0 text-center py-0.5 border-r ${isMonthEdge ? 'border-slate-300' : 'border-slate-100'} ${isToday ? 'bg-violet-50' : isWeekend ? 'bg-slate-50/60' : ''}`}>
                    {showDayLabel(i) && <span className={`text-[10px] font-medium ${isToday ? 'text-violet-700 font-bold' : 'text-slate-400'}`}>
                        {d.getDate()}
                      </span>}
                  </div>;
            })}
        ...
        return <div key={job.id} className="flex border-b border-slate-100 hover:bg-slate-50/60 transition-colors" style={{
          height: 44
        }}>
            ...
            width: totalDays * DW
                ...
                {headerDays.map((d, i) => {
              const isWeekend = d.getDay() === 0 || d.getDay() === 6;
              return isWeekend ? <div key={i} className="absolute top-0 bottom-0 bg-slate-50/80" style={{
                left: i * DW,
                width: DW
              }} /> : null;
            })}
                {todayOffset >= 0 && todayOffset < totalDays && <div className="absolute top-0 bottom-0 w-px bg-violet-500 z-10 opacity-70" style={{
              left: todayOffset * DW + DW / 2
            }} />}
                {bar && <div title={`${job.name}\n${fmt(job.scheduledStartDate)} → ${fmt(job.expectedCompletionDate)}\nDrag to reschedule`} onMouseDown={e => handleBarMouseDown(e, job)} className={`absolute top-2.5 h-[18px] flex items-center text-white text-[10px] font-semibold truncate shadow-sm cursor-grab active:cursor-grabbing transition-all ${barColor(job.status)} ${bar.clippedLeft && !bar.clippedRight ? 'rounded-r-md' : bar.clippedRight && !bar.clippedLeft ? 'rounded-l-md' : bar.clippedLeft && bar.clippedRight ? '' : 'rounded-md'} ${isDraggingThis ? 'opacity-70 ring-2 ring-white/60' : 'hover:brightness-110'}`} style={{
              left: bar.left + dayShift * DW,
              width: bar.width
            }}>
        ...
        <div className="flex flex-wrap gap-3 px-4 py-3 border-t border-slate-100 bg-slate-50">
          {Object.entries(STATUS_BAR).map(([status, cls]) => <div key={status} className="flex items-center gap-1.5">
...
function CrewView({
  ...
  const allRows = [...members.map(m => ({
    ...
    id: 'unassigned',
    label: 'Unassigned',
    sublabel: `${unassignedJobs.length} jobs`,
    jobs: unassignedJobs
  }] : [])];
  return <div className="overflow-x-auto select-none">
      <div style={{
      minWidth: totalDays * DW + 200
    }}>
        ...
        <div className="flex border-b border-slate-200 bg-slate-50 sticky top-0 z-10">
          ...
          <div className="flex">
            ...
            return <div key={i} style={{
              width: DW
            }} className={`shrink-0 text-center py-1.5 border-r border-slate-100 ${isToday ? 'bg-violet-50' : isWeekend ? 'bg-slate-50/60' : ''}`}>
                  {showDayLabel(i) && <span className={`text-[10px] font-medium ${isToday ? 'text-violet-700 font-bold' : 'text-slate-400'}`}>
                      {d.getDate()}
                    </span>}
                </div>;
        ...
        return <div key={row.id} className="flex border-b border-slate-100" style={{
          height: rowH
        }}>
            ...
            width: totalDays * DW
              ...
              left: todayOffset * DW + DW / 2
                ...
                {lanes.map((lane, laneIdx) => lane.map(job => {
              const bar = barProps(job);
              if (!bar) return null;
              return <Link key={job.id} to={`/jobs/${job.id}`} title={`${job.name}${job.scheduledStartTime ? ` · ${fmtTime(job.scheduledStartTime)}${job.scheduledEndTime ? `–${fmtTime(job.scheduledEndTime)}` : ''}` : ''}\n${fmt(job.scheduledStartDate)} → ${fmt(job.expectedCompletionDate)}${job.address ? `\n📍 ${job.address}` : ''}`} className={`absolute h-[18px] flex items-center text-white text-[10px] font-semibold truncate shadow-sm hover:brightness-110 transition-all rounded-md ${barColor(job.status)}`} style={{
                left: bar.left,
                width: bar.width,
                top: 4 + laneIdx * 24
              }}>
...
function UnscheduledSection({
  ...
  jobs: SchedulerJob[];
  onSchedule: (job: SchedulerJob) => void;
}) {
  if (jobs.length === 0) return null;
  return <div className="mt-4 bg-white border border-slate-200 rounded-xl overflow-hidden">
      ...
      <div className="divide-y divide-slate-100">
        {jobs.map(job => {
        const style = getStatusStyle(job.status);
        return <div key={job.id} className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition-colors">
              ...
              <button onClick={() => onSchedule(job)} className="shrink-0 px-3 py-1.5 text-xs font-semibold text-violet-700 hover:bg-violet-50 border border-violet-200 rounded-lg transition-colors">
                Schedule
              </button>
  ...
  return <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input type="date" value={start} onChange={e => setStart(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-400" />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Start Time</label>
              <input type="time" value={startTime} onChange={e => setStartTime(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-400" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">End Date</label>
              <input type="date" value={end} onChange={e => setEnd(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-400" />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">End Time</label>
              <input type="time" value={endTime} onChange={e => setEndTime(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-400" />
            </div>
          </div>
        </div>
        <div className="flex gap-2 mt-5">
          ...
          <button onClick={handleSave} disabled={saving || !start || !end} className="flex-1 px-4 py-2 text-sm font-semibold text-white bg-violet-500 hover:bg-violet-700 rounded-lg transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
...
export default function SchedulerPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/scheduler/jobs', {
      ...
      setJobs(data.jobs ?? []);
      setError('');
    }).catch(() => setError('Failed to load scheduler data')).finally(() => setLoading(false));
  }, []);
  ...
  useEffect(() => {
    ...
    }).catch(() => {}).finally(() => setCrewLoading(false));
  }, [view]);
  ...
  const filtered = scheduled.filter(job => {
    ...
    if (search) {
      ...
      return job.name.toLowerCase().includes(q) || (job.client ?? '').toLowerCase().includes(q) || (job.address ?? '').toLowerCase().includes(q) || (job.jobNumber ?? '').toLowerCase().includes(q);
    }
    return true;
  });
  ...
  const windowLabel = useMemo(() => {
    ...
    if (timeWindow === 'day') return anchorDate.toLocaleDateString('en-AU', {
      weekday: 'long',
      day: 'numeric',
      month: 'long',
      year: 'numeric'
    ...
    if (timeWindow === 'month') return anchorDate.toLocaleDateString('en-AU', {
      month: 'long',
      year: 'numeric'
    });
    const s = anchorDate.toLocaleDateString('en-AU', {
      month: 'short',
      year: 'numeric'
    });
  ...
  return <div className="portal-page">
      <Helmet>
        <title>Scheduler — IWIIlBUILD</title>
        ...
        <meta property="og:title" content="Scheduler — IWIIlBUILD" />
        ...
        <meta name="twitter:title" content="Scheduler — IWIIlBUILD" />
      ...
      <div className="flex-1 flex flex-col min-w-0 lg-portal">
        ...
        <div className="op-page-header flex flex-wrap items-center gap-x-2 gap-y-1.5 shrink-0 min-w-0">
          ...
          {/* Home — mobile only */}
          <Link to="/home" title="Home" className="lg:hidden shrink-0 w-7 h-7 flex items-center justify-center rounded hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition-colors">
            <Home size={14} />
          </Link>
          ...
          <div className="flex items-center bg-gray-100 rounded p-0.5 gap-0.5 shrink-0">
            <button onClick={() => setSearchParams({})} className={`px-2.5 py-1 rounded text-xs font-semibold transition-all ${activeTab === 'jobs' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
              Jobs
            </button>
          ...
          <div className="ml-auto flex items-center gap-2 flex-wrap">
            ...
            {activeTab === 'jobs' && <>
                ...
                {view !== 'table' && view !== 'assets' && <div className="flex items-center bg-gray-100 rounded p-0.5 gap-0.5 shrink-0">
                    {(Object.keys(WINDOW_LABELS) as TimeWindow[]).map(key => <button key={key} onClick={() => switchWindow(key)} className={`px-2.5 py-1 rounded text-xs font-semibold transition-all ${timeWindow === key ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                        {WINDOW_LABELS[key]}
                      </button>)}
                  </div>}
                ...
                icon,
                label
              }) => <button key={key} onClick={() => setView(key)} className={`flex items-center gap-1 px-2.5 py-1 rounded text-xs font-semibold transition-all ${view === key ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                      {icon} {label}
                    </button>)}
                </div>
              </>}
          ...
          {activeTab === 'jobs' && view !== 'assets' && <div className="op-toolbar flex-wrap">
          <div className="relative">
            <Search size={12} className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400" />
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search jobs..." className="op-toolbar-search pl-7 w-40" />
          </div>

          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="op-toolbar-search appearance-none cursor-pointer">
            <option value="All">All statuses</option>
            {JOB_STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
          </select>

          {supervisors.length > 1 && <select value={supervisorFilter} onChange={e => setSupervisorFilter(e.target.value)} className="op-toolbar-search appearance-none cursor-pointer">
        ...
        <div className={`flex-1 overflow-y-auto ${view === 'assets' && activeTab === 'jobs' ? '' : 'p-3 md:p-4'}`}>
          ...
          {/* ── Tasks tab ── */}
          {activeTab === 'tasks' && <div className="bg-white border border-gray-200 rounded-md overflow-hidden">
              <TasksSchedulerView />
            </div>}
          ...
          {activeTab === 'jobs' && <>
              ...
              {view !== 'assets' && !loading && error && <div className="flex items-center gap-2 text-sm text-red-600 bg-red-50 border border-red-200 rounded-md p-4">
                  <AlertCircle size={16} /> {error}
                </div>}

              {view !== 'assets' && !loading && !error && <motion.div initial={{
            opacity: 0,
            y: 8
          }} animate={{
            opacity: 1,
            ...
            duration: 0.2
                  ...
                  {view !== 'crew' && <UnscheduledSection jobs={unscheduled} onSchedule={setScheduleTarget} />}
                </motion.div>}
            </>}
        </div>
      </div>
      ...
      {/* Quick Schedule Modal */}
      {scheduleTarget && <QuickScheduleModal job={scheduleTarget} onClose={() => setScheduleTarget(null)} onSave={handleQuickSchedule} />}
    </div>;
}
