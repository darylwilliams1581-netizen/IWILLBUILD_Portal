...
import { Receipt, ArrowLeft, Save, Send, Copy, Trash2, FileText, Plus, GripVertical, X, ChevronDown, Loader2, AlertCircle, Check, DollarSign, CreditCard, Ban, AlertTriangle, ChevronUp, User, Building2, RefreshCw, CheckCircle2, XCircle, Download, Share2, RotateCcw, Lock, Mail } from 'lucide-react';
...
function LineRow({
  ...
  return <div className="flex items-start gap-2 py-2 border-b border-slate-100 last:border-0 group">
      {/* Drag handle / order */}
      ...
      <div className="flex-1 min-w-0">
        <textarea value={line.description} onChange={e => onChange(line._key, 'description', e.target.value)} placeholder="Description of work or materials…" rows={2} className={`${inp} resize-none`} />
      </div>

      {/* Qty */}
      <div className="w-16 shrink-0">
        <input type="number" min="0" step="any" value={line.quantity} onChange={e => onChange(line._key, 'quantity', e.target.value)} className={inp} placeholder="1" />
      </div>

      {/* Unit */}
      <div className="w-20 shrink-0">
        <input value={line.unit} onChange={e => onChange(line._key, 'unit', e.target.value)} className={inp} placeholder="ea" />
      </div>

      {/* Rate */}
      {seeDollars && <div className="w-24 shrink-0">
          <input type="number" min="0" step="0.01" value={line.rate} onChange={e => onChange(line._key, 'rate', e.target.value)} className={inp} placeholder="0.00" />
        </div>}

      {/* Amount */}
      {seeDollars && <div className="w-24 shrink-0 pt-2 text-right">
      ...
      duration: 0.15
        ...
        <form onSubmit={submit} className="p-5 flex flex-col gap-4">
          <div>
            ...
            <input type="date" value={form.payment_date} onChange={e => setForm(f => ({
            ...f,
          ...
          <div>
            ...
            <input type="number" min="0.01" step="0.01" value={form.amount} onChange={e => setForm(f => ({
            ...f,
          ...
          <div>
            ...
            <select value={form.method} onChange={e => setForm(f => ({
            ...f,
            method: e.target.value
          }))} className={inp}>
              <option value="">Select…</option>
              {['Bank Transfer', 'Cash', 'Cheque', 'Credit Card', 'EFTPOS', 'Other'].map(m => <option key={m} value={m}>{m}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1.5">Reference</label>
            <input value={form.reference} onChange={e => setForm(f => ({
            ...f,
            reference: e.target.value
          }))} placeholder="e.g. BSB/Acc or receipt #" className={inp} />
          </div>
          <div>
...
export default function InvoiceBuilderPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetchInvoice(Number(id)).then(inv => {
      setInvoice(inv);
      setTitle(inv.title);
      ...
      setDirty(false);
    }).catch(() => setError('Invoice not found or failed to load.')).finally(() => setLoading(false));
  }, [id, isNew]);
  function markDirty() {
    setDirty(true);
    setSaveError('');
  ...
  function updateLine(key: string, field: keyof LineItem, val: string) {
    setLines(prev => prev.map(l => {
      ...
      if (field === 'quantity' || field === 'rate') {
        updated.amount = calcAmount(field === 'quantity' ? val : l.quantity, field === 'rate' ? val : l.rate);
      }
      return updated;
    }));
    markDirty();
  ...
  async function handleXeroSync() {
    ...
    if (!selectedCustomer) {
      setXeroMsg({
        type: 'error',
        text: 'Please add a customer before syncing to Xero.'
      });
      return;
    }
    ...
    if (nonEmptyLines.length === 0) {
      setXeroMsg({
        type: 'error',
        text: 'Please add at least one invoice line before syncing to Xero.'
      });
      return;
    }
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!l.description?.trim()) {
        setXeroMsg({
          type: 'error',
          text: `Line ${i + 1} needs a description before syncing to Xero.`
        });
        return;
      }
      if (!(l.quantity > 0)) {
        setXeroMsg({
          type: 'error',
          text: `Line ${i + 1} needs a valid quantity (greater than 0) before syncing to Xero.`
        });
        return;
      }
    }
    ...
    try {
      ...
      try {
        ...
        setXeroMsg({
          type: 'error',
          text: `Sync failed: server returned HTTP ${res.status}`
        });
        return;
      }
      if (!res.ok) {
        setXeroMsg({
          type: 'error',
          text: d.error ?? `Xero sync failed (HTTP ${res.status})`
        });
      } else {
        setXeroMsg({
          type: 'ok',
          text: d.message ?? 'Synced to Xero'
        });
        ...
        if (updated.ok) {
          const ud = (await updated.json()) as {
            invoice: Invoice;
          };
  ...
  async function handleSave(andNavigate = false) {
    if (!title.trim()) {
      setSaveError('Invoice title is required');
      return;
    }
    setSaving(true);
    setSaveError('');
    try {
      ...
      setDirty(false);
      if (andNavigate || isNew) navigate(`/invoices/${saved.id}`, {
        replace: true
      });
    } catch (err) {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          setSaveError(err instanceof Error ? err.message : 'Failed to recall');
        } finally {
          setSaving(false);
        }
      }
  ...
  const sourceEstimateId = (invoice as (typeof invoice & {
    source_estimate_id?: number;
  }) | null)?.source_estimate_id;
  ...
  return <div className="min-h-screen bg-gray-50 flex flex-col lg-portal">
      <PortalSidebar />
      <Helmet>
        <title>{isNew ? 'New Invoice' : `Invoice ${invoiceNumber}`} — IWIIlBUILD Portal</title>
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 safe-top">
        ...
        <button onClick={() => navigate('/invoices')} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
          Invoices
        </button>
        <span className="text-gray-300">|</span>
      ...
      <div className="flex-1 px-4 py-6 max-w-4xl mx-auto w-full">
        ...
        {!loading && !error && <div className="flex flex-col gap-5">

            {/* Sent banner */}
            {!isNew && isSent && <div className="flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
                ...
                sent_at?: string;
              }) | null)?.sent_at ? `Sent on ${new Date((invoice as (typeof invoice & {
                sent_at?: string;
              }))!.sent_at!).toLocaleDateString('en-AU', {
                day: 'numeric',
                month: 'short',
                year: 'numeric',
                hour: '2-digit',
                ...
                <button onClick={handleRecall} disabled={saving} className="flex items-center gap-1.5 text-xs font-bold text-amber-700 bg-amber-50 border border-amber-200 hover:bg-amber-100 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50 shrink-0">
            ...
            <div className="bg-white border border-border rounded-xl p-5">
              <div className="flex items-start justify-between gap-3 mb-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {!isNew && <span className={`inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full border ${s.bg} ${s.text} ${s.border} mt-0.5`}>
                ...
                <div className="flex items-center gap-2 flex-wrap justify-end">
                  {!isNew && canEdit && <button onClick={() => setShowPrintModal(true)} className="flex items-center gap-1.5 px-3 py-2 border border-border rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
                      <FileText size={13} />Preview / PDF
                    </button>}
                  {/* Send via Email */}
                  {!isNew && invoice && <button onClick={() => setShowEmailModal(true)} className="flex items-center gap-1.5 px-3 py-1.5 border border-border rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
                      <Mail size={13} />
                      <span className="hidden sm:inline">Send Email</span>
                      <span className="sm:hidden">Email</span>
                    </button>}
                  {!isNew && <button onClick={() => setShowShare(true)} className="flex items-center gap-1.5 px-3 py-2 border border-border rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors" title="Share link">
                      <Share2 size={13} />
                      Share
                    </button>}
                  {!isNew && canEdit && status === 'draft' && <button onClick={handleMarkSent} disabled={saving} className="flex items-center gap-1.5 px-3 py-2 border border-blue-200 bg-blue-50 rounded-lg text-xs font-semibold text-blue-700 hover:bg-blue-100 transition-colors disabled:opacity-50">
                      <Send size={13} />Send Invoice
                    </button>}
                  {/* Recall button — only on sent invoices */}
                  {!isNew && isSent && <button onClick={handleRecall} disabled={saving} title="Recall to draft — unlocks source estimate for adjustment" className="flex items-center gap-1.5 px-3 py-2 border border-amber-200 bg-amber-50 rounded-lg text-xs font-semibold text-amber-700 hover:bg-amber-100 transition-colors disabled:opacity-50">
                      <RotateCcw size={13} />Recall to Draft
                    </button>}
                  {!isNew && canEdit && !isPaid && !isVoid && <button onClick={() => setShowPaymentModal(true)} disabled={saving} className="flex items-center gap-1.5 px-3 py-2 border border-emerald-200 bg-emerald-50 rounded-lg text-xs font-semibold text-emerald-700 hover:bg-emerald-100 transition-colors disabled:opacity-50">
                      <DollarSign size={13} />Record Payment
                    </button>}
                  {/* Xero sync button — available on any saved, non-void invoice regardless of edit lock */}
                  {!isNew && !isVoid && canInvoices && <button onClick={handleXeroSync} disabled={xeroSyncing || saving} title={invoice?.accounting_invoice_id ? `Synced to Xero (${invoice.accounting_invoice_id})` : 'Push to Xero'} className={`flex items-center gap-1.5 px-3 py-2 border rounded-lg text-xs font-semibold transition-colors disabled:opacity-50 ${invoice?.accounting_sync_status === 'synced' ? 'border-[#13B5EA]/30 bg-[#13B5EA]/5 text-[#0fa0d4] hover:bg-[#13B5EA]/10' : invoice?.accounting_sync_status === 'error' ? 'border-red-200 bg-red-50 text-red-700 hover:bg-red-100' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50'}`}>
                      {xeroSyncing ? <Loader2 size={13} className="animate-spin" /> : invoice?.accounting_sync_status === 'synced' ? <CheckCircle2 size={13} /> : invoice?.accounting_sync_status === 'error' ? <XCircle size={13} /> : <span className="font-black text-xs">X</span>}
                      {invoice?.accounting_sync_status === 'synced' ? invoice?.accounting_invoice_id ? 'Re-sync Xero' : 'Synced' : invoice?.accounting_invoice_id ? 'Re-sync Xero' : 'Sync to Xero'}
                    </button>}
                  {!isNew && <button onClick={handleDuplicate} disabled={saving} className="flex items-center gap-1.5 px-3 py-2 border border-border rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors disabled:opacity-50">
                      <Copy size={13} />Duplicate
                    </button>}
                  {canEdit && <button onClick={() => handleSave(false)} disabled={saving || !dirty} className="flex items-center gap-1.5 px-4 py-2 bg-primary hover:bg-violet-700 text-white rounded-lg text-xs font-bold transition-colors disabled:opacity-50">
              ...
              {saveError && <div className="flex items-center gap-2 text-red-700 bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-sm mb-4">
                  <AlertCircle size={13} className="shrink-0" />{saveError}
                </div>}

              {xeroMsg && <div className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm mb-4 ${xeroMsg.type === 'ok' ? 'bg-[#13B5EA]/10 border border-[#13B5EA]/30 text-[#0a7fa0]' : 'bg-red-50 border border-red-200 text-red-700'}`}>
              ...
              <div className="flex flex-col gap-4">
                ...
                markDirty();
              }} disabled={!canEdit} placeholder="e.g. Progress Claim #1 — Bathroom Renovation" className={inp} />
                </div>
                <div>
                ...
                markDirty();
              }} disabled={!canEdit} placeholder="Auto-generated" className={inp} />
                </div>
                <div>
                ...
                markDirty();
              }} disabled={!canEdit} className={inp}>
            ...
            <div className="bg-white border border-border rounded-xl p-5">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  <CustomerSelector value={selectedCustomer} onChange={c => {
            ...
            <div className="bg-white border border-border rounded-xl p-5">
              ...
              <div className="flex flex-col">
                {lines.map((line, idx) => <LineRow key={line._key} line={line} idx={idx} total={lines.length} onChange={updateLine} onDelete={deleteLine} onMoveUp={k => moveLine(k, 'up')} onMoveDown={k => moveLine(k, 'down')} onCopy={copyLine} seeDollars={seeDollars} />)}
              </div>

              {canEdit && <button type="button" onClick={addLine} className="mt-3 flex items-center gap-2 text-sm font-semibold text-primary hover:text-violet-700 transition-colors">
                  <Plus size={14} />Add Line
                </button>}

              {/* Totals */}
              {seeDollars && <div className="mt-5 pt-4 border-t border-slate-100 flex flex-col items-end gap-1.5">
            ...
            <div className="bg-white border border-border rounded-xl p-5">
              ...
              <div className="flex flex-col gap-4">
                ...
                markDirty();
              }} disabled={!canEdit} rows={3} placeholder="Internal notes about this invoice…" className={`${inp} resize-y`} />
                </div>
                <div>
                  <label className={lbl}>Payment Terms</label>
                  <textarea value={terms} onChange={e => {
                setTerms(e.target.value);
                markDirty();
              }} disabled={!canEdit} rows={2} placeholder="e.g. Payment due within 14 days of invoice date." className={`${inp} resize-y`} />
                </div>
              </div>
            </div>

            {/* Payment history */}
            {!isNew && invoice?.payments && invoice.payments.length > 0 && seeDollars && <div className="bg-white border border-border rounded-xl p-5">
                <h2 className="font-heading font-bold text-sm text-muted-foreground uppercase tracking-wider mb-4">Payment History</h2>
                <div className="flex flex-col gap-2">
                  {invoice.payments.map(p => <div key={p.id} className="flex items-center justify-between text-sm py-2 border-b border-slate-50 last:border-0">
                      <div>
                        <span className="font-semibold text-foreground">{new Date(p.payment_date).toLocaleDateString('en-AU', {
                    day: 'numeric',
                    month: 'short',
                    year: 'numeric'
                  })}</span>
            ...
            {/* Danger zone */}
            {!isNew && <div className="bg-white border border-red-100 rounded-xl p-5">
                <h2 className="font-heading font-bold text-sm text-red-500 uppercase tracking-wider mb-3">Danger Zone</h2>
                <div className="flex flex-wrap gap-3">
                  {!isVoid && <button onClick={handleVoid} disabled={saving} className="flex items-center gap-2 px-4 py-2 border border-amber-200 bg-amber-50 text-amber-700 rounded-lg text-sm font-semibold hover:bg-amber-100 transition-colors disabled:opacity-50">
                      <Ban size={14} />Void Invoice
                    </button>}
                  {(status === 'draft' || status === 'void') && <button onClick={handleDelete} disabled={saving} className="flex items-center gap-2 px-4 py-2 border border-red-200 bg-red-50 text-red-700 rounded-lg text-sm font-semibold hover:bg-red-100 transition-colors disabled:opacity-50">
            ...
            {!isNew && <div className="bg-white border border-border rounded-xl p-5">
                ...
                {invoice?.accounting_provider === 'xero' && invoice?.accounting_invoice_id ? <div className="flex items-center gap-3 p-3 bg-[#13B5EA]/5 border border-[#13B5EA]/20 rounded-lg">
                    ...
                    <span className={`text-xs font-bold px-2 py-1 rounded-full border ${invoice.accounting_sync_status === 'synced' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : invoice.accounting_sync_status === 'error' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                  ...
                  </div> : <div className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-lg">
                ...
                {invoice?.accounting_sync_error && <div className="mt-2 flex items-start gap-2 text-xs text-red-700 bg-red-50 border border-red-200 rounded-lg px-3 py-2">
      ...
      {/* Modals */}
      <AnimatePresence>
        {showPaymentModal && invoice && <RecordPaymentModal invoiceId={invoice.id} balanceDue={balanceDue} onClose={() => setShowPaymentModal(false)} onSaved={updated => {
        setInvoice(updated);
        setStatus(updated.status);
        setShowPaymentModal(false);
      }} />}
        {showPrintModal && invoice && <InvoicePreviewModal invoice={{
        ...invoice,
        lines: lines.map((l, i) => ({
          id: i,
      ...
      {showEmailModal && invoice && <SendInvoiceEmailModal invoiceId={invoice.id} invoiceNumber={invoice.invoice_number ?? `#${invoice.id}`} onClose={() => setShowEmailModal(false)} />}

      {showShare && invoice && <ShareLinkModal open={showShare} onClose={() => setShowShare(false)} targetType="invoice" targetId={String(invoice.id)} title={invoice.title ?? `Invoice #${invoice.invoiceNumber ?? invoice.id}`} />}
      <JobContextTab />

      {/* ── Custom confirm dialog ─────────────────────────────────────── */}
      <AnimatePresence>
        {confirmDialog && <motion.div className="fixed inset-0 z-[200] flex items-center justify-center p-4" initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} exit={{
        opacity: 0
            ...
            <motion.div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col gap-4" initial={{
          scale: 0.92,
          opacity: 0,
          y: 12
        }} animate={{
          ...
          damping: 30
              ...
              <div className="flex gap-3 mt-1">
                <button onClick={() => setConfirmDialog(null)} className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-gray-700 font-semibold text-sm hover:bg-gray-50 transition-colors">
                  Cancel
                </button>
                <button onClick={confirmDialog.onConfirm} className={`flex-1 px-4 py-2.5 rounded-xl font-bold text-sm text-white transition-colors ${confirmDialog.danger ? 'bg-red-600 hover:bg-red-700' : 'bg-violet-500 hover:bg-violet-700'}`}>
                  Confirm
                </button>
              </div>
            </motion.div>
