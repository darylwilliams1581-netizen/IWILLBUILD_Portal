...
export default function ViewInvoicePage() {
  ...
  useEffect(() => {
    if (!id) { setError('Invalid invoice ID'); setLoading(false); return; }
    const numId = parseInt(id, 10);
    if (isNaN(numId)) { setError('Invalid invoice ID'); setLoading(false); return; }
    fetchInvoice(numId)
      .then(setInvoice)
        ...
        <div className="flex-1 overflow-auto">
          ...
          {!loading && invoice && (
            <div className="max-w-4xl mx-auto px-4 py-8">
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div>
                    ...
                    <p className="text-sm text-gray-400 print:text-gray-500">
                      ...
                      {invoice.issue_date ? ` · Issued ${new Date(invoice.issue_date).toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })}` : ''}
                      {invoice.due_date ? ` · Due ${new Date(invoice.due_date).toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })}` : ''}
                    </p>
                  </div>
                  {sc && (
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-bold border ${sc.bg} ${sc.text} ${sc.border}`}>
              ...
              {invoice.terms && (
                <div className="mt-6 bg-gray-900 border border-gray-800 rounded-2xl p-6 print:bg-white print:border-gray-200">
