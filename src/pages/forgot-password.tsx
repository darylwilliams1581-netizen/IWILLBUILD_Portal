...
export default function ForgotPasswordPage() {
  ...
  async function handleSmsSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!phone.trim()) { setSmsError('Please enter your mobile number.'); return; }
    setSmsError('');
    setSmsLoading(true);
    try {
          ...
          {smsAvailable && (
            <div className="px-8 pt-5">
              <div className="flex bg-white/5 border border-white/10 rounded-lg p-1 gap-1">
                <button
                  onClick={() => { setMode('email'); setEmailSent(false); setEmailError(''); }}
                ...
                <button
                  onClick={() => { setMode('sms'); setSmsSent(false); setSmsError(''); }}
                  className={`flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-md text-sm font-semibold transition-all ${
                    mode === 'sms'
                      ? 'bg-primary text-white shadow'
                      : 'text-white/40 hover:text-white/70'
