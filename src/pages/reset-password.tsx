...
function PasswordStrength({ password }: { password: string }) {
  const checks = [
    ...
    { label: 'At least one symbol', ok: /[!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?`~]/.test(password) },
  ];
  ...
  return (
    <div className="mt-2">
      ...
      <div className="flex flex-col gap-1">
        {checks.map((c) => (
          <div key={c.label} className={`flex items-center gap-1.5 text-xs ${c.ok ? 'text-green-400' : 'text-white/30'}`}>
            <CheckCircle size={11} />
            {c.label}
          </div>
        ))}
...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/auth/validate-reset-token?token=${encodeURIComponent(token)}&uid=${encodeURIComponent(uid)}`)
      .then((r) => r.json())
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/auth/reset-password', {
        ...
        body: JSON.stringify({ token, userId: uid, newPassword }),
      });
