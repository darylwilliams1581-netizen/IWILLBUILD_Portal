...
function MilestoneForm({
  ...
  return <AnimatePresence>
      ...
      opacity: 0
    }} className="fixed inset-0 z-50 bg-black/40 backdrop-blur-[2px] flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
        <motion.div initial={{
        y: 60,
        opacity: 0
      }} animate={{
        ...
        damping: 28,
        stiffness: 300
      }} className="bg-white w-full sm:max-w-lg rounded-t-3xl sm:rounded-2xl overflow-hidden" style={{
        boxShadow: '0 -4px 40px rgba(0,0,0,0.15)'
          ...
          <div className="px-5 py-4 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <div>
              ...
              <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Slab pour complete" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-violet-400" />
            </div>

            {/* Description */}
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1">Description</label>
              <textarea value={description} onChange={e => setDescription(e.target.value)} rows={2} placeholder="Optional notes…" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-violet-400 resize-none" />
            </div>
            ...
            <div className="flex flex-col gap-3">
              <div>
                ...
                <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-violet-400" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1">Due Date</label>
                <input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-violet-400" />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                const Icon = cfg.icon;
                return <button key={s} onClick={() => setStatus(s)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-semibold transition-colors ${status === s ? 'bg-violet-500 border-violet-500 text-white' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
            ...
            <div>
              ...
              <input value={assignedTo} onChange={e => setAssignedTo(e.target.value)} placeholder="e.g. John Smith" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-violet-400" />
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                {COLORS.map(c => <button key={c.value} onClick={() => setColor(c.value)} className={`w-7 h-7 rounded-full ${c.cls} transition-transform ${color === c.value ? 'ring-2 ring-offset-2 ring-violet-500 scale-110' : 'hover:scale-105'}`} title={c.label} />)}
              </div>
            </div>
          </div>
          ...
          <div className="px-5 py-4 border-t border-gray-100 flex gap-2">
            ...
            <button onClick={() => void handleSave()} disabled={!title.trim() || saving} className="flex-1 py-2.5 rounded-xl bg-violet-500 hover:bg-violet-600 text-white text-sm font-bold disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
...
export default function JobSchedulePage() {
  ...
  const load = () => {
    ...
    }>).then(data => setMilestones(data.milestones ?? []))]).catch(() => {}).finally(() => setLoading(false));
  };
  useEffect(load, [id]);
  ...
  const deleteMilestone = async (milestoneId: number) => {
    ...
    try {
      await fetch(`/api/jobs/${id}/milestones/${milestoneId}`, {
        method: 'DELETE',
        credentials: 'include'
      });
  ...
  return <div className="flex-1 bg-gray-50 flex flex-col lg-portal">
      ...
      paddingTop: 'max(env(safe-area-inset-top), 12px)'
        ...
        <button onClick={() => navigate(`/jobs/${id}`)} className="hidden md:flex w-9 h-9 rounded-xl bg-gray-100 items-center justify-center text-gray-600 hover:bg-gray-200 transition-colors shrink-0">
          ...
          <div className="hidden md:flex items-center gap-1.5 shrink-0">
            <button onClick={() => navigate(-1)} className="flex items-center justify-center w-9 h-9 rounded-lg bg-violet-500 text-white hover:bg-violet-700 active:bg-violet-800 transition-colors touch-manipulation shadow-sm" title="Dashboard"><Home size={18} /></button>
          </div>
      ...
      paddingBottom: 'env(safe-area-inset-bottom)'
        ...
        <div className="flex items-center gap-2 px-3 py-2">
          ...
          <button onClick={() => navigate(-1)} aria-label="Dashboard" className="w-10 h-10 rounded-xl bg-violet-50 border border-violet-200 flex items-center justify-center text-violet-600 active:bg-violet-100 transition-colors touch-manipulation shrink-0">
            <Home size={16} />
          </button>
          ...
          setEditTarget(null);
          setShowForm(true);
        }} className="flex items-center gap-1.5 px-3 py-1.5 bg-violet-500 hover:bg-violet-600 text-white text-xs font-bold rounded-lg transition-colors shrink-0">
              <Plus size={13} /> Add
            </button>}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? <div className="flex items-center justify-center py-20">
          ...
          </div> : <div className="px-4 py-5 pb-24 max-w-2xl mx-auto w-full space-y-5">

            {/* Progress bar */}
            {milestones.length > 0 && <div className="bg-white rounded-2xl border border-gray-100 px-5 py-4" style={{
          boxShadow: '0 1px 6px rgba(0,0,0,0.06)'
                ...
                <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
                  <motion.div className="h-full bg-violet-500 rounded-full" initial={{
              width: 0
            }} animate={{
              width: `${progress}%`
            }} transition={{
              ...
              ease: 'easeOut'
                ...
                <div className="flex items-center gap-4 mt-2.5">
                  ...
                  {milestones.filter(m => isOverdue(m)).length > 0 && <span className="text-xs text-red-500 font-semibold">
            ...
            {/* Empty state */}
            {milestones.length === 0 && <div className="bg-white rounded-2xl border border-gray-100 px-6 py-14 text-center" style={{
          boxShadow: '0 1px 4px rgba(0,0,0,0.06)'
            ...
            {/* Timeline */}
            {milestones.length > 0 && <div className="relative">
                {/* Vertical line */}
              ...
              return <motion.div key={m.id} initial={{
                opacity: 0,
                x: -8
              }} animate={{
                opacity: 1,
                ...
                delay: i * 0.05
                        ...
                        <div className="relative z-10 shrink-0 mt-3">
                          <button onClick={() => void quickStatus(m)} disabled={!canEdit} className={`w-10 h-10 rounded-full border-2 ${cfg.ring} bg-white flex items-center justify-center transition-transform ${canEdit ? 'hover:scale-110 cursor-pointer' : 'cursor-default'}`} title={canEdit ? 'Click to advance status' : cfg.label}>
                            <StatusIcon size={16} className={es === 'completed' ? 'text-emerald-500' : es === 'in_progress' ? 'text-blue-400' : es === 'overdue' ? 'text-red-400' : 'text-gray-300'} />
                          </button>
                        </div>

                        {/* Card */}
                        <div className="flex-1 bg-white rounded-2xl border border-gray-100 overflow-hidden" style={{
                  boxShadow: '0 1px 6px rgba(0,0,0,0.06)'
                }}>
                          {/* Color accent bar */}
                          ...
                          <div className="px-4 py-3">
                            <div className="flex items-start gap-2">
                              <div className="flex-1 min-w-0">
                                ...
                                {m.description && <p className="text-gray-400 text-xs mt-0.5 leading-relaxed">{m.description}</p>}
                              </div>
                              {/* Status badge */}
                              <span className={`shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full ${es === 'completed' ? 'bg-emerald-50 text-emerald-700' : es === 'in_progress' ? 'bg-blue-50 text-blue-700' : es === 'overdue' ? 'bg-red-50 text-red-600' : 'bg-gray-100 text-gray-500'}`}>
                                {cfg.label}
                              </span>
                            </div>
                            ...
                            <div className="flex flex-wrap items-center gap-3 mt-2">
                              {(m.start_date || m.due_date) && <div className="flex items-center gap-1 text-xs text-gray-400">
                                  <Calendar size={11} />
                                  {m.start_date && m.due_date ? `${fmtDate(m.start_date)} → ${fmtDate(m.due_date)}` : m.due_date ? `Due ${fmtDate(m.due_date)}` : `From ${fmtDate(m.start_date)}`}
                                </div>}
                              {m.assigned_to && <div className="flex items-center gap-1 text-xs text-gray-400">
                                  <Flag size={11} />
                                  {m.assigned_to}
                                </div>}
                            </div>
                            ...
                            {canEdit && <div className="flex items-center gap-2 mt-3 pt-2 border-t border-gray-50">
                                <button onClick={() => {
                        setEditTarget(m);
                        setShowForm(true);
                                ...
                                <button onClick={() => void deleteMilestone(m.id)} disabled={isDeleting} className="flex items-center gap-1 text-xs font-semibold text-gray-400 hover:text-red-500 disabled:opacity-50 transition-colors">
                                  {isDeleting ? <Loader2 size={11} className="animate-spin" /> : <Trash2 size={11} />}
                                  Delete
                                </button>
                                <div className="flex-1" />
                                <button onClick={() => void quickStatus(m)} className="flex items-center gap-1 text-xs font-semibold text-violet-500 hover:text-violet-700 transition-colors">
      ...
      {/* ── Milestone form modal ── */}
      {showForm && <MilestoneForm initial={editTarget ?? undefined} onSave={editTarget ? data => updateMilestone(editTarget.id, data) : data => addMilestone(data)} onClose={() => {
      setShowForm(false);
      setEditTarget(null);
    }} />}
    </div>;
