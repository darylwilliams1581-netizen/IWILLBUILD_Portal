...
async function fetchSiteMeta(url: string): Promise<SiteMeta> {
  try {
    const res = await fetch(`/api/quick-links/site-meta?url=${encodeURIComponent(url)}`, {
      credentials: 'include'
    });
    if (!res.ok) return {
      favicon: null,
...
function TileIcon({
  ...
  const candidates = [link.favicon, googleFaviconUrl(link.url)].filter(Boolean) as string[];
  ...
  if (allFailed || candidates.length === 0) {
    return <div className={`${dim} ${radius} flex items-center justify-center ${textSize} font-black shrink-0 select-none`} style={{
      background: pal.bg,
      color: pal.text
    }}>
        {initial}
  ...
  return <img src={candidates[idx]} alt="" width={size === 'lg' ? 56 : size === 'sm' ? 32 : 44} height={size === 'lg' ? 56 : size === 'sm' ? 32 : 44} onError={() => {
...
function UrlPreviewStrip({
  ...
  return <div className="flex items-center gap-3 px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl">
      {loading ? <Loader2 size={18} className="animate-spin text-slate-400 shrink-0" /> : meta?.favicon ? <img src={meta.favicon} alt="" width={28} height={28} className="w-7 h-7 rounded-lg object-contain bg-white border border-slate-100 shrink-0" onError={e => {
      ...
      <div className="flex-1 min-w-0">
        {meta?.title && <p className="text-xs font-semibold text-slate-700 truncate">{meta.title}</p>}
        <p className="text-[11px] text-slate-400 truncate">{displayHost(normaliseUrl(url))}</p>
      </div>
      {meta?.ogImage && <img src={meta.ogImage} alt="" className="w-12 h-8 rounded-lg object-cover border border-slate-100 shrink-0" onError={e => {
      (e.target as HTMLImageElement).style.display = 'none';
    }} />}
    </div>;
}
...
function LinkModal({
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!label.trim()) {
      setErr('Please enter a label for this link.');
      return;
    }
    const norm = normaliseUrl(url);
    if (!norm || !isValidUrl(norm)) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      ease: 'easeOut' as const
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4">
          {err && <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-lg px-3 py-2.5 text-xs text-red-700">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <textarea value={url} onChange={e => {
              setUrl(e.target.value);
              setErr('');
            }} placeholder="https://portal.example.com" rows={2} className="w-full pl-9 pr-4 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors resize-none leading-relaxed" autoFocus />
            </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1.5">
              ...
              {metaLoading && <span className="ml-2 text-[10px] font-normal text-slate-400 inline-flex items-center gap-1">
            ...
            setErr('');
          }} placeholder="e.g. BYDA, Outlook, Teletrac, Xero…" className="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors" />
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      ease: 'easeOut' as const
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <button onClick={onConfirm} className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white text-sm font-bold px-4 py-2 rounded-lg transition-colors">
            <Trash2 size={13} />
            Remove
          </button>
        </div>
...
function LinkTile({
  ...
  return <motion.div layout initial={{
    ...
    duration: 0.15
      ...
      {isAdmin && <div className="absolute top-3 right-3 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={e => {
        e.stopPropagation();
        onEdit(link);
      }} title="Edit" className="p-1.5 rounded-lg bg-white border border-slate-200 text-slate-400 hover:text-slate-700 hover:border-slate-300 transition-colors shadow-sm">
            <Pencil size={11} />
          </button>
          <button onClick={e => {
        e.stopPropagation();
        onDelete(link);
      }} title="Remove" className="p-1.5 rounded-lg bg-white border border-slate-200 text-slate-400 hover:text-red-600 hover:border-red-200 transition-colors shadow-sm">
            <Trash2 size={11} />
          </button>
        </div>}
      ...
      {link.ogImage && <img src={link.ogImage} alt="" className="w-full h-20 object-cover rounded-xl border border-slate-100" onError={e => {
      (e.target as HTMLImageElement).style.display = 'none';
    }} loading="lazy" />}

      {/* Open button */}
      <a href={link.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-1.5 w-full py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold text-slate-600 hover:bg-primary hover:border-primary hover:text-white transition-colors">
        Open
...
export default function QuickLinksPage() {
  ...
  useEffect(() => {
    fetch('/api/company-settings', {
      credentials: 'include'
    }).then(r => r.ok ? r.json() : Promise.resolve({})).then((data: {
      structure?: {
  ...
  const persist = useCallback(async (next: QuickLink[]) => {
    ...
    setSaveErr('');
    try {
      const getRes = await fetch('/api/company-settings', {
        credentials: 'include'
      });
      const current = getRes.ok ? (await getRes.json()) as {
  ...
  function handleSave(label: string, url: string, meta: SiteMeta | null) {
    if (editTarget) {
      const next = links.map(l => l.id === editTarget.id ? {
        ...l,
        label,
        url,
        favicon: meta?.favicon ?? l.favicon,
        ogImage: meta?.ogImage ?? l.ogImage
      } : l);
  ...
  return <div className="flex-1 bg-gray-50 flex flex-col lg-portal">
      ...
      <PortalErrorBoundary inline>
        <div className="flex flex-col flex-1">
          ...
          <header className="sticky top-0 z-30 bg-white border-b border-border shrink-0 safe-top">
            <div className="flex items-center gap-2 px-4 h-12">
              ...
              {!loadingCfg && links.length > 0 && <span className="text-xs bg-slate-100 text-slate-500 font-semibold px-2 py-0.5 rounded-full shrink-0">
                  {links.length}
                </span>}
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6">
            ...
            {loadingCfg && <div className="flex items-center justify-center py-24">
                <Loader2 size={22} className="animate-spin text-slate-300" />
              </div>}

            {!loadingCfg && links.length === 0 && <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              setShowModal(true);
                    ...
                    <Plus size={15} />
                    Add first link
                  </button> : <p className="text-xs text-slate-400 italic">Ask an admin to add quick links.</p>}
              </div>}

            {!loadingCfg && links.length > 0 && <motion.div layout className="grid gap-4" style={{
            gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))'
          }}>
                <AnimatePresence mode="popLayout">
                  {links.map(link => <LinkTile key={link.id} link={link} isAdmin={isAdmin} onEdit={l => {
                setEditTarget(l);
                setShowModal(true);
        ...
        {/* ── Add / Edit modal ── */}
        <AnimatePresence>
          {showModal && <LinkModal initial={editTarget} saving={saving} onSave={handleSave} onClose={() => {
          setShowModal(false);
          setEditTarget(null);
        }} />}
        </AnimatePresence>
        ...
        {/* ── Delete confirm ── */}
        <AnimatePresence>
          {delTarget && <DeleteConfirm link={delTarget} onConfirm={handleDelete} onCancel={() => setDelTarget(null)} />}
        </AnimatePresence>
      </PortalErrorBoundary>
    </div>;
}
