...
async function fetchLists(): Promise<GlobalList[]> {
  const r = await fetch('/api/form-global-lists', { credentials: 'include' });
  if (!r.ok) throw new Error('Failed to load');
  const d = await r.json() as { lists: GlobalList[] };
  return d.lists ?? [];
}
...
async function updateList(id: number, patch: { name?: string; items?: string[] }): Promise<void> {
...
async function deleteList(id: number): Promise<void> {
  const r = await fetch(`/api/form-global-lists/${id}`, { method: 'DELETE', credentials: 'include' });
  if (!r.ok) throw new Error('Failed to delete');
}

// ── List card ─────────────────────────────────────────────────────────────────

function ListCard({ list, onUpdated, onDeleted }: {
  list: GlobalList;
  ...
  useEffect(() => { setItems(list.items); setNameVal(list.name); }, [list]);

  async function saveName() {
    if (!nameVal.trim() || nameVal.trim() === list.name) { setEditingName(false); setNameVal(list.name); return; }
    setSaving(true);
    try {
      await updateList(list.id, { name: nameVal.trim() });
      onUpdated({ ...list, name: nameVal.trim() });
  ...
  async function saveItems(next: string[]) {
    ...
    setSaving(true);
    try {
      await updateList(list.id, { items: next });
      onUpdated({ ...list, items: next });
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try { await deleteList(list.id); onDeleted(list.id); }
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      >
        ...
        {editingName ? (
          <input
            ...
            onBlur={saveName}
            onKeyDown={(e) => { if (e.key === 'Enter') void saveName(); if (e.key === 'Escape') { setEditingName(false); setNameVal(list.name); } }}
            onClick={(e) => e.stopPropagation()}
            className="flex-1 min-w-0 text-sm font-semibold text-slate-800 bg-white border border-primary/40 rounded-lg px-2 py-1 focus:outline-none focus:border-primary"
            autoFocus
          />
        ) : (
          <span className="flex-1 min-w-0 text-sm font-semibold text-slate-800 truncate">{list.name}</span>
        )}
        ...
        <button
          ...
          disabled={deleting}
          className="p-1.5 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors shrink-0 disabled:opacity-40"
          title="Delete list"
        >
      ...
      {expanded && (
        <div className="border-t border-slate-100 px-4 py-3 flex flex-col gap-2">
          {items.length === 0 && (
            <p className="text-xs text-slate-400 italic">No items yet — add some below.</p>
          )}
          {items.map((item, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <input
                ...
                onBlur={() => blurItem(i)}
                className="flex-1 min-w-0 text-xs px-2.5 py-1.5 rounded-lg border border-slate-200 bg-slate-50 focus:outline-none focus:border-primary/60 focus:bg-white text-slate-700"
              />
              <button
          ...
          <div className="flex gap-2 mt-1">
            <input
              ...
              className="flex-1 min-w-0 text-xs px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white focus:outline-none focus:border-primary/60 text-slate-700 placeholder:text-slate-400"
            />
            <button
              onClick={addItem}
    ...
    <div className="flex flex-col flex-1 min-h-0 lg-portal">
      ...
      <header className="sticky top-0 z-30 h-12 bg-white border-b border-border flex items-center px-4 shrink-0 gap-2 safe-top">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-primary text-white text-xs font-bold hover:bg-violet-700 transition-colors shrink-0"
        >
          <Plus size={13} />
          New list
        </button>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-2xl mx-auto px-4 py-5 flex flex-col gap-3">
          ...
          {/* Create form */}
          {showCreate && (
            <div className="bg-white rounded-2xl border border-primary/30 shadow-sm px-4 py-3 flex flex-col gap-3">
              ...
              <input
                ...
                placeholder="e.g. Trade Types, PPE Items, Risk Categories…"
                autoFocus
                className="w-full text-sm px-3 py-2 rounded-xl border border-slate-200 bg-slate-50 focus:outline-none focus:border-primary/60 focus:bg-white text-slate-800 placeholder:text-slate-400"
              />
              <div className="flex gap-2 justify-end">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-1.5 text-xs font-bold text-white bg-primary hover:bg-violet-700 rounded-lg transition-colors disabled:opacity-50"
                >
          ...
          {!loading && lists.length === 0 && !showCreate && (
            <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
              ...
              <button
                onClick={() => setShowCreate(true)}
                className="mt-2 flex items-center gap-1.5 px-4 py-2 rounded-xl bg-primary text-white text-sm font-bold hover:bg-violet-700 transition-colors"
              >
                <Plus size={14} />
                Create a list
              </button>
          ...
          {lists.map((list) => (
            <div key={list.id} id={`list-${list.id}`}>
              <ListCard
                list={list}
                onUpdated={(updated) => setLists((prev) => prev.map((l) => l.id === updated.id ? updated : l))}
