...
import { HardHat, Plus, Search, MapPin, ChevronRight, Loader2, AlertCircle, UserCheck, ArrowLeft } from 'lucide-react';
...
export default function JobsPage() {
  ...
  const filtered = jobs.filter(j => {
    ...
    const matchesSearch = !q || j.name.toLowerCase().includes(q) || (j.client ?? '').toLowerCase().includes(q) || (j.address ?? '').toLowerCase().includes(q) || (j.jobNumber ?? '').toLowerCase().includes(q);
      ...
      <div className="flex flex-col flex-1 min-h-0">
        ...
        <header className="md:hidden h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 shrink-0 sticky top-0 z-30 safe-top">
          <div className="flex items-center gap-2">
            <button onClick={() => navigate(-1)} className="p-1.5 -ml-1 rounded text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors" aria-label="Back">
              <ArrowLeft size={18} />
            </button>
        ...
        <header className="op-page-header hidden md:flex sticky top-0 z-30">
          <button onClick={() => navigate(-1)} className="p-1 -ml-0.5 rounded text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors shrink-0" aria-label="Back">
            <ArrowLeft size={15} />
          </button>
        ...
        {!loading && jobs.length > 0 && <div className="hidden md:flex op-toolbar">
            {[{
            label: 'In Progress',
            count: counts.active,
            ...
            color: 'text-blue-700'
          }].map(s => <span key={s.label} className="flex items-center gap-1 text-[12px] text-gray-500 pr-3 border-r border-gray-200 last:border-0">
            ...
            {customers.length > 0 && <select value={customerFilter ?? ''} onChange={e => setCustomerFilter(e.target.value ? Number(e.target.value) : null)} className="op-toolbar-search appearance-none cursor-pointer pr-2 w-36">
                <option value="">All customers</option>
                {customers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>}
            <div className="flex gap-1">
              {['All', 'Works in Progress', 'Quoting', 'On Hold', 'Completed'].map(f => <button key={f} onClick={() => setActiveFilter(f)} className={`op-filter-pill ${activeFilter === f ? 'active' : ''}`}>
        ...
        <div className="flex-1 overflow-y-auto p-3 md:p-4 flex flex-col gap-3">
          ...
          {!loading && jobs.length > 0 && <div className="md:hidden grid grid-cols-2 gap-2">
              {[{
              label: 'In Progress',
              count: counts.active,
              ...
              color: 'text-blue-600',
              bg: 'bg-blue-50'
            }].map(s => <div key={s.label} className={`${s.bg} rounded-lg px-3 py-2 border border-white flex items-center gap-2`}>
          ...
          <div className="md:hidden flex flex-col gap-2">
            <div className="relative">
              ...
              <input type="text" placeholder="Search jobs, clients, locations…" value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 bg-white border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors" />
            </div>
            {customers.length > 0 && <div className="relative">
                <UserCheck size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none" />
                <select value={customerFilter ?? ''} onChange={e => setCustomerFilter(e.target.value ? Number(e.target.value) : null)} className="w-full pl-8 pr-8 py-2.5 bg-white border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors appearance-none cursor-pointer">
                  <option value="">All customers</option>
                  {customers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
              </div>}
            <div className="flex flex-wrap gap-1.5">
              {['All', 'Works in Progress', 'Quoting', 'On Hold', 'Completed'].map(f => <button key={f} onClick={() => setActiveFilter(f)} className={`text-xs font-bold px-3 py-1.5 rounded-full border transition-colors whitespace-nowrap ${activeFilter === f ? 'bg-primary text-white border-primary' : 'bg-white text-muted-foreground border-border hover:border-primary hover:text-primary'}`}>
                  {f === 'Works in Progress' ? 'In Progress' : f}
                </button>)}
            </div>
          </div>
          ...
          {/* Empty state */}
          {!loading && !error && jobs.length === 0 && <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <button onClick={() => !isViewOnly && setShowNewJob(true)} disabled={isViewOnly} title={isViewOnly ? 'Subscribe to continue' : undefined} className="inline-flex items-center gap-2 bg-primary hover:bg-violet-700 text-white text-sm font-bold px-5 py-2.5 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                <Plus size={15} />{addWorkLabel}
              </button>
            </div>}

          {/* No results from filter/search */}
          {!loading && !error && jobs.length > 0 && filtered.length === 0 && <div className="text-center py-12 text-muted-foreground text-sm">
              No {workPlural.toLowerCase()} match your search or filter.
            </div>}

          {/* Job list */}
          {!loading && filtered.length > 0 && <motion.div variants={stagger} initial="hidden" animate="visible" className="bg-white border border-gray-200 rounded-md overflow-hidden">
              {filtered.map((job, idx) => {
              const s = getStatusStyle(job.status);
              return <motion.div key={job.id} variants={fadeUp}>
                    <a href={`/jobs/${job.id}`} target="_blank" rel="noopener noreferrer" onClick={e => handleJobClick(`/jobs/${job.id}`, e)} className={`block hover:bg-gray-50 transition-colors duration-100 group ${idx > 0 ? 'border-t border-gray-100' : ''}`} style={{
                  WebkitTapHighlightColor: 'transparent'
                      ...
                      <div className="flex items-center gap-3 px-4 py-3 md:py-1.5 min-h-[44px] md:min-h-0">
                        ...
                        {job.address && <span className="hidden lg:flex items-center gap-1 text-[11px] text-gray-400 truncate flex-1 min-w-0 max-w-[180px]">
