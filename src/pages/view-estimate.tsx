...
import { fetchEstimate, estimateTotals, lineCalc, getEstimateStatusStyle, type Estimate, type EstimateLine } from '@/lib/estimates-api';
  ...
  return <>
      ...
      <div className="min-h-screen bg-gray-950 flex flex-col">
          ...
          <div className="flex-1 min-w-0">
            {estimate && <>
                ...
                <p className="text-xs text-gray-400">
                  ...
                  {statusStyle && <span className={`ml-2 inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-xs font-semibold border ${statusStyle.bg} ${statusStyle.color}`}>
                      {estimate.status}
                    </span>}
                </p>
              </>}
          </div>
          {estimate && <div className="flex items-center gap-2 shrink-0">
            ...
            link: window.location.href
              ...
              <a href={`/estimates/${estimate.id}`} className="flex items-center gap-1.5 text-xs bg-violet-500 hover:bg-violet-700 text-white px-3 py-1.5 rounded-lg font-semibold transition-colors">
                <Download size={13} />
                Open Editor
              </a>
        ...
        <div className="flex-1 overflow-auto">
          ...
          {!loading && estimate && totals && <div className="max-w-4xl mx-auto px-4 py-8">
              {/* Header card */}
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div>
                    ...
                    <p className="text-sm text-gray-400 print:text-gray-500">
                      Estimate #{estimate.id} · Created {new Date(estimate.createdAt).toLocaleDateString('en-AU', {
                    day: 'numeric',
                    month: 'long',
                    year: 'numeric'
                  })}
                  ...
                  {statusStyle && <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-bold border ${statusStyle.bg} ${statusStyle.color}`}>
                      <span className={`w-2 h-2 rounded-full ${statusStyle.dot}`} />
                      {estimate.status}
                    </span>}
                </div>
                {estimate.notes && <p className="mt-4 text-sm text-gray-300 print:text-gray-600 bg-gray-800 print:bg-gray-50 rounded-lg px-4 py-3">
                    {estimate.notes}
                  </p>}
              </div>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden mb-6 print:bg-white print:border-gray-200">
                ...
                {lines.length === 0 ? <div className="px-6 py-8 text-center text-gray-500 text-sm">No line items</div> : <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {lines.map((line, i) => <tr key={line.id} className={`border-b border-gray-800 print:border-gray-100 ${i % 2 === 0 ? '' : 'bg-gray-800/30 print:bg-gray-50'}`}>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 print:bg-white print:border-gray-200">
                <div className="max-w-xs ml-auto space-y-2">
                  ...
                  <div className="flex justify-between text-base font-bold border-t border-gray-700 print:border-gray-300 pt-2 mt-2">
                    ...
                    <span className="text-violet-400 print:text-violet-700">{fmtMoney(totals.total)}</span>
                  </div>
                </div>
              </div>
            </div>}
