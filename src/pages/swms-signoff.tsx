...
import {
  ShieldCheck, Loader2, AlertTriangle, CheckCircle2, User,
  Building2, CreditCard, Pen, ChevronDown, ChevronUp, X,
...
function SignaturePad({ onSave, onClear }: { onSave: (dataUrl: string) => void; onClear: () => void }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {hasStrokes && (
        <button type="button" onClick={clear} className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-red-600 transition-colors">
...
export default function SwmsSignoffPage() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`/api/public/swms/${token}`)
      .then(r => r.json())
      .then((data: { swms?: SwmsData; signoffs?: Signoff[]; company?: CompanyInfo; error?: string }) => {
        if (data.error) { setError(data.error); return; }
        setSwms(data.swms ?? null);
        setSignoffs(data.signoffs ?? []);
        setCompany(data.company ?? null);
      })
      .catch(() => setError('Failed to load SWMS'))
      .finally(() => setLoading(false));
  }, [token]);

  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    if (!workerName.trim()) { setSubmitError('Your name is required'); return; }
    if (!hasRead) { setSubmitError('Please confirm you have read and understood the SWMS'); return; }
    ...
    try {
      ...
      const data = await res.json() as { signoff?: Signoff; error?: string };
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          {company?.name && (
            <p className="ml-auto text-xs text-slate-400 shrink-0 hidden sm:block">{company.name}</p>
          )}
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
        ...
        {/* Job info */}
        {(swms.job_name || swms.job_address) && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 flex flex-wrap gap-3 text-sm text-emerald-800">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      required
                      className="w-full pl-8 pr-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400"
                    />
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Construction induction card"
                      className="w-full pl-8 pr-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400"
                    />
                  </div>
                </div>
              ...
              <label className="flex items-start gap-3 cursor-pointer group">
                <div
                  onClick={() => setHasRead(r => !r)}
                  className={`mt-0.5 w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 transition-colors ${
                    hasRead ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300 group-hover:border-emerald-400'
                  }`}
                >
              ...
              <button
                type="submit"
                disabled={submitting || !workerName.trim() || !hasRead || !signatureData}
                className="w-full py-3 text-sm font-bold text-white bg-emerald-500 hover:bg-emerald-600 rounded-xl transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
