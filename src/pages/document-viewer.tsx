...
function StatusBadge({
  ...
  return <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${map[status] ?? 'bg-slate-100 text-slate-600'}`}>
      {status.replace(/_/g, ' ')}
    </span>;
}
...
function SharePanel({
  ...
  return <div className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col gap-4">
      <div className="flex items-center gap-2">
        <Share2 size={15} className="text-primary" />
        <h3 className="text-sm font-bold text-slate-800">Share Links</h3>
      </div>

      {error && <div className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2 flex items-center gap-2">
          <AlertTriangle size={12} /> {error}
        </div>}

      {/* New token — show once */}
      {newToken && <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            <button onClick={() => void copyLink(newToken)} className="flex items-center gap-1.5 text-xs bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg font-semibold">
              {copied ? <CheckCircle2 size={11} /> : <Copy size={11} />}
              {copied ? 'Copied!' : 'Copy Link'}
            </button>
            <a href={`${window.location.origin}/share/${newToken}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-emerald-600 hover:underline">
      ...
      {/* Active shares */}
      {activeShares.length > 0 && <div className="flex flex-col gap-2">
          {activeShares.map(s => <div key={s.id} className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-center justify-between gap-3">
              <div className="flex flex-col gap-0.5">
                ...
                <span className="text-xs text-blue-500 flex items-center gap-1">
                  ...
                  {s.expiresAt && <><span className="mx-1">·</span><Clock size={10} /> Expires {new Date(s.expiresAt).toLocaleDateString('en-AU')}</>}
                  {s.submittedAt && <><span className="mx-1">·</span><CheckCircle2 size={10} className="text-emerald-500" /> Submitted</>}
                </span>
              </div>
            </div>)}
        </div>}

      {activeShares.length === 0 && !newToken && <p className="text-xs text-slate-400">No active share links.</p>}

      {/* Create controls */}
      <div className="flex flex-col gap-2 pt-2 border-t border-slate-100">
        <div className="flex gap-2">
          {(['view', 'complete'] as const).map(m => <button key={m} type="button" onClick={() => setShareMode(m)} className={`flex-1 text-xs py-1.5 rounded-lg border font-semibold transition-colors ${shareMode === m ? 'bg-primary text-white border-primary' : 'bg-white text-slate-600 border-slate-200 hover:border-primary/50'}`}>
              {m === 'view' ? 'View only' : 'Complete form'}
            </button>)}
        </div>
        <button onClick={() => void handleCreate()} disabled={creating} className="flex items-center justify-center gap-1.5 text-xs bg-primary hover:bg-violet-700 text-white px-3 py-2 rounded-lg font-semibold disabled:opacity-50">
          {creating ? <Loader2 size={11} className="animate-spin" /> : <Share2 size={11} />}
          {activeShares.length > 0 ? 'Regenerate Link' : 'Create Share Link'}
        </button>
        {activeShares.length > 0 && <button onClick={() => void handleRevoke()} disabled={revoking} className="flex items-center justify-center gap-1.5 text-xs text-red-600 border border-red-200 hover:bg-red-50 px-3 py-2 rounded-lg font-medium disabled:opacity-50">
...
function EventLog({
  ...
  return <div className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col gap-3">
      <div className="flex items-center gap-2">
      ...
      <div className="flex flex-col gap-2 max-h-64 overflow-y-auto">
        {events.map(e => <div key={e.id} className="flex items-start gap-2.5">
...
function VersionHistory({
  ...
  return <div className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-2">
        {versions.map(v => <div key={v.id} className="flex items-center justify-between text-xs text-slate-600">
...
export default function DocumentViewerPage() {
  ...
  return <>
      <Helmet>
        <title>{doc ? `${doc.title} — IWIIlBUILD` : 'Document — IWIIlBUILD'}</title>
        <meta name="description" content={doc ? `View and manage ${doc.title} in IWIIlBUILD.` : 'Internal document viewer — IWIIlBUILD portal.'} />
        <link rel="canonical" href={`https://iwillbuild.com/documents/${id ?? ''}`} />
        <meta name="robots" content="noindex, nofollow" />
        {/* OG tags — required by SEO checker even on noindex pages */}
        <meta property="og:title" content={doc ? `${doc.title} — IWIIlBUILD` : 'Document — IWIIlBUILD'} />
        <meta property="og:description" content={doc ? `View and manage ${doc.title} in IWIIlBUILD.` : 'Internal document viewer — IWIIlBUILD portal.'} />
      ...
      <div className="portal-content max-w-5xl mx-auto px-4 py-6">
        ...
        {!loading && doc && detail && <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            {/* Main column */}
            <div className="lg:col-span-2 flex flex-col gap-5">
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-6">
                ...
                <div className="flex items-center gap-2 mt-4 pt-4 border-t border-slate-100 flex-wrap">
                  {doc.sourceModule === 'job_form_submission' && doc.jobId && <Link to={`/jobs/${doc.jobId}`} className="flex items-center gap-1.5 text-xs border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 px-3 py-1.5 rounded-lg font-medium transition-colors">
                      <ClipboardList size={12} /> View in Job
                    </Link>}
                  {doc.sourceModule === 'estimate' && <a href={`/view/estimate/${doc.sourceId}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 px-3 py-1.5 rounded-lg font-medium transition-colors">
                      <ExternalLink size={12} /> Open Estimate
                    </a>}
                  {doc.sourceModule === 'invoice' && <a href={`/view/invoice/${doc.sourceId}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 px-3 py-1.5 rounded-lg font-medium transition-colors">
                      <ExternalLink size={12} /> Open Invoice
                    </a>}
                  <button onClick={() => window.print()} className="flex items-center gap-1.5 text-xs border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 px-3 py-1.5 rounded-lg font-medium transition-colors">
            ...
            <div className="flex flex-col gap-5">
              ...
              <SharePanel documentId={doc.id} shares={detail.shares} onRefresh={() => void load()} />
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col gap-3">
                ...
                <button onClick={() => setSecureShareTarget({
              type: 'document',
              id: String(doc.id),
              title: doc.title,
      ...
      {secureShareTarget && <ShareLinkModal open={true} onClose={() => setSecureShareTarget(null)} target={secureShareTarget} />}
    </>;
}
