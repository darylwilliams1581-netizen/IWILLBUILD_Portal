...
describe('Job detail — pill navigation', () => {
  ...
  async function renderJobDetail(jobId = '42', search = '') {
    ...
    const { default: JobDetailPage } = await import('../job-detail');
    render(
      <HelmetProvider>
  ...
  it('switching to Tasks pill marks it active', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('job-pill-tasks')).toHaveAttribute('aria-selected', 'true');
      expect(screen.getByTestId('job-pill-details')).toHaveAttribute('aria-selected', 'false');
    });
  });
