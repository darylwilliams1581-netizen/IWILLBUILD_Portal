...
import { Receipt, Plus, Search, Loader2, AlertCircle, ChevronRight, Calendar, Building2, FileText, Clock, CheckCircle2, XCircle, Send, AlertTriangle, ArrowLeft } from 'lucide-react';
...
function InvoiceRow({
  ...
  return <Link to={`/invoices/${invoice.id}`} className="flex items-center gap-3 bg-white border border-border rounded-xl px-4 py-3.5 hover:border-primary/40 hover:shadow-sm transition-all group">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-0.5 text-xs text-muted-foreground">
          {invoice.job_name && <span className="flex items-center gap-1 truncate"><Building2 size={10} />{invoice.job_name}</span>}
          {invoice.customer_name && <span className="flex items-center gap-1 truncate"><FileText size={10} />{invoice.customer_name}</span>}
          {invoice.due_date && <span className="flex items-center gap-1 shrink-0">
              <Calendar size={10} />Due {new Date(invoice.due_date).toLocaleDateString('en-AU', {
            day: 'numeric',
            month: 'short',
            year: 'numeric'
          })}
      ...
      {seeDollars && <div className="text-right shrink-0">
          ...
          {parseFloat(invoice.balance_due) > 0 && parseFloat(invoice.balance_due) < parseFloat(invoice.total) && <p className="text-[10px] text-amber-600 font-semibold">Bal {fmtMoney(invoice.balance_due)}</p>}
          {parseFloat(invoice.balance_due) <= 0 && invoice.status === 'paid' && <p className="text-[10px] text-emerald-600 font-semibold">Paid in full</p>}
        </div>}
...
export default function InvoicesPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    setError('');
    fetchInvoices().then(setInvoices).catch(() => setError('Failed to load invoices.')).finally(() => setLoading(false));
  }, []);
  useEffect(() => {
    if (!permLoading && canInvoices) load();
  }, [permLoading, canInvoices, load]);
  const filtered = invoices.filter(inv => {
    const matchStatus = statusFilter === 'all' || inv.status === statusFilter;
    const q = search.toLowerCase();
    const matchSearch = !q || inv.invoice_number.toLowerCase().includes(q) || inv.title.toLowerCase().includes(q) || (inv.job_name ?? '').toLowerCase().includes(q) || (inv.customer_name ?? '').toLowerCase().includes(q);
  ...
  return <div className="portal-page">
      ...
      <div className="portal-content">
        ...
        <div className="flex items-center justify-between gap-3 mb-5">
          ...
          <button onClick={() => navigate('/invoices/new')} className="flex items-center gap-2 bg-primary hover:bg-violet-700 text-white text-sm font-bold px-4 py-2.5 rounded-lg transition-colors">
            <Plus size={15} />
        ...
        {seeDollars && !loading && invoices.length > 0 && <div className="grid grid-cols-3 gap-3 mb-5">
            {[{
          label: 'Unpaid',
          value: fmtMoney(totalUnpaid),
          count: unpaid.length,
        ...
        }].map(s => <div key={s.label} className={`${s.bg} border ${s.border} rounded-xl p-3 text-center`}>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="relative flex-1">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search invoices, jobs, customers…" className="w-full pl-9 pr-4 py-2.5 bg-white border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors" />
          </div>
          <div className="flex gap-1.5 flex-wrap">
            {STATUS_FILTERS.map(f => <button key={f.key} onClick={() => setStatusFilter(f.key)} className={`text-xs font-bold px-3 py-1.5 rounded-full border transition-colors whitespace-nowrap ${statusFilter === f.key ? 'bg-primary text-white border-primary' : 'bg-white text-muted-foreground border-border hover:border-primary hover:text-primary'}`}>
                {f.label}
              </button>)}
          </div>
        </div>
        ...
        {!loading && !error && invoices.length === 0 && <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <button onClick={() => navigate('/invoices/new')} className="inline-flex items-center gap-2 bg-primary hover:bg-violet-700 text-white text-sm font-bold px-5 py-2.5 rounded-lg transition-colors">
              <Plus size={15} />New Invoice
            </button>
          </div>}

        {!loading && invoices.length > 0 && filtered.length === 0 && <div className="text-center py-12 text-muted-foreground text-sm">No invoices match your search or filter.</div>}
        ...
        duration: 0.2
            ...
            {filtered.map(inv => <InvoiceRow key={inv.id} invoice={inv} seeDollars={seeDollars} />)}
          </motion.div>}
      </div>
    </div>;
}
