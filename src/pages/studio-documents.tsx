...
function ToolBtn({ icon: Icon, label, onClick, danger = false, variant }: {
  icon: React.ElementType; label: string; onClick?: (e: React.MouseEvent) => void;
  danger?: boolean; variant?: 'orange' | 'green';
}) {
  const cls = danger
  ...
  return (
    <button title={label} onClick={(e) => { e.stopPropagation(); onClick?.(e); }}
...
function DocRow({ doc, index, onDelete, onShare, showShareBtn }: {
  doc: DocTemplate; index: number;
  onDelete: (id: number) => void; onShare: (id: number) => void; showShareBtn?: boolean;
}) {
  ...
  function handleJobPickerSubmit() {
    ...
    if (!jobNum || isNaN(Number(jobNum)) || Number(jobNum) < 1) {
      setJobPickerError('Please enter a valid job number.'); return;
    }
  ...
  async function handleDuplicate(e: React.MouseEvent) {
    ...
    try {
      ...
      if (r.ok) { toast.success('Document duplicated'); window.location.reload(); }
      else toast.error('Could not duplicate document.');
    } catch { toast.error('Network error — could not duplicate document.'); }
  }
    ...
    >
      <div className="flex items-center gap-3 px-4 py-3 cursor-pointer" onClick={openBuilder}>
        ...
        <div className="flex-1 min-w-0 flex items-center gap-2.5">
          ...
          {hasAcknowledgement && (
            <span className="hidden md:inline-flex flex-shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full border bg-emerald-50 text-emerald-700 border-emerald-200">
              Sign-On
            </span>
          )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input type="number" min={1} placeholder="e.g. 1042" value={jobNumberInput}
                  onChange={(e) => { setJobNumberInput(e.target.value); setJobPickerError(''); }}
                  onKeyDown={(e) => { if (e.key === 'Enter') handleJobPickerSubmit(); }}
                  autoFocus
...
function SubmissionsTab({ templates }: { templates: DocTemplate[] }) {
  ...
  useEffect(() => {
    const url = templateFilter
      ? `/api/document-submissions?templateId=${templateFilter}`
      : '/api/document-submissions';
    fetch(url, { credentials: 'include' })
      .then(r => r.json())
      .then((d: { submissions?: DocSubmission[] }) => setSubmissions(d.submissions ?? []))
      .catch(() => {})
  ...
  return (
    <div className="p-6 pb-16 space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        <select value={templateFilter} onChange={e => { setTemplateFilter(e.target.value); setLoading(true); }}
                  ...
                  {isOpen && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                      ...
                      <div className="border-t border-slate-100 px-4 py-3 bg-slate-50">
                        {answerCount === 0 ? (
                          <p className="text-xs text-slate-400 italic">No fields recorded</p>
                        ) : (
...
export default function StudioDocumentsPage() {
  ...
  async function handleDelete(id: number) {
    try {
      ...
      if (r.ok) { setTemplates(prev => prev.filter(t => t.id !== id)); toast.success('Document deleted'); }
      else toast.error('Could not delete document.');
    } catch { toast.error('Network error — could not delete document.'); }
  }
  ...
  const handleOpenImporter = useCallback(async () => {
    ...
    } catch { toast.error('Could not start import. Please try again.'); }
  }, []);
  ...
  const handleStudioImported = useCallback(async (blocks: DocumentBlock[], docxName: string, templateId: number) => {
    try {
      await fetch(`/api/document-templates/${templateId}`, {
        ...
        body: JSON.stringify({ name: docxName.replace(/\.(docx|pdf)$/i, '') || 'Imported Document', blocks }),
      });
    } finally { navigate(`/studio/builder/${templateId}`); }
  }, [navigate]);
    ...
    <div className="flex flex-col flex-1 min-h-0 lg-portal">
      ...
      <header className="sticky top-0 z-30 h-12 bg-white border-b border-border flex items-center px-4 shrink-0 gap-2 safe-top">
        <button onClick={() => navigate('/home')}
          className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors shrink-0" aria-label="Back to Home">
        ...
        {([
          ...
          <button key={key} onClick={() => switchTab(key)}
            className={`flex items-center gap-1.5 px-4 py-3 text-sm font-semibold border-b-2 transition-colors ${
              pageTab === key ? 'border-primary text-primary' : 'border-transparent text-slate-500 hover:text-slate-700'
            }`}>
            <Icon size={13} />{label}
          </button>
        ))}
          ...
          <>
            ...
            <div className="px-6 py-4">
              ...
              {loading ? (
                ...
                <motion.div variants={{ visible: { transition: { staggerChildren: 0.015 } } }}
                  initial="hidden" animate="visible" className="flex flex-col gap-2">
                  {filtered.map((doc, i) => (
                    <DocRow key={doc.id} doc={doc} index={i} onDelete={handleDelete}
                      onShare={id => setShareDocId(id)} showShareBtn={isPlatformOwner} />
                  ))}
                </motion.div>
              )}
            </div>
      ...
      <AnimatePresence>
        {shareDocId && shareDoc && (
          <ShareToLibraryModal templateId={shareDocId} templateName={shareDoc.name}
            isPlatformOwner={isPlatformOwner} onClose={() => setShareDocId(null)} />
        )}
      </AnimatePresence>

      {/* Import modal */}
      {showImporter && importTemplateId !== null && (
        <DocxImporter templateId={importTemplateId} hasExistingBlocks={false}
