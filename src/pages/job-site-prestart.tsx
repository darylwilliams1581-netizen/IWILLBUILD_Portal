...
import { ChevronLeft, ChevronDown, ChevronUp, Plus, CheckCircle2, AlertTriangle, Loader2, ClipboardCheck, Users, FileText, Pen, X, Check, Printer, HardHat, Shield, Info, ChevronRight, Clock, Wrench, Phone, CalendarDays, Home, Search } from 'lucide-react';
...
function SignaturePad({
  ...
  return <div className="space-y-1">
      ...
      <div className="border-2 border-dashed border-slate-300 rounded-xl bg-white overflow-hidden">
        <canvas ref={canvasRef} width={600} height={160} className="w-full touch-none" style={{
        height: 120
...
function Section({
  ...
  return <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center gap-3 px-4 py-3.5 text-left">
        ...
        {badge && <Badge variant="secondary" className="text-xs mr-1">{badge}</Badge>}
...
function CheckRow({
  ...
  return <button type="button" disabled={disabled} onClick={() => onChange(!checked)} className={cn('w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border text-left transition-colors', checked ? 'bg-green-50 border-green-300' : 'bg-slate-50 border-slate-200', disabled && 'opacity-50 cursor-not-allowed')}>
      <div className={cn('w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-colors', checked ? 'bg-green-500 border-green-500' : 'border-slate-300')}>
...
function PrestartList({
  ...
  return <div className="space-y-3">
      ...
      {prestarts.length === 0 ? <div className="text-center py-10 text-slate-400 text-sm">
          No prestarts yet for this job.
        </div> : <div className="space-y-2">
          {prestarts.map(p => <button key={p.id} onClick={() => onSelect(p)} className="w-full flex items-center gap-3 p-3.5 rounded-xl border border-slate-200 bg-white text-left hover:border-violet-300 transition-colors">
              <div className={cn('w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0', p.status === 'finalised' ? 'bg-green-100' : 'bg-amber-100')}>
                <ClipboardCheck size={18} className={p.status === 'finalised' ? 'text-green-600' : 'text-amber-600'} />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-slate-800 truncate">
                  {p.prestart_date ? new Date(p.prestart_date).toLocaleDateString('en-AU', {
              weekday: 'short',
              day: 'numeric',
              month: 'short'
            }) : 'No date'}
...
function NewPrestartDialog({
  ...
  useEffect(() => {
    fetch(`/api/jobs/${jobId}/site-prestarts`).then(r => r.json()).then(d => setPreviousPrestarts((d.prestarts ?? []).filter((p: SitePrestart) => p.status === 'finalised')));
  }, [jobId]);
  ...
  return <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
      ...
      }} className={cn('p-4 rounded-xl border-2 text-left transition-colors', mode === 'blank' ? 'border-violet-600 bg-violet-50' : 'border-slate-200 bg-white')}>
        ...
        <button onClick={() => setMode('copy')} disabled={previousPrestarts.length === 0} className={cn('p-4 rounded-xl border-2 text-left transition-colors', mode === 'copy' ? 'border-violet-600 bg-violet-50' : 'border-slate-200 bg-white', previousPrestarts.length === 0 && 'opacity-40 cursor-not-allowed')}>
      ...
      {mode === 'copy' && previousPrestarts.length > 0 && <div className="space-y-2">
          <p className="text-xs font-medium text-slate-600">Select prestart to copy from:</p>
          {previousPrestarts.slice(0, 5).map(p => <button key={p.id} onClick={() => setSelectedCopyId(p.id)} className={cn('w-full flex items-center gap-3 p-3 rounded-xl border text-left transition-colors', selectedCopyId === p.id ? 'border-violet-600 bg-violet-50' : 'border-slate-200 bg-white')}>
      ...
      <div className="flex gap-2 pt-1">
        ...
        <Button onClick={create} disabled={!mode || mode === 'copy' && !selectedCopyId || creating} className="flex-1 bg-violet-500 hover:bg-violet-700 text-white rounded-xl">
...
function WorkerSignOnScreen({
  ...
  useEffect(() => {
    ...
    }) => setMembers(d.members ?? [])).catch(() => setMembers([])).finally(() => setMembersLoading(false));
  }, []);
  const filteredMembers = memberQuery.trim() ? members.filter(m => m.name.toLowerCase().includes(memberQuery.toLowerCase())) : members;
  ...
  const swmsSnapshot = prestart.swms_snapshot ? (typeof prestart.swms_snapshot === 'string' ? JSON.parse(prestart.swms_snapshot) : prestart.swms_snapshot) as Array<{
    id: number;
    title: string;
  }> : [];
  ...
  return <div className="flex-1 bg-slate-50 flex flex-col overflow-hidden">
      {/* Locked summary */}
      <div className="bg-violet-500 text-white px-4 pt-3 pb-4 safe-top">
        ...
        <div className="bg-white/15 rounded-xl p-3 space-y-1 text-sm">
          ...
          {prestart.planned_work && <p><span className="opacity-70 text-xs">Today's work</span> <span className="line-clamp-2">{prestart.planned_work}</span></p>}
          {swmsSnapshot.length > 0 ? <p><span className="opacity-70 text-xs">SWMS reviewed</span> <span>{swmsSnapshot.map(s => s.title).join(', ')}</span></p> : <p className="flex items-center gap-1.5">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {success ? <div className="flex flex-col items-center justify-center py-12 gap-3">
          ...
          }} className={`flex-1 h-10 rounded-xl text-sm font-semibold border transition-colors ${mode === 'pick' ? 'bg-violet-500 border-violet-600 text-white' : 'bg-white border-slate-300 text-slate-600 hover:border-violet-400'}`}>
                Pick from team
              </button>
              <button type="button" onClick={() => {
            setMode('new');
            ...
            setMemberQuery('');
          }} className={`flex-1 h-10 rounded-xl text-sm font-semibold border transition-colors ${mode === 'new' ? 'bg-violet-500 border-violet-600 text-white' : 'bg-white border-slate-300 text-slate-600 hover:border-violet-400'}`}>
                New person
              </button>
            </div>

            {mode === 'pick' ? (/* ── Team member picker ── */
        <div className="space-y-2">
                <div className="flex items-center gap-2 bg-slate-100 rounded-xl px-3 py-2.5">
                  <Search size={14} className="text-slate-400 shrink-0" />
                  <input type="text" value={memberQuery} onChange={e => setMemberQuery(e.target.value)} placeholder="Search team…" className="flex-1 bg-transparent text-sm text-slate-800 placeholder-slate-400 outline-none" />
                  {memberQuery && <button onClick={() => setMemberQuery('')} className="text-slate-400">
                      <X size={13} />
                    </button>}
                </div>
                {membersLoading ? <div className="flex items-center justify-center gap-2 py-6 text-slate-400 text-sm">
                    <Loader2 size={15} className="animate-spin" /> Loading team…
                  </div> : filteredMembers.length === 0 ? <p className="text-center text-slate-400 text-sm py-4">
                    {memberQuery ? 'No matches' : 'No team members found'}
                  </p> : <div className="space-y-1.5 max-h-52 overflow-y-auto">
                    {filteredMembers.map(m => {
              const alreadySigned = signedNames.has(m.name.toLowerCase());
              const selected = form.fullName === m.name;
              return <button key={m.userId} type="button" disabled={alreadySigned} onClick={() => setForm(f => ({
                ...f,
                fullName: m.name
              }))} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border text-left transition-colors ${selected ? 'bg-violet-50 border-violet-300' : alreadySigned ? 'bg-slate-50 border-slate-200 opacity-50 cursor-not-allowed' : 'bg-white border-slate-200 hover:border-violet-300 hover:bg-violet-50'}`}>
              ...
              </div>) : (/* ── New person — manual name entry ── */
        <Field label="Full Name *">
                <Input value={form.fullName} onChange={e => setForm(f => ({
            ...f,
            fullName: e.target.value
          }))} placeholder="Visitor or subcontractor name" className="h-12 text-base rounded-xl" autoFocus />
              </Field>)}

            {/* Selected name confirmation */}
            {form.fullName && <div className="flex items-center gap-2 px-3 py-2 bg-violet-50 border border-violet-200 rounded-xl">
            ...
            <Field label="Role *">
              <button type="button" onClick={() => setRolePickerOpen(true)} className={`w-full h-12 rounded-xl border px-4 text-sm font-medium flex items-center justify-between gap-2 transition-colors ${form.roleTrade ? 'border-violet-400 bg-violet-50 text-violet-800' : 'border-slate-300 bg-white text-slate-400 hover:border-violet-400'}`}>
                <span>{form.roleTrade || 'Select role…'}</span>
                <ChevronRight size={15} className="shrink-0 text-slate-400" />
              </button>
            </Field>

            <CheckRow label="I am fit for work today" checked={form.fitForWork} onChange={v => setForm(f => ({
          ...f,
          fitForWork: v
        }))} />

            <Field label="Signature *">
              <SignaturePad onSave={sig => setForm(f => ({
            ...f,
            signature: sig
          }))} onClear={() => setForm(f => ({
            ...f,
            ...
            <Button onClick={submit} disabled={submitting} className="w-full h-14 bg-violet-500 hover:bg-violet-700 text-white rounded-xl text-base font-bold">
              {submitting ? <Loader2 size={18} className="animate-spin mr-2" /> : <Pen size={18} className="mr-2" />}
              Sign On
            </Button>
          </>}

        {workers.length > 0 && <div className="space-y-2">
            {/* Role summary */}
            <div className="flex flex-wrap gap-1.5 pb-1">
              <span className="text-xs font-semibold text-slate-500">{workers.length} on site</span>
              {Object.entries(workers.reduce<Record<string, number>>((acc, w) => {
            const r = w.role_trade || 'Unspecified';
            acc[r] = (acc[r] ?? 0) + 1;
            return acc;
          }, {})).sort(([a], [b]) => a.localeCompare(b)).map(([role, count]) => <span key={role} className="text-xs text-slate-400">· {count} {role}</span>)}
            </div>
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Signed on</p>
            {workers.map(w => {
          const badge = ROLE_BADGE[w.role_trade];
          return <div key={w.id} className="flex items-center gap-3 p-3 bg-white rounded-xl border border-slate-200">
                  <CheckCircle2 size={16} className="text-green-500 flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-slate-800 truncate">{w.full_name}</p>
                    <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                      {badge ? <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${badge.bg} ${badge.text}`}>{badge.label}</span> : <span className="text-xs text-slate-400">{w.role_trade || 'Unspecified'}</span>}
        ...
        maxHeight: '70dvh'
          ...
          maxHeight: 'calc(70dvh - 60px)'
            ...
            return <button key={role} type="button" onClick={() => {
              ...
              setRolePickerOpen(false);
            }} className={`w-full flex items-center justify-between px-5 py-4 text-left transition-colors ${i > 0 ? 'border-t border-slate-100' : ''} ${active ? 'bg-violet-50' : 'bg-white active:bg-slate-50'}`}>
...
const SITE_ROLES = ['Worker', 'Apprentice', 'Operator', 'Supervisor', 'Person in Charge of Construction', 'First Aid Officer', 'Safety Observer', 'Visitor', 'Delivery Driver'] as const;
type SiteRole = typeof SITE_ROLES[number];

// Roles that get a highlighted badge in the signed-on list
const ROLE_BADGE: Record<string, {
  bg: string;
  text: string;
  label: string;
}> = {
...
export default function JobSitePrestartPage() {
  ...
  useEffect(() => {
    if (!jobId) return;
    fetch(`/api/jobs/${jobId}/swms`).then(r => r.json()).then(d => setJobSwms(d.jobSwms ?? []));
  }, [jobId]);
  ...
  async function loadPrestart(p: SitePrestart) {
    ...
    for (const key of ['situation_checkboxes', 'execution_checklist', 'admin_checklist', 'relevant_swms_ids', 'swms_snapshot']) {
      ...
      if (typeof val === 'string') {
        try {
          (full as Record<string, unknown>)[key] = JSON.parse(val);
        } catch {/* ignore */}
      }
    }
    setPrestart(full);
  ...
  function toggleSwms(swmsId: number) {
    ...
    const updated = current.includes(swmsId) ? current.filter(id => id !== swmsId) : [...current, swmsId];
    // Build snapshot
  ...
  if (view === 'signon' && prestart) {
    return <WorkerSignOnScreen prestart={prestart} workers={workers} onWorkerAdded={w => setWorkers(ws => [...ws, w])} onClose={() => setView('form')} />;
  }
  return <>
      <Helmet>
      ...
      <div className="min-h-dvh bg-slate-50">
        ...
        <div className="bg-white border-b border-slate-200 sticky top-0 z-20 safe-top">
          <div className="flex items-center gap-3 px-4 pt-3 pb-3">
            ...
            {view === 'form' && prestart && <div className="flex items-center gap-2">
                ...
                <div className="hidden sm:flex items-center gap-2">
                  {prestart.status === 'finalised' && <button onClick={printPrestart} className="p-2 rounded-xl hover:bg-slate-100" title="Print">
                      <Printer size={18} className="text-slate-600" />
                    </button>}
                </div>
                <MobileOverflowMenu surface="light" className="sm:hidden" items={[{
              label: 'Print',
        ...
        <div className="px-4 py-4 max-w-lg mx-auto space-y-4 pb-24">
          ...
          {/* List view */}
          {view === 'list' && <PrestartList jobId={jobId} onSelect={loadPrestart} onNew={() => setView('new')} />}

          {/* New prestart dialog */}
          {view === 'new' && <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
              <NewPrestartDialog jobId={jobId} onCreated={p => loadPrestart(p)} onCancel={() => setView('list')} />
            </div>}

          {/* Form view */}
          {view === 'form' && prestart && <>
              {/* Status bar */}
              <div className={cn('flex items-center justify-between px-4 py-2.5 rounded-xl', prestart.status === 'finalised' ? 'bg-green-50 border border-green-200' : 'bg-amber-50 border border-amber-200')}>
                <div className="flex items-center gap-2">
                  {prestart.status === 'finalised' ? <CheckCircle2 size={16} className="text-green-600" /> : <Clock size={16} className="text-amber-600" />}
                  <span className="text-sm font-medium capitalize text-slate-700">{prestart.status}</span>
                </div>
                {prestart.status !== 'finalised' && <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => setView('signon')} className="h-8 rounded-lg text-xs">
                      <Users size={13} className="mr-1" />
                      Sign-on ({workers.length})
                    </Button>
                    <Button size="sm" onClick={() => setShowFinaliseConfirm(true)} className="h-8 rounded-lg text-xs bg-green-600 hover:bg-green-700 text-white">
                      Finalise
                    </Button>
                  </div>}
                {prestart.status === 'finalised' && <div className="flex gap-2">
              ...
              <Section title="Job Details" icon={HardHat} defaultOpen accent="bg-violet-500">
                ...
                {/* Job picker button */}
                <Field label="Job">
                  <button type="button" disabled={isReadOnly} onClick={() => setJobPickerOpen(true)} className="w-full min-h-[2.75rem] rounded-xl border border-input bg-background px-3 py-2 text-sm text-left flex items-center justify-between gap-2 hover:border-violet-400 transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
                    <div className="flex-1 min-w-0">
                ...
                {prestart.customer_name && <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-50 border border-slate-200">
              ...
              <Section title="S — Situation" icon={AlertTriangle} accent="bg-red-500">
                ...
                <div className="space-y-2">
                  {job_site_prestart.SITUATION_CHECKS.map(label => <button key={label} type="button" disabled={isReadOnly} onClick={() => !isReadOnly && updateChecklist('situation_checkboxes', label, !(prestart.situation_checkboxes as Record<string, boolean>)?.[label])} className={cn('w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border text-left transition-colors', (prestart.situation_checkboxes as Record<string, boolean>)?.[label] ? 'bg-green-50 border-green-300' : 'bg-slate-50 border-slate-200', isReadOnly && 'opacity-50 cursor-not-allowed')}>
              ...
              <Section title="E — Execution" icon={Shield} accent="bg-purple-500">
                ...
                <div className="space-y-2">
                  {job_site_prestart.EXECUTION_CHECKS.map(label => <button key={label} type="button" disabled={isReadOnly} onClick={() => !isReadOnly && updateChecklist('execution_checklist', label, !(prestart.execution_checklist as Record<string, boolean>)?.[label])} className={cn('w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border text-left transition-colors', (prestart.execution_checklist as Record<string, boolean>)?.[label] ? 'bg-green-50 border-green-300' : 'bg-slate-50 border-slate-200', isReadOnly && 'opacity-50 cursor-not-allowed')}>
              ...
              <Section title="A — Administration" icon={Wrench} accent="bg-teal-500">
                ...
                <div className="space-y-2">
                  {job_site_prestart.ADMIN_CHECKS.map(label => <button key={label} type="button" disabled={isReadOnly} onClick={() => !isReadOnly && updateChecklist('admin_checklist', label, !(prestart.admin_checklist as Record<string, boolean>)?.[label])} className={cn('w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border text-left transition-colors', (prestart.admin_checklist as Record<string, boolean>)?.[label] ? 'bg-green-50 border-green-300' : 'bg-slate-50 border-slate-200', isReadOnly && 'opacity-50 cursor-not-allowed')}>
              ...
              <Section title="C — Command / Communications" icon={Phone} accent="bg-slate-600">
                ...
                <CheckRow label="Assembly point confirmed with crew" checked={!!prestart.assembly_point_confirmed} onChange={v => update('assembly_point_confirmed', v)} disabled={isReadOnly} />
                <CheckRow label="Stop-work authority confirmed with crew" checked={!!prestart.stop_work_authority_confirmed} onChange={v => update('stop_work_authority_confirmed', v)} disabled={isReadOnly} />
              </Section>
              ...
              <Section title="Relevant SWMS" icon={FileText} accent="bg-indigo-500" badge={`${Array.isArray(prestart.relevant_swms_ids) ? prestart.relevant_swms_ids.length : 0} selected`}>
                ...
                {jobSwms.length === 0 ? <div className="flex items-center gap-2 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                  ...
                  return <button key={s.id} disabled={isReadOnly} onClick={() => toggleSwms(s.id)} className={cn('w-full flex items-center gap-3 p-3 rounded-xl border text-left transition-colors', selected ? 'bg-indigo-50 border-indigo-300' : 'bg-slate-50 border-slate-200', isReadOnly && 'cursor-default')}>
                ...
                <CheckRow label="SWMS reviewed and understood by crew" checked={!!prestart.swms_reviewed_confirmed} onChange={v => update('swms_reviewed_confirmed', v)} disabled={isReadOnly} />
                ...
                <CheckRow label="No SWMS required today" checked={!!prestart.no_swms_required} onChange={v => update('no_swms_required', v)} disabled={isReadOnly} />
                {prestart.no_swms_required && <Field label="Reason (required)">
              ...
              <Section title={`Worker Sign-On (${workers.length})`} icon={Users} accent="bg-violet-500">
                <Button onClick={() => setView('signon')} className="w-full bg-violet-500 hover:bg-violet-700 text-white rounded-xl h-11">
                  <Users size={16} className="mr-2" />
                  {isReadOnly ? 'View Sign-On Register' : 'Open Sign-On Screen'}
                </Button>
                {workers.length > 0 && <div className="space-y-2 mt-2">
                    {/* Key role highlights */}
                ...
                return match ? <div key={role} className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border ${badge.bg} border-transparent`}>
                    ...
                    {workers.map(w => {
                const badge = ROLE_BADGE[w.role_trade];
                return <div key={w.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-200">
                          <CheckCircle2 size={15} className="text-green-500 flex-shrink-0" />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-slate-800 truncate">{w.full_name}</p>
                            <div className="flex items-center gap-1.5 mt-0.5">
                              {badge ? <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full ${badge.bg} ${badge.text}`}>{badge.label}</span> : <span className="text-xs text-slate-400">{w.role_trade || 'Unspecified'}</span>}
                            </div>
                          </div>
                          {!w.fit_for_work && <Badge variant="destructive" className="text-xs">Not fit</Badge>}
                        </div>;
              ...
              paddingBottom: 'max(1.25rem, env(safe-area-inset-bottom))'
                  ...
                  setFinaliseError('');
                      ...
                      <Button onClick={finalise} disabled={finalising} className="flex-1 bg-green-600 hover:bg-green-700 text-white rounded-xl">
      ...
      {/* Job picker */}
      <JobPickerSheet open={jobPickerOpen} onClose={() => setJobPickerOpen(false)} title="Pick a Job" subtitle="Select the job for this prestart" iconBg="bg-violet-100" iconFg="text-violet-700" Icon={HardHat} onSelect={job => {
