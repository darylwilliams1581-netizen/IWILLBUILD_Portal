...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetchFleetFlags().then(flags => setFleetFlags(flags)).catch(() => {
      setFleetError(true);
    });
    fetch('/api/dashboard/todos').then(r => r.json()).then(d => {
      setDueTodayTodos(d.dueToday ?? []);
      setOverdueTodos(d.overdue ?? []);
  ...
  return <div className="portal-page">
      ...
      <div className="portal-main lg-portal">
        ...
        <div className="hidden lg:flex items-center justify-between px-6 py-4 shrink-0 print:hidden" style={{
        ...
        boxShadow: '0 2px 16px rgba(0,0,0,0.25)'
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="font-extrabold text-[20px] leading-tight tracking-[-0.025em]" style={{
        ...
        <main className="flex-1 overflow-y-auto p-2 md:p-3">
          ...
          {usageWarning && <motion.div initial={{
          opacity: 0,
          y: -6
        }} animate={{
          opacity: 1,
          y: 0
        }} className={`mb-2 flex items-start gap-2 rounded-lg border px-3 py-2 ${usageWarning.hasBlocked ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'}`}>
              {usageWarning.hasBlocked ? <XCircle size={14} className="text-red-600 shrink-0 mt-0.5" /> : <AlertTriangle size={14} className="text-amber-600 shrink-0 mt-0.5" />}
              <div className="flex-1 min-w-0">
              ...
              <Link to="/settings?tab=data" className={`shrink-0 flex items-center gap-1 text-[11px] font-bold px-2.5 py-1.5 rounded-lg transition-colors ${usageWarning.hasBlocked ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-amber-600 hover:bg-amber-700 text-white'}`}>
                <BarChart3 size={10} />
                Usage
              </Link>
            </motion.div>}
          ...
          {/* ── Fleet Flags ── */}
          {fleetFlags && fleetFlags.totalFlags > 0 && <motion.div variants={itemVariants} initial="hidden" animate="visible" className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex flex-col gap-2 mb-2">
              ...
              <div className="flex flex-col gap-1.5">
                {fleetFlags.attentionFlags.map(f => <Link key={`att-${f.assetId}`} to={`/fleet/${f.assetId}`} className="flex items-start gap-2 bg-white border border-amber-200 rounded-lg px-3 py-2 hover:border-amber-400 transition-colors group">
                ...
                {fleetFlags.dueDateFlags.map(f => <Link key={`due-${f.assetId}-${f.type}`} to={`/fleet/${f.assetId}`} className="flex items-start gap-2 bg-white border border-amber-200 rounded-lg px-3 py-2 hover:border-amber-400 transition-colors group">
                    {f.type === 'service' ? <Wrench size={12} className="text-amber-500 shrink-0 mt-0.5" /> : <Calendar size={12} className="text-amber-500 shrink-0 mt-0.5" />}
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold text-amber-800">
                        {f.assetName} — {f.type === 'service' ? 'Service' : 'Rego'} due{' '}
                        {new Date(f.dueDate) < new Date() ? 'overdue' : `${new Date(f.dueDate).toLocaleDateString('en-AU', {
                    day: 'numeric',
                    month: 'short'
                  })}`}
                      </p>
          ...
          {fleetError && !fleetFlags && <div className="flex items-center gap-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded-lg mb-2 text-[11px] text-amber-700">
              <AlertTriangle size={12} className="shrink-0" />
              Fleet status unavailable — check connection.
            </div>}

          {/* ── To-do Alerts ── */}
          {(overdueTodos.length > 0 || dueTodayTodos.length > 0) && <motion.div variants={itemVariants} initial="hidden" animate="visible" className="mb-2 flex flex-col gap-2">
              {overdueTodos.length > 0 && <div className="bg-red-50 border border-red-200 rounded-lg p-3 flex flex-col gap-1.5">
                  <div className="flex items-center gap-2">
                  ...
                  {overdueTodos.map(t => <Link key={t.id} to={`/jobs/${t.jobId}?tab=todos`} className="flex items-start gap-2 bg-white border border-red-200 rounded-lg px-3 py-2 hover:border-red-400 transition-colors group">
              ...
              {dueTodayTodos.length > 0 && <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex flex-col gap-1.5">
                  <div className="flex items-center gap-2">
                  ...
                  {dueTodayTodos.map(t => <Link key={t.id} to={`/jobs/${t.jobId}?tab=todos`} className="flex items-start gap-2 bg-white border border-amber-200 rounded-lg px-3 py-2 hover:border-amber-400 transition-colors group">
          ...
          <motion.div variants={itemVariants} initial="hidden" animate="visible" className="bg-white rounded-lg border border-border">
            ...
            {!jobsLoaded ? <div className="divide-y divide-border">
                {Array.from({
              length: 5
            }).map((_, i) => <div key={i} className="flex items-center justify-between px-4 py-2.5 gap-3">
              ...
              </div> : jobsError ? <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
              ...
              fetchJobs().then(data => {
              ...
              </div> : recentJobs.length === 0 ? <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
                ...
                <Link to="/jobs" className="inline-flex items-center gap-1.5 bg-primary hover:bg-violet-700 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors duration-150">
                  <Plus size={12} />
                  {addWorkLabel}
                </Link>
              ...
              const sm = statusMeta[job.status?.toLowerCase() ?? ''] ?? {
                dot: 'bg-gray-300',
                label: job.status ?? ''
              };
              return <Link key={job.id} to={`/jobs/${job.id}`} className="flex items-center gap-3 px-4 py-2.5 hover:bg-muted/40 transition-colors group">
                      {/* Status dot */}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          {job.jobNumber && <span className="text-[10px] font-mono text-muted-foreground bg-muted px-1.5 py-0.5 rounded">{job.jobNumber}</span>}
                          {job.client && <span className="text-[11px] text-muted-foreground truncate">{job.client}</span>}
                        </div>
                      </div>
      ...
      {/* Start Driving modal */}
      <AnimatePresence>
        {showStartDriving && <StartDrivingModal onClose={() => setShowStartDriving(false)} onStarted={handleSessionStarted} />}
      </AnimatePresence>
    </div>;
}
