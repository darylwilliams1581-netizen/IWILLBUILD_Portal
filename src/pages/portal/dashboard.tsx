...
import { Briefcase, FileText, Receipt, AlertCircle, Loader2, ChevronRight, Building2, LogOut, CheckCircle, Clock, DollarSign, HardHat } from 'lucide-react';
interface PortalJob {
  id: number;
  job_number: string;
  name: string;
...
const JOB_STATUS: Record<string, {
  label: string;
  color: string;
  bg: string;
}> = {
...
export default function PortalDashboardPage() {
  ...
  const loadJobs = useCallback(async () => {
    if (!token) {
      setError('No access token. Please use your invite link.');
      setLoading(false);
      return;
    }
    setLoading(true);
  ...
  return <>
      <Helmet>
        <title>My Portal — IWIIlBUILD</title>
        <meta name="description" content="View your jobs, estimates, and invoices in the IWIIlBUILD client portal." />
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <header className="bg-white border-b border-slate-200 sticky top-0 z-20 safe-top">
          <div className="max-w-4xl mx-auto px-4 py-4 flex items-center gap-3">
            ...
            <button onClick={handleLogout} className="flex items-center gap-1.5 text-xs text-slate-600 hover:text-slate-800 transition-colors px-3 py-1.5 rounded-lg hover:bg-slate-100">
        ...
        <main className="max-w-4xl mx-auto px-4 py-8">
          ...
          {/* Summary cards */}
          {!loading && !error && <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              {[{
            label: 'Active jobs',
            ...
            label: 'Outstanding',
            value: totalOutstanding > 0 ? fmtMoney(totalOutstanding) ?? '$0' : '$0',
            icon: DollarSign,
          ...
          <div>
            ...
            {loading && <div className="flex items-center justify-center gap-2 text-slate-400 py-16">
                <Loader2 size={20} className="animate-spin" />
                <span className="text-sm">Loading your jobs…</span>
              </div>}

            {error && <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-2xl p-4 text-red-700">
                <AlertCircle size={18} />
                <p className="text-sm">{error}</p>
              </div>}

            {!loading && !error && jobs.length === 0 && <div className="text-center py-16 text-slate-400">
            ...
            <div className="flex flex-col gap-3">
              ...
              return <motion.div key={job.id} initial={{
                opacity: 0,
                y: 8
              }} animate={{
                opacity: 1,
                ...
                delay: i * 0.04
                    ...
                    <Link to={`/portal/jobs/${job.id}?token=${token}`} className="block bg-white rounded-2xl border border-slate-200 p-5 hover:border-violet-300 hover:shadow-sm transition-all group">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-2 flex-wrap">
                            {job.pending_estimates > 0 && <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200">
                                <Clock size={9} /> {job.pending_estimates} estimate{job.pending_estimates > 1 ? 's' : ''} to review
                              </span>}
                            {job.outstanding_amount != null && job.outstanding_amount > 0 && <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-violet-700 bg-violet-50 px-2 py-0.5 rounded-full border border-violet-200">
                                <Receipt size={9} /> {fmtMoney(job.outstanding_amount)} outstanding
                              </span>}
                            {job.approved_estimates > 0 && job.pending_estimates === 0 && <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">
