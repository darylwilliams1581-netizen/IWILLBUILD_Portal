...
import {
  HardHat, FileText, Receipt, ChevronLeft, Loader2, AlertCircle,
  CheckCircle, XCircle, Clock, DollarSign, Check, X, CreditCard,
...
const EST_STATUS: Record<string, { label: string; color: string; icon: React.ElementType }> = {
  pending:   { label: 'Awaiting your review', color: 'text-amber-600',   icon: Clock },
  approved:  { label: 'Approved',             color: 'text-emerald-600', icon: CheckCircle },
  declined:  { label: 'Declined',             color: 'text-red-500',     icon: XCircle },
  sent:      { label: 'Sent',                 color: 'text-blue-600',    icon: FileText },
};
...
export default function PortalJobDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/portal/jobs/${id}?token=${encodeURIComponent(token)}`)
      .then(r => r.json() as Promise<{ job?: PortalJob; estimates?: PortalEstimate[]; invoices?: PortalInvoice[]; error?: string }>)
      .then(data => {
  ...
  async function handleEstimateAction(estId: number, action: 'approve' | 'decline') {
    ...
    try {
      const res = await fetch(`/api/portal/estimates/${estId}/approve`, {
        ...
        body: JSON.stringify({ token, action, notes: actionNotes || undefined }),
      });
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {job.description && (
                    <p className="text-sm text-slate-500 mt-2 leading-relaxed">{job.description}</p>
                  )}
                </div>
              </div>
            </motion.div>
            ...
            {estimates.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {estimates.map((est, i) => {
                  ...
                  return (
                    <motion.div key={est.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                      ...
                      {/* Action buttons */}
                      {canAct && actionEstId !== est.id && (
                        <div className="flex gap-2">
                          <button
                            onClick={() => setActionEstId(est.id)}
                            className="flex-1 py-2 rounded-xl bg-emerald-500 text-white text-sm font-semibold hover:bg-emerald-600 transition-colors flex items-center justify-center gap-1.5"
                          >
                            ...
                            <div className="pt-3 border-t border-slate-100 mt-3">
                              ...
                              <div className="flex gap-2">
                                <button
                                  ...
                                  className="flex-1 py-2 rounded-xl bg-emerald-500 text-white text-sm font-semibold hover:bg-emerald-600 disabled:opacity-50 transition-colors flex items-center justify-center gap-1.5"
                                >
                                ...
                                <button
                                  ...
                                  disabled={actionSaving}
                                  className="flex-1 py-2 rounded-xl border border-red-200 text-red-500 text-sm font-semibold hover:bg-red-50 disabled:opacity-50 transition-colors flex items-center justify-center gap-1.5"
                                >
                                  {actionSaving ? <Loader2 size={13} className="animate-spin" /> : <X size={13} />}
                                  Confirm Decline
                                </button>
                                <button onClick={() => { setActionEstId(null); setActionNotes(''); }}
            ...
            {invoices.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {invoices.map((inv, i) => {
                  ...
                  return (
                    <motion.div key={inv.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                      className="bg-white rounded-2xl border border-slate-200 p-5">
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div>
                      ...
                      {canPay && (
                        ...
                        >
                          {payingId === inv.id
                            ...
                            : <><CreditCard size={14} /> Pay {fmtMoney(inv.total_inc_gst)}</>
                          }
                        </button>
                      )}
