...
import {
  ArrowLeft, ClipboardCheck, Loader2, Car, ChevronRight,
  CheckCircle2, XCircle, AlertTriangle, AlertCircle, Play,
  X,
} from 'lucide-react';
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                ...
                {v.rego && !v.rego_not_applicable && (
                  <span className="text-gray-500 text-xs bg-gray-100 px-1.5 py-0.5 rounded font-mono">{v.rego}</span>
                )}
                {v.current_driver && (
                  <span className="text-amber-600 text-xs font-semibold">In use · {v.current_driver}</span>
                )}
              </div>
            </div>
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 bg-violet-50 border border-violet-200 rounded-2xl px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {vehicle.rego && !vehicle.rego_not_applicable && (
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className={`flex items-center justify-center gap-2 rounded-xl py-3.5 border font-semibold text-sm transition-colors ${
              form.safeToOperate
      ...
      {form.safeToOperate && (
        <div>
          ...
          <button
            ...
            className={`flex items-center gap-2.5 w-full rounded-xl px-4 py-3.5 border text-sm font-semibold transition-colors ${
...
function DoneState({ vehicle, onAnother }: { vehicle: Vehicle; onAnother: () => void }) {
  ...
  const loadVehicles = useCallback(async () => {
    ...
    try {
      ...
      if (vehicleIdParam) {
        ...
        if (found) { setSelectedVehicle(found); setStep('form'); }
      }
    } catch { setVehicles([]); }
        ...
        <div className="max-w-lg mx-auto px-4 py-5" style={{ paddingBottom: 'max(3rem, env(safe-area-inset-bottom))' }}>
          <AnimatePresence mode="wait">
            ...
            {step === 'done' && selectedVehicle && (
              <motion.div key="done" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }} transition={{ duration: 0.18 }}>
