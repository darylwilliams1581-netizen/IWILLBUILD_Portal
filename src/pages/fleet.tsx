        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <input type="text" value={form.name} onChange={e => set('name', e.target.value)} placeholder="e.g. Toyota HiLux SR5 — White" className="w-full px-3 py-2.5 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors" />
            </div>

            <div>
              <label className="block text-xs font-semibold mb-1.5">Asset Number</label>
              <input type="text" value={form.assetNumber} onChange={e => set('assetNumber', e.target.value)} placeholder="e.g. FLT-001" className="w-full px-3 py-2.5 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors" />
            </div>
            ...
            <div className="col-span-2">
              ...
              <input type="text" value={form.makeModel} onChange={e => set('makeModel', e.target.value)} placeholder="e.g. Toyota HiLux SR5 2022" className="w-full px-3 py-2.5 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors" />
            </div>

            <div className="col-span-2">
              <label className="block text-xs font-semibold mb-1.5">VIN</label>
              <input type="text" value={form.vin ?? ''} onChange={e => set('vin', e.target.value)} placeholder="e.g. 1HGBH41JXMN109186" className="w-full px-3 py-2.5 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors" />
            </div>

            <div>
              <label className="block text-xs font-semibold mb-1.5">Rego Number</label>
              <input type="text" value={form.rego} onChange={e => set('rego', e.target.value)} disabled={form.regoNotApplicable} placeholder="e.g. 123 ABC" className="w-full px-3 py-2.5 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors disabled:opacity-40 disabled:bg-slate-50" />
            </div>
...
export default function FleetPage() {
  ...
  const filtered = assets.filter(a => {
    ...
    return a.name.toLowerCase().includes(q) || (a.assetNumber ?? '').toLowerCase().includes(q) || (a.rego ?? '').toLowerCase().includes(q) || a.type.toLowerCase().includes(q) || (a.makeModel ?? '').toLowerCase().includes(q);
  });
  const counts = {
      ...
      <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
        ...
        <header className="sticky top-0 z-30 bg-white border-b border-border shrink-0 safe-top">
          <div className="flex items-center gap-2 px-3 h-12 min-w-0">
            ...
            {!loading && <span className="text-xs bg-slate-100 text-slate-500 font-semibold px-1.5 py-0.5 rounded-full shrink-0">
                {assets.length}
              </span>}
            {attentionCount > 0 && <span className="hidden sm:flex items-center gap-1 text-xs bg-amber-100 text-amber-700 font-bold px-1.5 py-0.5 rounded-full shrink-0">
            ...
            <div className="shrink-0">
              {view === 'assets' ? <button onClick={() => !isViewOnly && setShowModal(true)} disabled={isViewOnly} title={isViewOnly ? 'Subscribe to continue' : 'Add asset'} className="flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-colors bg-primary text-white hover:bg-violet-700 disabled:opacity-50 disabled:cursor-not-allowed">
                  <Plus size={14} />
        ...
        <div className="flex-1 overflow-hidden flex flex-col min-h-0">
          ...
          {/* ── Assets view ── */}
          {view === 'assets' && <div className="flex-1 overflow-y-auto p-4 md:p-6 pb-28 flex flex-col gap-5">

          {/* Success banner */}
          <AnimatePresence>
            {successName && <motion.div initial={{
                opacity: 0,
                y: -8
              }} animate={{
                opacity: 1,
          ...
          {/* Error */}
          {error && <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-lg px-4 py-3 text-sm text-red-700">
          ...
          {!loading && <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                }].map(s => <div key={s.label} className={`${s.bg} rounded-xl p-4 border`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setShowDriveModal(true)} className="flex flex-col items-center justify-center gap-2 rounded-2xl px-4 py-5 bg-blue-500 hover:bg-blue-600 text-white font-bold transition-colors active:scale-95">
                ...
                <button onClick={() => navigate('/prestart')} className="flex flex-col items-center justify-center gap-2 rounded-2xl px-4 py-5 bg-amber-500 hover:bg-amber-600 text-white font-bold transition-colors active:scale-95">
                  <div className="w-11 h-11 rounded-2xl bg-white/20 flex items-center justify-center">
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Search name, rego, type…" value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors" />
              </div>

              {/* Empty state */}
              {assets.length === 0 && <div className="flex flex-col items-center justify-center py-20 text-center">
                  ...
                  <button onClick={() => !isViewOnly && setShowModal(true)} disabled={isViewOnly} title={isViewOnly ? 'Subscribe to continue' : undefined} className="flex items-center gap-2 bg-primary hover:bg-violet-700 text-white text-sm font-bold px-5 py-2.5 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    <Plus size={15} />
                    Add First Asset
                  </button>
                </div>}
              ...
              {/* No results */}
              {assets.length > 0 && filtered.length === 0 && <div className="text-center py-16 text-slate-400 text-sm">No assets match your search.</div>}

              {/* Asset list */}
              {filtered.length > 0 && <motion.div variants={stagger} initial="hidden" animate="visible" className="flex flex-col gap-3">
                  {filtered.map(asset => {
                  const style = getAssetStatusStyle(asset.status);
                  return <motion.div key={asset.id} variants={fadeUp}>
                        <Link to={`/fleet/${asset.id}`} className="block bg-white border border-slate-200 rounded-xl p-5 hover:border-primary/40 hover:shadow-md transition-all duration-150 group">
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 flex-wrap mb-1">
                                {asset.assetNumber && <span className="text-xs font-mono text-slate-400">{asset.assetNumber}</span>}
                              ...
                              <div className="flex items-center gap-4 mt-1 text-xs text-slate-500 flex-wrap">
                                ...
                                {!asset.regoNotApplicable && asset.rego && <span>Rego: {asset.rego}</span>}
                                {asset.regoNotApplicable && <span className="text-slate-400">Rego N/A</span>}
                              </div>
                            </div>
                            <div className="flex flex-col items-end gap-1 shrink-0">
                              {asset.serviceDate && <span className="text-xs text-slate-400">
                                  Service: {new Date(asset.serviceDate).toLocaleDateString('en-AU', {
                              day: 'numeric',
                              month: 'short',
                              year: 'numeric'
                            })}
                                </span>}
                              {!asset.regoNotApplicable && asset.regoExpiry && <span className="text-xs text-slate-400">
                                  Rego exp: {new Date(asset.regoExpiry).toLocaleDateString('en-AU', {
                              day: 'numeric',
                              month: 'short',
                              year: 'numeric'
                            })}
        ...
        {view === 'assets' && <div className="shrink-0 bg-white/95 backdrop-blur-sm border-t border-slate-200" style={{
          paddingBottom: 'max(env(safe-area-inset-bottom), 12px)'
        }}>
            <div className="scroll-x-hide flex items-center gap-2 px-4 pt-3 pb-1">
              {FILTERS.map(f => <button key={f.value} onClick={() => setFilter(f.value)} className={`text-xs font-bold px-3.5 py-2 rounded-full border transition-colors whitespace-nowrap ${filter === f.value ? 'bg-primary text-white border-primary shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:border-primary hover:text-primary'}`}>
                  {f.label}
                </button>)}
            </div>
          </div>}
        ...
        {/* New Asset Modal */}
        <AnimatePresence>
          {showModal && <NewAssetModal onClose={() => setShowModal(false)} onCreated={asset => {
        ...
        {/* Drive Fleet Modal */}
        <AnimatePresence>
          {showDriveModal && <StartDrivingModal onClose={() => setShowDriveModal(false)} onStarted={() => {
