...
import {
  ...
  RotateCcw, RotateCw, Loader2, AlertCircle, Lock, Map,
} from 'lucide-react';
...
export default function PlanManagerSharePage() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`/api/plan-manager/share/validate?token=${token}`)
      .then(r => r.json() as Promise<ShareData & { error?: string }>)
      .then(d => {
  ...
  const annotationsByPage = useCallback((pageNo: number): Annotation[] => {
    ...
    return data.annotations
      .filter(a => Number(a.page_no) === pageNo)
      .map(a => ({
        id: String(a.id),
        dbId: a.id as number,
    ...
    <>
      ...
      <div className="flex flex-col h-screen bg-slate-950 text-slate-100">
        ...
        <div className="flex items-center gap-3 px-4 py-3 bg-slate-900 border-b border-slate-700 flex-shrink-0">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage <= 1}
              className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-100 hover:bg-slate-700 disabled:opacity-30 transition-colors">
              <ChevronLeft size={16} />
            </button>
            ...
            <button onClick={() => setScale(s => nextScale(s, -1))}
            ...
            <div className="relative inline-block shadow-2xl">
              ...
              {scaledW > 0 && scaledH > 0 && (
                <AnnotationCanvas
                  ...
                  activeTool="select"
                  activeStyle={{ color: '#ef4444', strokeWidth: 2, opacity: 1 }}
                  isLocked={true}
                  onAnnotationsChange={() => {}}
                />
              )}
        ...
        <div className="px-4 py-2 bg-slate-900 border-t border-slate-700 text-[10px] text-slate-600 text-center flex-shrink-0">
          Shared via IWILLBUILD Plan Manager · View only · Expires {new Date(data.expiresAt).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' })}
        </div>
      </div>
    </>
  );
