...
interface LocalRecipeLine extends RecipeLine {
  _key: string;
}
...
function CostGuidePickerModal({
  ...
  useEffect(() => {
    fetch('/api/cost-guide', {
      credentials: 'include'
    }).then(r => r.json() as Promise<{
      items?: CostItem[];
    }>).then(d => setItems(d.items ?? [])).finally(() => setLoading(false));
  }, []);
  const filtered = items.filter(i => !search || i.description.toLowerCase().includes(search.toLowerCase())).sort((a, b) => a.description.toLowerCase().localeCompare(b.description.toLowerCase()));
  return <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[70vh]">
        ...
        <div className="px-4 py-2.5 border-b border-slate-100 shrink-0">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search cost items…" className="w-full pl-8 pr-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" autoFocus />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {loading && <div className="flex justify-center py-8"><Loader2 size={18} className="animate-spin text-primary" /></div>}
          {!loading && filtered.length === 0 && <div className="text-center py-10 text-slate-400 text-sm">
              {search ? 'No items match your search' : 'No cost items in your guide yet'}
            </div>}
          {!loading && filtered.map(item => <button key={item.id} onClick={() => {
          onInsert(item);
          onClose();
...
function RecipeModal({
  ...
  const [lines, setLines] = useState<LocalRecipeLine[]>(initial?.lines?.length ? initial.lines.map(l => ({
    ...l,
    _key: rk(),
    unit: l.unit ?? null
  })) : [blankRLine()]);
  ...
  function deleteLine(key: string) {
    setLines(prev => {
      const n = prev.filter(l => l._key !== key);
      return n.length ? n : [blankRLine()];
    });
  }
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-1.5">
                  <button type="button" onClick={() => setShowCostPicker(true)} className="flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-primary hover:bg-violet-50 px-2 py-1 rounded-lg transition-colors border border-slate-200 hover:border-primary/30">
                    <Calculator size={11} />Cost Guide
                  </button>
                  <button type="button" onClick={addLine} disabled={lines.length >= LIMITS.RECIPE_LINES} title={lines.length >= LIMITS.RECIPE_LINES ? `Recipe limit reached (${LIMITS.RECIPE_LINES} lines)` : undefined} className="flex items-center gap-1 text-xs font-bold text-primary hover:bg-violet-50 disabled:opacity-40 disabled:cursor-not-allowed px-2 py-1 rounded-lg transition-colors">
                    <Plus size={12} />Add Line
                  </button>
                </div>
              </div>
              {lines.length >= LIMITS.RECIPE_LINES && <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-xs text-amber-800 mb-2">
      ...
      {showCostPicker && <CostGuidePickerModal onInsert={insertCostItem} onClose={() => setShowCostPicker(false)} />}
    </div>;
}
...
export function CostGuideTab() {
  ...
  async function handleExportCsv() {
    ...
    try {
      const res = await fetch('/api/cost-guide/export-csv', {
        credentials: 'include'
      });
      if (!res.ok) {
        alert('Export failed');
  ...
  return <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {/* CSV actions */}
          <button onClick={downloadTemplate} className="flex items-center gap-1.5 text-xs font-semibold border border-slate-200 text-slate-600 hover:bg-slate-50 px-3 py-2 rounded-lg transition-colors" title="Download CSV template">
            <FileText size={13} />Template
          </button>
          <button onClick={() => setShowImportModal(true)} className="flex items-center gap-1.5 text-xs font-semibold border border-slate-200 text-slate-600 hover:bg-slate-50 px-3 py-2 rounded-lg transition-colors">
            <Upload size={13} />Import CSV
          </button>
          <button onClick={handleExportCsv} disabled={exportingCsv || items.length === 0} className="flex items-center gap-1.5 text-xs font-semibold border border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed px-3 py-2 rounded-lg transition-colors">
            {exportingCsv ? <Loader2 size={13} className="animate-spin" /> : <Download size={13} />}Export CSV
          </button>
          <button onClick={() => {
          setEditing(undefined);
          setShowModal(true);
        }} disabled={items.length >= 200} title={items.length >= 200 ? 'Cost Guide limit reached (200 items). Delete unused items to add more.' : undefined} className="flex items-center gap-1.5 text-sm font-bold bg-primary hover:bg-violet-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-4 py-2 rounded-lg transition-colors">
            <Plus size={14} />Cost Item
          </button>
        </div>
      </div>

      {items.length >= 200 && <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2.5 text-sm text-amber-800">
      ...
      {!loading && !error && <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          {filtered.length === 0 ? <div className="text-center py-16 text-slate-400">
      ...
      {showImportModal && <CsvImportModal title="Import Cost Guide CSV" uploadUrl="/api/cost-guide/import-csv" showDuplicateOption onSuccess={() => {
      load();
...
export function RecipesTab() {
  ...
  return <div className="flex flex-col gap-4">
      ...
      {!loading && !error && recipes.length === 0 && <div className="bg-white rounded-xl border border-slate-200 text-center py-16 text-slate-400">
      ...
      {!loading && !error && recipes.length > 0 && <div className="flex flex-col gap-2">
          {recipes.map(recipe => {
        ...
        return <div key={recipe.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                ...
                {isOpen && recipe.lines.length > 0 && <div className="border-t border-slate-100 overflow-x-auto">
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {recipe.lines.map((l, i) => <tr key={i} className="border-t border-slate-50">
      ...
      {showModal && <RecipeModal initial={editing} onSave={handleSave} onClose={() => {
      setShowModal(false);
      setEditing(undefined);
    }} />}
    </div>;
...
export default function EstimatingPage() {
  ...
  const [tab, setTab] = useState<Tab>(tabFromUrl && VALID_TABS.includes(tabFromUrl) ? tabFromUrl : 'cost-guide');
  useEffect(() => {
  ...
  return <div className="portal-page">
      ...
      <div className="portal-main">
        ...
        <div className="bg-white border-b border-slate-200 px-4 md:px-6 flex gap-1 shrink-0 overflow-x-auto">
          ...
          key: 'recipes',
          label: 'Recipes'
        }] as const).map(t => <button key={t.key} onClick={() => handleTabClick(t.key)} className={`px-4 py-3 text-sm font-semibold border-b-2 transition-colors whitespace-nowrap ${tab === t.key ? 'border-primary text-primary' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
              {t.label}
            </button>)}
        </div>
