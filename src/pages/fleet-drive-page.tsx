  ...
  return <motion.div initial={{
    ...
    opacity: 0
      ...
      boxShadow: '0 -4px 32px rgba(0,0,0,0.14)'
        ...
        {/* Body */}
        {done ? <div className="flex-1 flex flex-col items-center justify-center gap-3 px-5 py-12">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              ...
              <div>
                ...
                <input type="text" value={driverName} onChange={e => setDriverName(e.target.value)} placeholder="e.g. Daryl Williams" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-900 text-sm placeholder-gray-400 focus:outline-none focus:border-blue-400 transition-colors" />
              </div>
              ...
              <div>
                ...
                <input type="datetime-local" value={startAt} onChange={e => setStartAt(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-900 text-sm focus:outline-none focus:border-blue-400 transition-colors" />
              </div>
              ...
              <div>
                ...
                <input type="datetime-local" value={endAt} onChange={e => setEndAt(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-900 text-sm focus:outline-none focus:border-blue-400 transition-colors" />
                {startAt && endAt && new Date(endAt) > new Date(startAt) && <p className="text-xs text-gray-400 mt-1.5 flex items-center gap-1">
                    <Clock size={10} />
              ...
              <div>
                ...
                <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Any notes about this trip…" rows={2} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-900 text-sm placeholder-gray-400 focus:outline-none focus:border-blue-400 resize-none transition-colors" />
              </div>
            ...
            <div className="px-5 pb-8 pt-3 border-t border-gray-100 shrink-0">
              <button onClick={() => void handleSave()} disabled={saving} className="w-full flex items-center justify-center gap-2.5 bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white font-bold py-4 rounded-2xl transition-colors shadow-md shadow-blue-200 disabled:opacity-60">
...
export default function FleetDrivePage() {
  ...
  const loadData = () => {
    ...
    Promise.all([fetch(`/api/fleet/${id}`, {
      credentials: 'include'
    }).then(r => r.json() as Promise<{
      asset?: FleetAsset;
    } | FleetAsset>).then(data => {
      const a = data && typeof data === 'object' && 'asset' in data ? data.asset : data as FleetAsset;
      setAsset(a ?? null);
    }), fetch(`/api/fleet/${id}/driver-sessions`, {
      credentials: 'include'
    }).then(r => r.json() as Promise<{
      sessions: DriverSession[];
    }>).then(data => setSessions(data.sessions ?? []))]).catch(() => {}).finally(() => setLoading(false));
  };
  useEffect(() => {
    loadData();
  ...
  const exportCsv = async () => {
    setExporting(true);
    try {
      const res = await fetch(`/api/fleet/${id}/usage-export`, {
        credentials: 'include'
      });
  ...
  return <div className="flex-1 bg-gray-50 flex flex-col overflow-hidden">
      ...
      boxShadow: '0 1px 0 rgba(0,0,0,0.05)'
        ...
        <button onClick={() => goBack(navigate, '/fleet')} className="w-9 h-9 rounded-xl bg-gray-100 flex items-center justify-center text-gray-600 hover:bg-gray-200 transition-colors shrink-0">
          <ArrowLeft size={18} />
        </button>
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setShowLogTrip(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-500 hover:bg-blue-600 text-white text-xs font-bold rounded-lg transition-colors shadow-sm shadow-blue-200">
      ...
      {/* ── Mobile: back arrow ── */}
      <button onClick={() => goBack(navigate, '/fleet')} className="md:hidden fixed top-3 left-3 z-20 w-9 h-9 rounded-xl bg-white/90 backdrop-blur-sm shadow-sm border border-gray-100 flex items-center justify-center text-gray-600 active:bg-gray-100 transition-colors" aria-label="Back">
        <ArrowLeft size={18} />
      </button>
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? <div className="flex items-center justify-center py-20">
          ...
          </div> : <div className="px-4 py-4 pb-32 md:pb-6 max-w-3xl mx-auto w-full space-y-4">

            {/* Active session banner */}
            {activeSession && <motion.div initial={{
          opacity: 0,
          y: -4
        }} animate={{
          opacity: 1,
          y: 0
            ...
            color: 'text-blue-600'
          }].map(c => <div key={c.label} className="bg-white rounded-2xl border border-gray-100 px-3 py-3 text-center" style={{
            ...
            {/* Session list */}
            {sessions.length === 0 ? <div className="bg-white rounded-2xl border border-gray-100 px-6 py-12 text-center" style={{
          boxShadow: '0 1px 4px rgba(0,0,0,0.06)'
        }}>
                ...
                <button onClick={() => setShowLogTrip(true)} className="mt-4 inline-flex items-center gap-2 px-4 py-2.5 bg-blue-500 hover:bg-blue-600 text-white text-sm font-bold rounded-xl transition-colors">
            ...
            return <motion.div key={s.id} initial={{
              ...
              delay: i * 0.025
            }} className="bg-white rounded-2xl border border-gray-100 px-4 py-3.5 flex items-center gap-3" style={{
              boxShadow: '0 1px 4px rgba(0,0,0,0.06)'
            }}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {s.notes && <p className="text-gray-400 text-xs mt-0.5 truncate italic">{s.notes}</p>}
                      </div>
                    </motion.div>;
          })}
              </div>}
      ...
      paddingBottom: 'env(safe-area-inset-bottom)'
        ...
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <button onClick={() => setShowLogTrip(true)} className="flex items-center gap-1.5 bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white font-bold text-sm px-4 py-2.5 rounded-xl transition-colors shadow-sm shadow-blue-200">
      ...
      {/* ── Log Trip Sheet ── */}
      <AnimatePresence>
        {showLogTrip && id && asset && <LogTripSheet assetId={id} assetName={asset.name} onClose={() => setShowLogTrip(false)} onSaved={() => {
        setLoading(true);
        loadData();
      }} />}
      </AnimatePresence>
