...
export default function StudioLibraryPage() {
  ...
  return <div className="flex flex-col flex-1 min-h-0 lg-portal">
      ...
      <DesktopDock />
      <Helmet>
        <title>Library — IWIIlBUILD</title>
        <meta name="description" content="IWIIlBUILD content library — browse and install document and form templates." />
      ...
      <div className="flex border-b border-slate-200 bg-white px-6 gap-1 shrink-0">
        ...
        label,
        icon: Icon
      }) => <button key={key} onClick={() => setLibTab(key)} className={`flex items-center gap-1.5 px-4 py-3 text-sm font-semibold border-b-2 transition-colors ${libTab === key ? 'border-primary text-primary' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
            <Icon size={13} />
            {label}
          </button>)}
      </div>
