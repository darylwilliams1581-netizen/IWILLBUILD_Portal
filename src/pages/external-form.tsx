...
function FieldInput({
  ...
  const options: string[] = (() => {
    ...
    try { return JSON.parse(field.options_json) as string[]; }
    catch { return []; }
  })();
    ...
    return (
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-3">
        ...
        {field.instruction_image_url && (
          <img src={field.instruction_image_url} alt="Instruction" className="mt-2 max-h-48 rounded-lg" />
        )}
      </div>
    );
  }
  ...
  if (field.field_type === 'short_text' || field.field_type === 'link_url') {
    return (
      <input
        type={field.field_type === 'link_url' ? 'url' : 'text'}
        className={base}
        value={String(value ?? '')}
    ...
    return (
      <div className="border border-dashed border-slate-300 rounded-xl p-4 text-center bg-slate-50">
        ...
        <textarea
          ...
          onChange={(e) => onChange(e.target.value)}
          disabled={disabled}
          placeholder="Describe what the photo would show…"
        />
      </div>
    );
  }
...
export default function ExternalFormPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/external/form/${token}`)
      .then(async (r) => {
        const body = await r.json() as { error?: string } & FormData;
  ...
  const pages = (() => {
    if (!data) return [[]];
    const pageBreakIndices = data.fields
      .map((f, i) => ({ f, i }))
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      await fetch(`/api/external/form/${token}`, {
        ...
        body: JSON.stringify({ action: 'save_draft', answers, submitterName, submitterEmail }),
      });
    } catch { /* non-fatal */ }
    setSaving(false);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const r = await fetch(`/api/external/form/${token}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'submit', answers, submitterName, submitterEmail }),
      });
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <main className="max-w-xl mx-auto px-4 py-6">
          ...
          {!loading && data && !submitted && !error && (
            <div className="flex flex-col gap-5">
              {/* Form header */}
              <div className="bg-white border border-slate-200 rounded-2xl p-5">
                <h1 className="text-base font-bold text-slate-800">{data.submission.template_name}</h1>
                {data.submission.template_description && (
                  <p className="text-xs text-slate-500 mt-1">{data.submission.template_description}</p>
                )}
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col gap-5">
                {currentFields.map((field) => (
                  <div key={field.id} className="flex flex-col gap-1.5">
                    {!['section_heading', 'instruction', 'instruction_image', 'page_break', 'checkbox'].includes(field.field_type) && (
                    ...
                    <FieldInput
                      ...
                      disabled={isLocked}
                    />
                    {validationErrors[String(field.id)] && (
                      <p className="text-xs text-red-500">{validationErrors[String(field.id)]}</p>
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="flex items-center gap-3">
                {currentPage > 1 && (
                  <button
                    ...
                    onClick={handleBack}
                    disabled={saving}
                    className="flex items-center gap-1.5 text-sm text-slate-600 border border-slate-200 bg-white hover:bg-slate-50 px-4 py-2.5 rounded-xl font-medium transition-colors disabled:opacity-50"
                  >
                    <ChevronLeft size={15} />
                    Back
                  </button>
                ...
                {currentPage < totalPages ? (
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-sm bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2.5 rounded-xl font-bold transition-colors disabled:opacity-50"
                  >
