 ...
 *
 * EXTERNAL SHARING:
 *  The sharing endpoints (photos/share, job-forms/send-email) require
 *  imageSafeguardAcknowledged: true in the request body when images are
 *  present. This is a user confirmation and audit control, not a
 ...
 *
 * Returns:
 *  { allowed: true, worstStatus }   — sharing permitted (requires confirmation)
...
// ── Server-side image reference resolution ────────────────────────────────────
...
export type SharingAuditAction = 'job_photo_share' | 'form_email';
...
/**
 * Record a simple audit event after a successful acknowledged share.
 *
 * Stores: company ID, user ID, action, resource ID, image count, timestamp.
 * NEVER stores: recipients, image contents, signed URLs, R2 keys.
 *
 * Best-effort — never throws.
 */
export async function recordSharingAuditEvent(
  ...
  try {
    await db.execute(sql`
      ...
      VALUES
        (${event.companyId}, ${event.userId}, ${event.action},
         'image_share', ${event.resourceId},
         ${JSON.stringify({ imageCount: event.imageCount })},
         ${new Date()})
    `);
  } catch (err) {
    // Audit failure must never block the share action
    console.error('[imageSafeguard] recordSharingAuditEvent failed:', err instanceof Error ? err.message : err);
  }
}
