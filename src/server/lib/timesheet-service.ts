...
export async function listTimesheets(params: ListTimesheetsParams): Promise<{
  ...
  const [rows] = await db.execute(sql`
    SELECT
      t.id, t.company_id, t.profile_id, t.job_id, t.week_ending,
      t.status, t.notes, t.submitted_at, t.approved_by_profile_id,
    ...
    WHERE t.company_id = ${params.companyId}
      ${params.profileId && !params.isAdmin ? sql`AND t.profile_id = ${params.profileId}` : sql``}
      ...
      ${params.search ? sql`AND (u.name LIKE ${`%${params.search}%`} OR j.job_number LIKE ${`%${params.search}%`} OR j.name LIKE ${`%${params.search}%`})` : sql``}
...
export async function getTimesheet(
  ...
  const [rows] = await db.execute(sql`
    ...
    WHERE t.id = ${id} AND t.company_id = ${companyId}
      ${!isAdmin ? sql`AND t.profile_id = ${profileId}` : sql``}
    LIMIT 1
  `);
...
export async function createTimesheet(params: {
  ...
  const [result] = await db.execute(sql`
    INSERT INTO timesheets (company_id, profile_id, job_id, week_ending, status, notes)
    VALUES (${params.companyId}, ${params.profileId}, ${params.jobId ?? null}, ${params.weekEnding}, 'draft', ${params.notes ?? null})
  `);
...
export async function updateTimesheet(params: {
  ...
  if (params.weekEnding !== undefined || params.jobId !== undefined || params.notes !== undefined) {
    await db.execute(sql`
