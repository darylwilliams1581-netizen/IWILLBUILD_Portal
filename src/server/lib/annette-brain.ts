...
export type AnswerSource =
  ...
  | 'portal_data'      // IWIIlBUILD DB lookup
...
function extractPortalDataContent(s: string): string {
  // Remove the "📋 From IWIIlBUILD data:\n" prefix if present
  const withoutHeader = s.replace(/^📋 From IWIIlBUILD data:\n/, '');
...
export function formatDazzaAnswer(answer: DazzaAnswer): string {
  ...
  // 1. From IWIIlBUILD data
  if (answer.portalDataSection) {
    sections.push(`📋 From IWIIlBUILD data:\n${answer.portalDataSection}`);
  }
  ...
  //
  //  • Portal data loaded AND AI reasoning used  → "IWIIlBUILD data + AI reasoning\n<modules>"
  ...
  if (answer.modulesUsed.length === 0) {
    ...
    const moduleNames = answer.modulesUsed
      ...
      .join(', ');
    sourceLabel = `IWIIlBUILD data + AI reasoning\n${moduleNames || answer.modulesUsed.join(', ')}`;
...
function parseOpenAIResponse(reply: string): ParsedOpenAIResponse {
  ...
  const sectionPatterns: Array<{ key: keyof ParsedOpenAIResponse; prefix: string }> = [
    { key: 'portalDataSection',  prefix: '📋 From IWIIlBUILD data:' },
...
export async function processDazzaQuestion(
  ...
  if (internalAnswer && internalSource === 'portal_data' && parsed.portalDataSection) {
    ...
    if (internalHasData && openaiSaysNone) {
      ...
      conflictDetail = 'OpenAI suggested no data exists, but IWIIlBUILD portal data was found. Portal data is used as the authoritative source.';
    }
  }
  ...
  const openaiHasStructure = openaiReply.includes('📋 From IWIIlBUILD data:') ||
    ...
    if (/📦\s*Source modules:/i.test(builtReply)) {
      ...
      if (modulesUsed.length === 0) {
        ...
        const moduleNames = modulesUsed
          .filter((m) => m !== 'Settings' && m !== 'Company')
          .join(', ');
        correctSourceLabel = `IWIIlBUILD data + AI reasoning\n${moduleNames || modulesUsed.join(', ')}`;
