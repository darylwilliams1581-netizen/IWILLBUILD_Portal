...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    const { name, abn, phone, email, website, address, industry } = req.body as {
      name?: string;
      abn?: string;
      phone?: string;
      email?: string;
    ...
    // Validate industry — must be a known value or null (falls back to construction)
    const VALID_INDUSTRIES = ['construction','civil','landscaping','fuel_dangerous_goods','plant_hire','general_trades','other'];
    const cleanIndustry = industry && VALID_INDUSTRIES.includes(industry) ? industry : undefined;
    ...
    await db.update(companies)
      .set({
        ...
        ...(cleanIndustry !== undefined ? { industry: cleanIndustry } : {}),
      })
