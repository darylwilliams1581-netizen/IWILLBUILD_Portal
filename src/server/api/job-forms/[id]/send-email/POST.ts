    ...
    if (f.photos && f.photos.length > 0) {
      for (const photo of f.photos) {
        try {
          ...
          // If embedding fails, fall back to text
          ensureSpace(14);
...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    // Cookie header for server-side photo fetching
    ...
    for (const field of fieldRows) {
      ...
      if (field.fieldType === 'photo') {
        if (empty) {
          ...
          // Parse photo URLs from the answer (stored as array or single string)
          ...
          for (const photoUrl of photoUrls) {
            try {
              ...
              if (photoRes.ok) {
                ...
                fetched.push({ bytes: new Uint8Array(buf), mimeType });
              }
            } catch { /* skip failed photo */ }
          }
          photos = fetched.length > 0 ? fetched : undefined;
          display = photos ? `${photos.length} photo${photos.length !== 1 ? 's' : ''} (embedded in PDF)` : `${photoUrls.length} photo${photoUrls.length !== 1 ? 's' : ''} (could not load)`;
        }
