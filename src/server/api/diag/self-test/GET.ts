 ...
 * Platform-owner only — diagnostic self-test for route order, schema, upload pipeline.
...
export default async function handler(req: Request, res: Response) {
  // Platform-owner guard — this endpoint must never be publicly accessible
  ...
  // ── 0. V3 flag diagnostic (presence + resolved value only — never raw value) ─
  {
    ...
    report.dazzaV3Flag = {
      ...
      // secretTrimmedLower intentionally omitted — that is a value, not a status
    };
  }
