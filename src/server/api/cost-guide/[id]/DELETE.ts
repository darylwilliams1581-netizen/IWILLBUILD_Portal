...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    await db.delete(costGuideItems).where(eq(costGuideItems.id, itemId));
    res.json({ ok: true });
  } catch (err) {
    console.error('DELETE /api/cost-guide/:id error:', err);
    res.status(500).json({ error: 'Failed to delete cost guide item' });
  }
}
