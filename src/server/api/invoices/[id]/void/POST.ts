...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    if (rows[0].status === 'void') return res.status(400).json({ error: 'Invoice is already void' });

    await db.execute(
      sql`UPDATE invoices SET status = 'void' WHERE id = ${id} AND company_id = ${profile.companyId}`
    );
