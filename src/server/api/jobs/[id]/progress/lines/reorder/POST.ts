/**
 * POST /api/jobs/:id/progress/lines/reorder
 * Atomically reorder all activities for a job.
 *
 * Body: { ids: number[] }  — complete ordered list of activity IDs.
 *
 * Validation:
 ...
 *   - sort_order is the ONLY ordering mechanism — legacy bulk PUT cannot change it.
 */
...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    // Fetch current activities for this job+company
    const existing = await db
      .select({ id: jobProgressLines.id })
    ...
    if (parsedIds.length !== existingSet.size) {
      return res.status(400).json({ error: 'IDs list length does not match activity count' });
    }
    for (const id of parsedIds) {
      if (!existingSet.has(id)) {
        return res.status(400).json({ error: `Unknown or cross-job activity ID: ${id}` });
      }
    }
    ...
    await db.transaction(async (tx) => {
      for (let i = 0; i < parsedIds.length; i++) {
        await tx.update(jobProgressLines)
          .set({ sortOrder: i + 1 })
          .where(
            and(
              eq(jobProgressLines.id, parsedIds[i]),
    ...
    console.error('POST /api/jobs/:id/progress/lines/reorder error:', err);
