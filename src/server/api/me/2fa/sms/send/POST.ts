 ...
 *
 * Called immediately after a successful password login when the user has
 * SMS 2FA enabled. Generates a 6-digit OTP, stores a hashed copy in
 * sms_verification_codes, and sends it via Twilio.
 *
 * This endpoint is intentionally unauthenticated-session-tolerant: the user
 * has a valid BetterAuth session (password was accepted) but hasn't passed
 * 2FA yet. We identify them by the session cookie.
 *
 * Rate-limited: reuses the existing checkSmsRate (3/IP/10min).
...
export default async function handler(req: Request, res: Response) {
  ...
  try {
    ...
    // Clear any existing 2FA OTP codes for this user (reuse sms_verification_codes table
    // with a purpose discriminator stored in the phone field prefix)
    await db.execute(sql.raw(
      `DELETE FROM sms_verification_codes WHERE user_id = '${userId}' AND phone LIKE '2fa:%'`
    ));
    ...
    await db.execute(sql.raw(
      `INSERT INTO sms_verification_codes (id, user_id, code_hash, phone, expires_at, attempts)
       VALUES ('${id}', '${userId}', '${hashed}', '2fa:${phone}', '${expiresAt.toISOString().slice(0, 19).replace('T', ' ')}', 0)`
    ));
    ...
    console.error('[2fa/sms/send]', err);
