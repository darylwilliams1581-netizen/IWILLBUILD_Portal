 ...
 * Downloads estimate lines as a CSV file.
 */
...
import { estimateLinesToCsv } from '../../../../lib/csv-utils.js';
...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    const csv = estimateLinesToCsv(lines);
    const safeName = (estimate.title ?? 'estimate').replace(/[^a-z0-9]/gi, '-').toLowerCase();
    const filename = `estimate-${safeName}-${new Date().toISOString().slice(0, 10)}.csv`;

    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    res.send(csv);
  } catch (err) {
    console.error('GET /api/estimates/:id/export-csv error:', err);
