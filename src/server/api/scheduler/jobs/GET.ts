  ...
  try {
    ...
    const [rows] = await db.execute(sql`
      SELECT
        ...
        j.scheduled_start_time        AS scheduledStartTime,
        j.scheduled_end_time          AS scheduledEndTime,
    ...
    const jobs = (rows ?? []).map((row) => ({
      ...
      scheduledStartTime:     row.scheduledStartTime     ? String(row.scheduledStartTime).slice(0, 5)      : null,
      scheduledEndTime:       row.scheduledEndTime       ? String(row.scheduledEndTime).slice(0, 5)        : null,
