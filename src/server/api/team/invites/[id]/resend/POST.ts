...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    try {
      ...
      await sendEmail({
        to: invite.email,
        subject: `Reminder: You've been invited to join ${company?.name ?? 'IWIIlBUILD'}`,
        html: `
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
            ...
            <div style="background: #f9f9f9; padding: 24px; border-radius: 0 0 8px 8px; border: 1px solid #e5e7eb;">
              ...
              <p style="color: #444;">This is a reminder that you've been invited to join <strong>${company?.name}</strong> on IWIIlBUILD Portal.</p>
