...
export default async function handler(req: Request, res: Response) {
  ...
  try {
    ...
    await db.execute(sql.raw(`UPDATE am_tender_todos SET ${sets.join(', ')}, updated_at = NOW() WHERE id = ${todoId}`));
    const [rows] = await db.execute(sql.raw(`SELECT * FROM am_tender_todos WHERE id = ${todoId}`)) as unknown as [unknown[], unknown];
    return res.json({ todo: (rows as Record<string, unknown>[])[0] });
  } catch (err) {
    console.error('PUT tender todo error:', err);
