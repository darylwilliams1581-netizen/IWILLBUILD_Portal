 ...
 * followed by ADD COLUMN IF NOT EXISTS guards for every column.
 * This handles the case where the table was created in an earlier partial run
 * and is missing columns added in subsequent schema revisions.
 * Called once at server startup from entry.ts.
 */
import { db } from '../client.js';
import { sql } from 'drizzle-orm';

/** Run a raw SQL statement, ignoring errors (idempotent ALTER TABLE helpers). */
...
export async function runRecoveryEmailMigration(): Promise<void> {
  try {
    ...
    // ── 2. ADD COLUMN IF NOT EXISTS guards for recovery_email_state ───────────
    // Handles environments where the table was created in an earlier partial run.
    // MySQL 8.0+ supports IF NOT EXISTS on ALTER TABLE ADD COLUMN.
    const stateColumns: [string, string][] = [
      ['active_email',            'VARCHAR(255)'],
      ['active_verified_at',      'DATETIME(3)'],
      ['proposed_email',          'VARCHAR(255)'],
      ['proposed_at',             'DATETIME(3)'],
    ...
    for (const [col, type] of stateColumns) {
      await tryExec(
        `ALTER TABLE \`recovery_email_state\` ADD COLUMN IF NOT EXISTS \`${col}\` ${type}`
      );
    }
