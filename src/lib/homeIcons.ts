...
export interface HomeIconDef {
  ...
  ownerOnly?: boolean;
  /** If true: only shown to the IWILLBUILD platform owner */
...
export const MANAGEMENT_ICON_DEFS: HomeIconDef[] = [
  ...
  { key: 'dazza_ai',       label: 'Dazza AI',        icon: Bot,              href: '/dazza-ai',               bg: 'bg-violet-700',  fg: 'text-white', group: 'management', platformOnly: true },
...
export function resolveHomeIcons(
  ...
  const live = ALL_HOME_ICONS.filter(i => {
    if (i.platformOnly && !isPlatformOwner) return false;
