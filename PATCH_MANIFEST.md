...
Prepared 2026-08-16 from the raw Airo download at `C:\Users\daryl_ey\Downloads\IWILLBUILD_Portal2\IWILLBUILD_Portal` and the GitHub `main` baseline commit `18938cb5cbcb9fa4b21529e337c480d807b4cfbc`.
...
- 39 payload files: 36 retained delivery files, 2 version files and the local ESM build fix in `vite.config.ts`, with `DELETE_FILES.txt` as the control record.
...
- Retained overlay comparison: exactly 36 files absent from the raw download.
- New Patch payload: 39 files including the two version files and `vite.config.ts`, plus `DELETE_FILES.txt`.
- Package, Capacitor and both Xcode build configurations agree on Build 14.
- Production build and Capacitor sync must be rerun for this new release before it is pushed or archived.
