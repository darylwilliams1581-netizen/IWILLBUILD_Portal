...
// ---------------------------------------------------------------------------
const isE2ERun = process.env.PW_E2E === "1" || process.env.VITE_E2E === "1" || process.env.NODE_ENV === "test" || process.env.CI === "1";
...
function e2eDbStubPlugin(): Plugin {
  ...
  return {
    ...
    resolveId(source: string, importer?: string) {
      ...
      if (importer && importer.startsWith(dbDir) && (source === "./config" || source === "./config.js" || source === "./config.ts")) {
        return configStub;
      }
...
export default defineConfig(({
  ...
  //      leaflet.js (stale disk-cached chunk) and returns the same stub.
  {
    ...
    transform(_code: string, id: string) {
      if (id.includes('leaflet') && (id.includes('node_modules') || id.includes('.vite/deps'))) {
        return {
          code: 'export default {}; export const map = () => ({}); export const tileLayer = () => ({addTo:()=>({})});',
          map: null
        };
      }
    },
  ...
  e2eDbStubPlugin(), apiDevPlugin(),
  // Optional builder-only Vite plugins — skipped when null (local dev)
  formatOverridesPlugin ? (formatOverridesPlugin as (d: string) => Plugin)(__dirname) : null, contentPlugin ? (contentPlugin as () => Plugin)() : null, ...(mode === "development" ? [devToolsPlugin ? (devToolsPlugin as () => Plugin)() : null, fullStoryPlugin ? (fullStoryPlugin as () => Plugin)() : null, errorInterceptorPlugin ? (errorInterceptorPlugin as () => Plugin)() : null, mediaVersionsPlugin ? (mediaVersionsPlugin as () => Plugin)() : null] : [])].filter(Boolean) as Plugin[],
  resolve: {
    ...
    // SSR (the stub satisfies all named imports as no-op components).
    {
      ...
      customResolver() {
        return path.resolve(__dirname, 'src/fallbacks/icon-stub.ts');
      }
    }, {
      find: /^@heroicons(\/.*)?$/,
      replacement: path.resolve(__dirname, 'src/fallbacks/icon-stub.ts'),
    ...
    // the SSR render doesn't crash.
    {
      ...
      customResolver() {
        return path.resolve(__dirname, 'src/fallbacks/motion-stub.ts');
      }
    },
    ...
    // ~1 MB of AST from the SSR render phase.
    {
      ...
      customResolver() {
        return path.resolve(__dirname, 'src/fallbacks/better-auth-client-stub.ts');
      }
    },
    ...
    // succeeds without bundling the full 14 MB package.
    {
      ...
      customResolver() {
        return path.resolve(__dirname, 'src/fallbacks/opentelemetry-stub.ts');
      }
    },
    ...
    // bundled into server.bundle.mjs so photo upload/download works at runtime.
      ...
      find: 'nothing',
      replacement: '/src/fallbacks/missingModule.ts'
    },
    // ── dev-tools alias ────────────────────────────────────────────────────
    ...
    // the dev server and E2E runner both get a valid (empty-shape) module.
    {
      ...
      find: '@/api',
      replacement: path.resolve(__dirname, './src/server/api')
    },
  ...
  ssr: {
    ...
    'pdfjs-dist', 'react-pdf', '@napi-rs', '@napi-rs/canvas', 'canvas',
  ...
  plugins: [devToolsPlugin ? (devToolsPlugin as () => Plugin)() : null].filter(Boolean) as Plugin[]
})));
