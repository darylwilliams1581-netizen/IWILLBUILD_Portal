...
import { isCrossCompanyDataRequest, isHealthCheckIntent, tryContextLocalTool, tryStatelessLocalTool, isQuickUrgentIntent } from "./localTools.js";
...
function buildNoOpenAiFallback(message: string, warnings: string[], context?: any): string {
  const lines = [
    dazzaOpening(true),
    "",
...
export async function handleDazzaChat(input: DazzaChatInput): Promise<DazzaChatResponse> {
  ...
  const context = await loadDazzaContext(input.user, input.adapter);
  ...
  if (isQuickUrgentIntent(trimmedMessage)) {
    const overdueTodos = context.modules.jobTodos.data.filter(t => !["done","complete","completed"].includes((t.status||"").toLowerCase()) && t.dueDate && new Date(t.dueDate) < new Date()).length;
    const overdueService = context.modules.fleet.data.filter(a => a.nextServiceDate && new Date(a.nextServiceDate) < new Date()).length;
    const highRiskNoSwms = context.modules.jobs.data.filter(j => (j.highRisk || (j.riskLevel||"").toLowerCase().includes("high")) && !context.modules.safety.data.some(s => s.jobId === j.id && (s.type||"").toLowerCase().includes("swms"))).length;
  ...
  return {
    reply: buildNoOpenAiFallback(trimmedMessage, context.warnings, context),
    mode: "context",
    findings: [],
